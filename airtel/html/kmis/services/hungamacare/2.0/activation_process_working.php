...
if ($con) {
    foreach ($_POST['circle'] as $key => $circleValue) {
        ...
        if ($id) {
            ...
            echo $selectQry = "SELECT start_time,end_time FROM master_db.tbl_doubleconsent_time WHERE sId='12'";
            //$selectQry_array = mysql_query($selectQry);
            $result = mysql_query($selectQry);
            echo $result_row = mysql_num_rows($result);echo "<br/>";
