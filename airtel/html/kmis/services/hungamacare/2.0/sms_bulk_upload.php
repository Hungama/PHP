...
if (service_info==1511)
{
var date=document.forms[type]["dpd2"].value;
if(date=='')
{
bootstrap_alert.warning('Please select the schedule date.');	
}
}
...
<div class="container">
	<div class="row">
       ...
       /*  function change_url(val) {
            window.location="sms_bulk_upload_q.php?service="+val;
			onchange="change_url(this.value);" selected
        } */
		
    </script>
						<select name="service_info" id="service_info" onchange="javascript:getService(this.value)" >
...
<!--code add by rahul-->					
<tr id="sche_mode" style="display:none">
...
<!--code end by rahul-->
		 <tr>
       <td align="left" width="16%" height="32"><span>Message For</span></td>
        <td>
	...
	<!-- <tr>
       <td align="left" width="16%" height="32"><span>Scrubbing Status</span></td>
        <td>
			...
			<INPUT TYPE="radio" NAME="ss" value="nonscrubed" checked> Non-Scrubbing 
					...
					<tr>
						<INPUT TYPE="hidden" NAME="ss" value="nonscrubed" class="in">
	...
	<div class="well well-small"><?php echo FILE_UPLOAD_SMS.'-'.FILE_UPLOAD_MESSAGE;?></div>
...
$('#loading').hide();
	$('#grid-active').hide();
	$('#grid-active').html('');
//code for hide and display in
function getService(sid)
{
document.getElementById('sche_mode').style.display='none';	
if(sid=='1511')
{
document.getElementById('sche_mode').style.display='table-row';	
}
