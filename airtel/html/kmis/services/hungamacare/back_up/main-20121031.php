...
<?php
    if ($_POST['Submit'] == "Submit" && $_POST['msisdn'] != "")
    {
		...
		if($_POST['service_info'] == 1509)
			$service_info=1511;
		if($_POST['service_info'] == 1504)
			$service_info=1511;
		if($_POST['service_info'] == 15071)
			$service_info=1507;
		...
		$select_query2="select msisdn, response_time, chrg_amount,circle from master_db.tbl_billing_success_backup nolock where msisdn='$_POST[msisdn]' and service_id=".$service_info." and plan_id IN (".implode(",",$planArray).")";
		...
		if ($numRows > 0)
        {
			...
			$select_query1.=" where ANI='$_POST[msisdn]' and status=1 "; 
			/*if($service_info == 15022) { 
				$select_query1.=" and DNIS = '5464623' "; 
			} elseif($service_info == 1502) { 
				$select_query1.=" and DNIS NOT IN ('5464623') "; 
			}*/
