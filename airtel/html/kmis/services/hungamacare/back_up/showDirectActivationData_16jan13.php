...
<div class='txt' align='right'><a href='DirectActivation.php'>Add Time-Slots</a>&nbsp;&nbsp;&nbsp;</div><br/>

<form name="tstest" action='showDirectActivationData.php' method='POST' onSubmit="return validateData();">
...
if($_REQUEST['service']) {
	...
	if($_REQUEST['id'] && $_REQUEST['act']) {
		...
		if($actionType == 'strt') {
			$startQuery = "UPDATE master_db.tbl_direct_act_time SET status=1 WHERE id=".$actionId;	
			mysql_query($startQuery);
		} elseif($actionType == 'end') {
			$endQuery = "UPDATE master_db.tbl_direct_act_time SET status=0 WHERE id=".$actionId;	
			mysql_query($endQuery);
		} elseif($actionType == 'del') {
			$delQuery = "DELETE FROM master_db.tbl_direct_act_time WHERE id=".$actionId;	
			mysql_query($delQuery);
		} 
	}

	$query = "select t.id,s.servicename,s.service_id,t.start_time,t.end_time,t.status,s.circle from master_db.tbl_direct_act s INNER JOIN master_db.tbl_direct_act_time t ON s.id=t.sId WHERE s.service_id='".$serviceId."'";
	...
	?>	
		<tr>
			...
			<td bgcolor="#FFFFFF" height="30px" align="center"><a href='showDirectActivationData.php?id=<?php echo $sId;?>&act=<?php echo $action;?>&service=<?php echo $serviceId;?>&circle=<?php echo $circle;?>' onclick='javascript: return showStatusConfirm("<?php echo $action;?>");'>Start/Stop</a> | <a href='showDirectActivationData.php?id=<?php echo $sId;?>&act=del&service=<?php echo $serviceId;?>&circle=<?php echo $circle;?>' onclick='return showDeleteConfirm();'>Delete</a></td> <!-- return showStatusConfirm(<?php echo $action;?>); -->
		</tr>
	<?php $i++; } ?>
		</table>
	<?php } else { ?>
