...
$select_scenerios_qry = "select rule.id from master_db.tbl_rule_engagement rule, master_db.tbl_rule_engagement_action action 
                         ...
                         and rule.filter_base='" . $details['filter_base'] . "' and rule.scenerios='" . $details['scenerios'] . "' 
                          ...
                          and action.action_name='" . $action_name . "' and action.sms_type='" . $sms_type . "' 
                              and rule.id=action.rule_id and rule.status in (1,5)";
...
if ($action_name == 'sms') {
  ...
  <div class=\"alert alert-danger\">Please upload file of less than 25,000 numbers otherwise it will not process.</div></div>";
     ...
     values('$rule_id[0]', '$escape_msg',1)";
        ...
        $insertquery = "insert into master_db.tbl_rule_engagement_action(rule_id,action_name,sms_sequence,sms_cli,time_slot,upload_file,status,added_on,added_by,cc_user,sms_type) 
     values('$rule_id[0]', '$action_name','$sms_sequence', '$sms_cli', '$time_slot', '$createfilename',1,now(),'$loginid', '$add_cc','$sms_type')";
    }
} else {
