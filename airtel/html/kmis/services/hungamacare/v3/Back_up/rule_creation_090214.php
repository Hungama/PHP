...
//print_r($services);
//$serviceArray1 = array('1515' => 'AirtelDevo');
$serviceArray1 = array('1515' => 'AirtelDevo', '1513' => 'AirtelMND', '1517' => 'AirtelSE', '1502' => 'Airtel54646', 'AirtelMNDKK' => 'AirtelMNDKK', '1511' => 'AirtelGL', '1518' => 'AirtelComedy');
    ...
    <body onload="javascript:viewUploadhistory('me');">
        ...
        <script>
            ...
            function setSmsCli(type) { 
                ...
                //                smscli[1515["promotional"]] = "A$-601666";
                //                smscli[1515["engagement"]] = "AX-HMDEVO";alert(smscli[service[type]]);
                
                if(type == 'promotional'){
                    document.forms["form-action"]["sms_cli"].value= "601666";
                }else{
                    if(service == '1515'){
                        document.forms["form-action"]["sms_cli"].value= "HMDEVO";
                    }else if(service == '1513'){
                        document.forms["form-action"]["sms_cli"].value= "HMLIFE";
                    }else if(service == '1517'){
                        document.forms["form-action"]["sms_cli"].value= "HMEDUT";
                    }else if(service == '1502'){
                        document.forms["form-action"]["sms_cli"].value= "HMMUSC";
                    }else if(service == 'AirtelMNDKK'){
                        document.forms["form-action"]["sms_cli"].value= "HMLIFE";
...
//                }
                ...
                //                var sms_cli_value = smscli[service[type]];alert(sms_cli_value);
            ...
            function checkEditfield(type) { 
                ...
                if(current_date == last_processed_date){ 
                    ...
                    var last_time_slot_split_array = last_processed_time.split(":"); 
                    var last_hour = parseFloat(last_time_slot_split_array[0]);
                    var last_min_AmPm_spit_array = last_time_slot_split_array[1].split(" ");
                    var last_min = parseFloat(last_min_AmPm_spit_array[0]);
                    var last_timing = last_min_AmPm_spit_array[1];
                    
                    var current_time_slot_split_array = time_slot.split(":"); 
                    var current_hour = parseFloat(current_time_slot_split_array[0]);
                    var current_min_AmPm_spit_array = current_time_slot_split_array[1].split(" ");
                    var current_min = parseFloat(current_min_AmPm_spit_array[0]);
                    var current_timing = current_min_AmPm_spit_array[1];
                    if(last_timing == 'AM' && current_timing == 'AM'){ 
                        if((current_hour > last_hour) || ((current_hour == last_hour) && (current_min > last_min) )){
                            alert('Today this rule has been executed. Selected time slot should be less than rule execution time');
                            return false;
                        }
                    }
                    else if(last_timing == 'PM' && current_timing == 'PM'){ 
                        if((current_hour > last_hour) || ((current_hour == last_hour) && (current_min > last_min) )){
                            alert('Today this rule has been executed. Selected time slot should be less than rule execution time');
                            return false;
                        }
                    }else if(last_timing == 'AM' && current_timing == 'PM'){ 
                        alert('Today this rule has been executed. Selected time slot should be less than rule execution time');
                        return false;
                    }
                }
                ...
                if (action_name == 'email'){
                    ...
                    if((hour < 9 && timing == 'AM') || ((hour == 9 || hour == 10 || hour == 11) && timing == 'PM')){
