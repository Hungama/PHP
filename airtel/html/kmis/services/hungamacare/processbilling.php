...
//echo "Conn: ".$dbConn;
	...
	{		
		...
		switch($upload_for)
		{
			case 'active':
				$reqtype=1;
				if($service_id==15211 || $service_id==15212 || $service_id==15213) $service_id1=1521;
				else $service_id1=$service_id;

				$amtquery="select iAmount from master_db.tbl_plan_bank where Plan_id='$price' and S_id=$service_id1";
		...
		for($i=0;$i<$file_size;$i++)
		{
			...
			if(strlen(trim($msisdn))==10)
			{
				...
				{	
					...
					if($service_id=='1515') {
						$langRelData = explode("-",$service_type);
						$lang=$langRelData[0];
						$Religion=$langRelData[1];	
						...
						if($Religion) { 
							...
							if($countRel) {									
								$updateReligion = "UPDATE airtel_devo.tbl_religion_profile SET lastreligion_cat='".strtolower($Religion)."',lang='".$lang."' WHERE ANI='".$msisdn."'";
