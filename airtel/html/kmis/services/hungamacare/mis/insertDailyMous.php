...
and dnis not in('546461','546461000','5464612','5464640') and dnis not like '%P%' and operator='airm' group by circle,status";
$mous_tf_result = mysql_query($mous_tf_query, $dbConn) or die(mysql_error());
$numRows2 = mysql_num_rows($mous_tf_result);
if ($numRows2 > 0)
{
...
$mous_tf=array();
$mous_tf_query="select 'MOU_TF',circle, count(id),'AirtelEU' as service_name,date(call_date),sum(duration_in_sec)/60 as mous from mis_db.tbl_AMU_calllog
where date(call_date)='$view_date1' and dnis like '546469%' and operator='airm' and dnis != '5464694' group by circle";
$mous_tf_query .=" Union select 'MOU_TF',circle, count(id),'AirtelEU' as service_name,date(call_date),sum(duration_in_sec)/60 as mous from mis_db.tbl_AMU_calllog
...
$mous_tf=array();
$mous_tf_query="select 'MOU_TF',circle, count(id),'AirtelEU' as service_name,date(call_date),sum(duration_in_sec)/60 as mous,status from 
mis_db.tbl_AMU_calllog where date(call_date)='$view_date1' and dnis like '546469%' and operator='airm' and dnis != '5464694' group by circle,status";
$mous_tf_query .=" Union select 'MOU_TF',circle, count(id),'AirtelEU' as service_name,date(call_date),sum(duration_in_sec)/60 as mous,status from 
mis_db.tbl_AMU_calllog where date(call_date)='$view_date1' and dnis = '5464694' and circle in('kol','wbl') and operator='airm' group by circle,status";
$mous_tf_result = mysql_query($mous_tf_query, $dbConn) or die(mysql_error());
$numRows2 = mysql_num_rows($mous_tf_result);
if ($numRows2 > 0)
{
