...
where date(call_date)='$view_date1' and dnis like '510168%' and operator='airm' group by circle";
...
$uu_tf = array();
$uu_tf_query = "(select 'UU_T_6',circle, count(distinct msisdn),'AirtelDevo' as service_name,date(call_date),status,'Non Active' as 'user_status' from mis_db.tbl_devotional_calllog where date(call_date)='$view_date1' and dnis like '510168%' and operator='airm' and status in(-1,11,0) AND MSISDN  NOT IN( select DISTINCT MSISDN from mis_db.tbl_devotional_calllog where date(call_date)='$view_date1' and dnis like '510168%' and operator='airm' and status IN (1)) group by circle)";
$uu_tf_query .= " UNION (select 'UU_T_6',circle, count(distinct msisdn),'AirtelDevo' as service_name,date(call_date),status,'Active' as 'user_status' from mis_db.tbl_devotional_calllog where date(call_date)='$view_date1' and dnis like '510168%' and operator='airm' and status=1 group by circle)";
