...
where date(response_time)=date(date_sub(now(),interval 2 day)) 
and event_type='SUB' and mode='USSD_Retail' and date(b.referDate) between date(date_sub(now(),interval 4 day)) and date(date_sub(now(),interval 2 day)) 
...
{
	$getMsisdnRecord="select max(id) 'id' from master_db.tbl_refer_ussdData where friendANI= ".$chrgData['msisdn']." and date(referDate) between  date(date_sub(now(),interval 2 day)) 
and date(date_sub(now(),interval 2 day)) and status=5 and service_id=1517 and optServiceId= ".$chrgData['service_id'];
	$getResult1=mysql_query($getMsisdnRecord);
	$Record=mysql_fetch_array($getResult1);

	echo $updateQuery="update master_db.tbl_refer_ussdData set status=5,chrgAmount=".$chrgData['chrg_amount'].",chrgDate=date(date_sub(now(),interval 2 day)) where date(referDate) between date(date_sub(now(),interval 4 day)) and date(date_sub(now(),interval 2 day)) and  friendANI=".$chrgData['msisdn']." and status=5 and service_id=1517 and optServiceId=".$chrgData['service_id']." and id=".$Record['id'];
