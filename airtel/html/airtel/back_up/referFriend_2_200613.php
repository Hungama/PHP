...
if ($reqtype == 0) {
    ...
    switch ($serviceId) {
        case '1517':
            ...
            $response = 'Welcome to Spoken English.' . "\n" . "Reply" . "\n" . "3 for Spoken English (2 Rs for 1 Day)" . "\n" . "4 for EU (2 Rs for 1 Day)" . "\n" . "5 for SARNAM 
                            (2 Rs for 1 Day)" . "\n" . "6 for MND (2 Rs for 1 Day)";
...
if ((strlen($reqtype) > 2) && $serviceId == 1517 && $retData) {
    ...
    if ($planid) {
        if (strlen($reqtype) == 10 || strlen($reqtype) == 12) {
            ...
            switch ($optServiceId) {
                case '1517':
                    $response = "Thank you for referring this customer to Spoken English service.";
                    $message = "Aptech certified Spoken English course apke mobile par ghar baithe. Rs X for XX days. Activate karne ke liye reply kare 1 se.";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
                case '1501':
                    $response = "Thank you for referring this customer to Entertainment Unlimited service.";
                    $message = "Paayein Apne pasandida gaane or banayein apni Hello Tunes sirf Entertainment Unlimited par Sirt Rs X for XX days. Abhi subscribe karne ke liye Reply kare 1 se.";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
                case '1515':
                    $response = "Thank you for referring this customer to SARNAM service.";
                    $message = "Sarnam par sune sarv dharm ki bhakti rachnai jitna mann chahe sirf Rs X for XX Days, abhi subscribe karne ke liye reply kare 1 se";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
                case '1513':
                    $response = "Thank you for referring this customer to My Naughty diary service.";
                    $message = "Sune Shararti naina ki naughty kiss se sirf My naughty diary par, sirt Rs X for XX days ke liye subscribe karne ke liye reply kare 1 se";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
