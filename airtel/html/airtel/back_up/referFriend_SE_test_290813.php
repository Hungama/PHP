...
if ((strlen($reqtype) > 2) && $serviceId == 1517 && $retData) {
    ...
    if ($planid) {
        if (strlen($reqtype) == 10 || strlen($reqtype) == 12) {
			...
			if($userplanId12){
				//$status='-1';
                            $updateQuery=" update master_db.tbl_refer_ussdData set status='-2' where date(referDate)=date(now()) and service_id=1517 and friendANI=".$frndMDN." and optServiceId=".$optServiceId."";
			...
			//$sub="select count(*) from ".$db.".".$subscriptionTable." where ANI='$frndMDN' and status=1";
                        $sub = "select ani,status from " . $db . "." . $subscriptionTable . " where ANI='$frndMDN'";
			$qry1=mysql_query($sub);
			$rows1 = mysql_fetch_row($qry1);
                        $status1 = $rows1[1];
			if($rows1[0]<=0) 
			{
				$qry="call ".$db.".". $subscriptionProcedure." ('".$frndMDN."','".$lang."','".$mode."','".$sc."','".$amount."',".$optServiceId.",".$userplanId.")";	$qry1=mysql_query($qry) or die( mysql_error() );					
			}
			else {
                ...
                switch ($status1) {
                    ...
                    case '11':
                        ...
                        break;
                    case '0':
                        $response = "Dear Retailer, Customer " . $frndMDN . " is in queue to the " . $sName1 . " . Please select any other service";
                        break;
                    case '5':
                        $response = "Dear Retailer, Customer " . $frndMDN . " is in billing retry process to the " . $sName1 . " . Please select any other service";
                        break;
                    default :
                       $response = "Dear Retailer, Customer " . $frndMDN . " is in  process to the " . $sName1 . " . Please select any other service";
                        break;
                }
                $status = '-1';
             }
