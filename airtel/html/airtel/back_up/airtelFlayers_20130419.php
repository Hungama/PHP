...
$QueryMNDWhl = mysql_query("SELECT count(1) FROM airtel_mnd.tbl_ussdMND_whitelist WHERE ANI='".$msisdn."'");
list($mndWhlCount) = mysql_fetch_array($QueryMNDWhl);

if($mndWhlCount && $circle!='KAR') $reqtype=1;
//-------------------------- Code end here ---------------------------------------
...
if(($reqtype=='1' && $s_id!='1513') || ($reqtype=='1' && $s_id=='1513' && $circle!='KAR') || ($reqtype=='5' && $s_id=='1513' && $circle=='KAR')) {		
	$sub="select count(*) from ".$db.".".$subscriptionTable." where ANI='$msisdn' and status=1";
	$qry1=mysql_query($sub);
	$rows1 = mysql_fetch_row($qry1);	
	if($rows1[0]<=0) {
