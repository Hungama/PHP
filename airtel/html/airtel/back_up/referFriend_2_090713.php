...
if ($reqtype == 0) {
    ...
    switch ($serviceId) {
        case '1517':
			...
			$response = "Select Services." . "\n" . "Press key from 3 to 5:" . "\n" . "3 Spoken English (Rs30/15d)" . "\n" . "4 EntUnLtd (Rs30/15d)" . "\n" . "5 MND (Rs30/30d)"."\n"."Reply";
...
if ((strlen($reqtype) > 2) && $serviceId == 1517 && $retData) {
    ...
    if ($planid) {
        if (strlen($reqtype) == 10 || strlen($reqtype) == 12) {
            ...
            switch ($optServiceId) {
                case '1517':
                    $response = "Thank you for referring your customer to Spoken English service.";
                    $message = "Aptech certified Spoken English course apke mobile par ghar baithe. Rs 30 for 15 days. Activate karne ke liye reply kare 1 se.";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
                case '1501':
                    $response = "Thank you for referring your customer to Entertainment Unlimited service.";
                    $message = "Paayein Apne pasandida gaane or banayein apni Hello Tunes sirf Entertainment Unlimited par Sirf Rs 30 for 15 days. Abhi subscribe karne ke liye Reply kare 1 se.";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
//                case '1515':
...
//                    break;
                case '1513':
                    $response = "Thank you for referring your customer to My Naughty diary service.";
                    $message = "Sune Shararti naina ki naughty kiss se sirf My naughty diary par, sirt Rs 30 for 30 days ke liye subscribe karne ke liye reply kare 1 se.";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
