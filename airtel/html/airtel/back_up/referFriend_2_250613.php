        ...
        if (strlen($reqtype) == 10 || strlen($reqtype) == 12) {
            ...
            switch ($optServiceId) {
                case '1517':
                    ...
                    $message = "Aptech certified Spoken English course apke mobile par ghar baithe. Rs X for XX days. Activate karne ke liye reply kare 1 se(Rs30/15d).";
                    $from = "571811";
                ...
                case '1501':
                    ...
                    $message = "Paayein Apne pasandida gaane or banayein apni Hello Tunes sirf Entertainment Unlimited par Sirt Rs X for XX days. Abhi subscribe karne ke liye Reply kare 1 se(Rs30/15d).";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
                case '1515':
                    $response = "Thank you for referring this customer to SARNAM service(Rs10/7d).";
                    $message = "Sarnam par sune sarv dharm ki bhakti rachnai jitna mann chahe sirf Rs X for XX Days, abhi subscribe karne ke liye reply kare 1 se(Rs10/7d).";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
                case '1513':
                    $response = "Thank you for referring this customer to My Naughty diary service(Rs30/30d).";
                    $message = "Sune Shararti naina ki naughty kiss se sirf My naughty diary par, sirt Rs X for XX days ke liye subscribe karne ke liye reply kare 1 se(Rs30/30d).";
                    $from = "571811";
                    $sndMsgQuery1 = "CALL master_db.SENDSMS('" . $frndMDN . "','" . $message . "','" . $from . "',4,'" . $from . "','RET')";
                    break;
