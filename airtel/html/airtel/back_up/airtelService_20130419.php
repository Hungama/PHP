...
if($msisdn=='9910040744' || $msisdn=='918527000779' || $msisdn=='+918527000779') $circle='UPW';

/*if((strlen($reqtype)==10 || strlen($reqtype)==12) && $planid==41 && $serviceId=='1517') {  
	$serviceId='1514';
} else {
	$serviceId='1517';
} */
...
switch($serviceId)
{
	...
	case '1515':
		...
		{
		$subscriptionProcedure="DEVO_USSD_SUB";
		//exit;
       }
		else
	...
	case '1511':
		...
		$actSucc = "Thanks for subscribing to GOODLIFE! Listen and Learn Indian or International Recipe. To listen, just dial 55001(Toll free).";
		$actFail = "Sorry! Your subscription to GOODLIFE service could not be processed due to some technical problem. Please try again later.";
		$actAlr = "You are already subscribed to GOODLIFE service.";
		$deactSucc = "Thanks! your request for unsubscription has been processed successfully.To restart the service dial 55001 or *321*001#.";
		$deactFail = "Sorry! Your request to Stop GOODLIFE service could not be processed due to some technical problem. Please try again later.";
		$deactAlr = "You are already unsubscribed from GOODLIFE service.";		
	break;
}
...
if($serviceId == 1) $serviceArray = array(1514,1517);
else $serviceArray = array($serviceId);
...
//echo $referCount; echo $serviceId; die;
	...
	if($reqtype == 0) {
		...
		switch($serviceId)
		{
			...
			case '1518': $response='Enjoy Mana Paata Mana Comedy at Rs.30/30days. Listen comedy clips and laugh'."\n".'Reply '."\n".'1 to Subscribe '."\n".'2 to Unsubscribe ';  
				header("Menu code:".$response);				
			break;
			case '1511': //$response='Welcome';
						//Subscribe & learn Indian or International recipe. 1 to Subscribe 2 to Unsubscribe Charges Rs.30 per 30 days.'."\n"."Reply";
						$response='Welcome to GOODLIFE! Listen & Learn Indian or International recipe, Charges @Rs.30/30 days. Please press'."\n".'1 to Subscribe '."\n".'2 to Unsubscribe '."\n"."Reply";  //."\n"."Charges Rs.30/month."
					//$response='Welcome to GOODLIFE! Your daily dose of love, dating, relationship & fashion tips. Please press'."\n".'1 to Subscribe '."\n".'2 to Unsubscribe '."\n"."Charges Rs.30/month."."\n"."Reply";  
				header("Menu code:".$response);								
			break;
		}
	...
	if(($reqtype=='1' || $reqtype=='2') && !$planid) {
		...
		switch($serviceId) 
		{
			case '1517': if($reqtype=='1') $response = "Welcome to Spoken English. "."\n"."Reply"."\n"."1 For Subscribe";
						 ...
						 //$response='Welcome to Spoken English. '.$data."\n".'Reply '."\n".$resData;									
				header('Menu code:'.$response);
			break;	
			case '1515': $response='Welcome to SARNAM. Subscribe @Rs.10/7 day & listen Bhakti songs.'."\n"."Reply"."\n"."1 to Subscribe"."\n"."2 to Unsubscribe";  
				header('Menu code:'.$response);
			...
			case '1518': if($i==1) { 
				...
				header('Menu code:'.$response);
			break;	
			case '1511': $response='Welcome to GOODLIFE! Your daily dose of love, dating, relationship & fashion tips. Please press'."\n".'1 to Subscribe '."\n".'2 to Unsubscribe '."\n"."Charges Rs.30/month."."\n"."Reply";  
						//$response='Hello, Welcome to Good Life.'."\n"."Subscribe & learn Indian/International recipe @Rs.30/30 days."."\n"."Reply"."\n".'1 to Subscribe '."\n".'2 to Unsubscribe';
				header("Menu code:".$response);				
			break;
		}
		header('Freeflow: FC');
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$logData=$msisdn."#".$planid."#".$serviceId."#".$reqtype."#".$circle."#".$qry."#Response:Freeflow:FB#".$response."#".date("Y-m-d H:i:s")."#st".$startSleepTime."#et".$endSleepTime."\n";
		error_log($logData,3,$logPath);
		if($serviceId == '1518' || $serviceId == '1515') {
	...
	//echo $reqtype.",".$serviceId.",".$planid; 
	if(($reqtype=='2') && $planid) //|| ($reqtype=='5' && $serviceId=='1511')
	{			
		//if(($reqtype=='5' && $serviceId=='1511') || ($reqtype=='2' && $serviceId!='1511')) {
			$sub="select count(*) from ".$db.".".$subscriptionTable." where ANI='$msisdn'";
			$qry1=mysql_query($sub);
			$rows1 = mysql_fetch_row($qry1);	
			...
			header('Freeflow: FB');
		/*} elseif($reqtype=='2' && $serviceId=='1511') {
			$response="To unsubscribe, press 5 to confirm"."\n"."Reply";
			header('Freeflow: FC');
		}*/
