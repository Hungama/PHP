...
if((strlen($reqtype)>2) && ($serviceId==1514 || $serviceId==1517 || $serviceId==1513) && $retData) {	
	...
	if(!$planid) {
		switch($serviceId) 	{			
			case '1514':
			case '1517': $response="To subscribe to following services,"."\n"."Reply."."\n"."1.Spoken English"."\n"."2.Personality Development"; 
			break;	
		}
		header('Freeflow: FB');
		header('charge: y');
		...
		switch($serviceId) {			
			case '1514': $message = "Dear Customer, English seekhna hua aur bhi aasan, saath main paye Aptech certification bhi. Is pack ko paane ke liye, SMS karein Yes";
			...
			break;
			case '1517': if($circle=="UPW") {
							$message = "Aptech certified Spoken English course apke mobile par ghar baithe. Rs 5 per day for 90 days. Activate karne ke liye reply kare 1 se.";
						 } else {
							$message = "Job aur padai mein tarakki ke liye seekhiye Spoken English apne mobile par! Subscribe karne ke liye reply mein bhejiye YES. Rs.5/day. Aptech certified!";
						 }
						 $response="Your Request to start Spoken English Service has been submitted successfully."; 
						 $from="571811";
						 $sndMsgQuery1 = "CALL master_db.SENDSMS('".$frndMDN."','".$message."','".$from."',4,'".$from."','Sub_OK')"; 
			break;
			case '1513': $message = "Naina, BPO Ex Maya aur actress Kajal ki zindagi ke rangeen raaz jaanne ke liye My Naughty Dairy subscribe karne ke liye  isi number pe Yes reply karein (tollfree)"; //5500196
		...
		$queryF = "INSERT INTO master_db.tbl_refer_ussdData VALUES ('','".$msisdn."','".$frndMDN."',NOW(),adddate(NOW(),3),'".$serviceId."','Retailer');";
		mysql_query($queryF);		
		...
		header('Response:'.$response);
		echo $response;
		$logData=$msisdn."#".$frndMDN."#".$planid."#".$serviceId."#".$reqtype."#".$circle."#".$qry."#Response:Freeflow:FB#".$response."#".$sndMsgQuery1. "#".date("Y-m-d H:i:s")."\n";
		error_log($logData,3,$logPath);
		error_log($msisdn."#".$reqtype."#".$circle."#".date("Y-m-d H:i:s")."\n",3,$logNewPath);
	}
} 
...
{	
	...
	$queryF = "INSERT INTO master_db.tbl_refer_ussdData VALUES ('','".$msisdn."','".$friendMDN."',NOW(),adddate(NOW(),3),'".$serviceId."','Friend');";
	mysql_query($queryF);
