...
if((strlen($reqtype)>2) && ($serviceId==1514 || $serviceId==1517 || $serviceId==1513) && $retData) {	
	...
	elseif($planid) {
		...
		$queryF = "INSERT INTO master_db.tbl_refer_ussdData VALUES ('','".$msisdn."','".$frndMDN."',NOW(),adddate(NOW(),3),'".$serviceId."','Retailer','".$userCircle."','',0,0,0,0,'');";
		mysql_query($queryF);		
...
{	
	...
	$queryF = "INSERT INTO master_db.tbl_refer_ussdData VALUES ('','".$msisdn."','".$friendMDN."',NOW(),adddate(NOW(),3),'".$serviceId."','Friend',0,0,0,0,'');";
	mysql_query($queryF);
