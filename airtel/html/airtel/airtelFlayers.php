...
/*if($msisdn==8527000779 || $msisdn==918527000779)
{
	echo "<pre>";
	print_r($_REQUEST);
	exit;
...
*/
$mode='USSD';
$realreqtype=$_REQUEST['reqtype'];
$reqtype=$_REQUEST['reqtype'];
if($reqtype=='*177*995#' || !isset($reqtype))
 $reqtype=0;
...
if($_REQUEST['test']) echo "Ret: ".$retData; 
...
//if($serviceId==1503 && ($circle == 'KER' || $msisdn==8527000779 || $msisdn==918527000779)) 
if($serviceId==1503 && ($circle == 'KER')) 
{
	$serviceId=1523;
	$planid=82;
...
//			$response1="Welcome to Manoranjana Pack. Listen to hit songs & Comedy. Subscribe @Rs.30/month and browse free 200 min. Reply"."\n"."1 to subscribe"."\n"."2 to unsubscribe"."\n"."Select Option";
			$response1="Comedy, Janapade geethegalu, hit hadugalu. Subscribe at just Rs.30/30days. Press 1 to subscribe Press 2 for Unsubscribe";
	...
	case '1507':
		...
		$lang='01';		
		$actSucc = "Your request to Start Radio GAGA service has been submitted successfully.";
		$actFail = "Sorry! Your subscription to Radio GAGA service could not be processed due to some technical problem. Please try again later.";
		$actAlr = "You are already subscribed to Radio GAGA service.";
		$deactSucc = "Your request to Stop Radio GAGA service has been submitted successfully. Thank you for using Radio GAGA service.";
		$deactFail = "Sorry! Your request to Stop Radio GAGA service could not be processed due to some technical problem. Please try again later.";
		$deactAlr = "You are already unsubscribed from Radio GAGA service.";
...
if(!$validateResponse)
{
	echo $response="Please provide the complete parameter1";
	$logData=$msisdn."#".$planid."#".$serviceId."#".$reqtype."#Response:".$response."#".date("Y-m-d H:i:s")."#".$realreqtype."\n";
...
if($reqtype == 0) {	
	...
	if($rows1[0]<=0) {
		if($isreq) {
			...
			header('Expires: -1');
			echo $response;
			$logData=$msisdn."#".$planid."#".$serviceId."#".$reqtype."#".$circle."#".$qry."#Response:Free flow state:FC#".$response."#".date("Y-m-d H:i:s")."#".$realreqtype."\n";;
			error_log($logData,3,$logPath);
...
if($reqtype=='1') {		
	...
	//$qry1=mysql_query($qry) or die( mysql_error() );					// changed by athar

	//$logData=$msisdn."#".$planid."#".$serviceId."#".$reqtype."#".$circle."#".$qry."#Response:Free flow state:FB#".$response."#".date("Y-m-d H:i:s")."\n";
	$logData=$msisdn."#".$planid."#".$serviceId."#".$reqtype."#".$circle."#".$qry."#Response:Freeflow:FB#".$response."#".date("Y-m-d H:i:s")."#st".$startSleepTime."#et".$endSleepTime."#".$realreqtype."\n";
...
{	
	...
	header('Response:'.$response);
	echo $response;
	$logData=$msisdn."#".$planid."#".$serviceId."#".$reqtype."#".$circle."#".$unsubsQuery."#Response:Free flow state:FB#".$response."#".date("Y-m-d H:i:s")."#".$realreqtype."\n";
	error_log($logData,3,$logPath);
