...
$allLogPathTest = "/var/www/html/airtel/logs/airtelService/" . $serviceId . "/AllLogsTest_" . date("Y-m-d") . ".txt";

$ipAddress = $_SERVER['REMOTE_ADDR'];
$logData1=$msisdn."#".$mode."#".$reqtype."#".$planid."#".$friendMDN."#".$serviceId."#".$ipAddress."#".date("Y-m-d H:i:s")."\n";
error_log($logData1,3,$allLogPathTest);
...
//if ($msisdn == '8587800665' || $msisdn == '8527000779' || $msisdn == '9910282375') {
if ($msisdn == '9910282375') {
	//eu contest for testing
    //include("/var/www/html/airtel/airtelService_euContest.php");
    //exit;
}
switch ($serviceId) {
    case '1517':
...
///////////////////////////////////// For Spoken English Retailer Mode    ///////////////////////////////////////////////////////////////////////////

if ($referCount && ($serviceId == 1517 || strlen($reqtype) == 10 || strlen($reqtype) == 12) && $reqtype != 1) {
    $circle = trim($circle);
    if ($msisdn == 9910040744 || $circle == "UPE")
        $referUrl = "/var/www/html/airtel/referFriend_SE_UPE.php";
    else
        $referUrl = "/var/www/html/airtel/referFriend_SE.php";
    include_once($referUrl);
    exit;
}
///////////////////////////////////// End For Spoken English Retailer Mode    ///////////////////////////////////////////////////////////////////////////

if ($referCount && ($serviceId == 1515 || $serviceId == 1514 || strlen($reqtype) == 10 || strlen($reqtype) == 12) && $reqtype != 1) {
    include_once($referUrl);
    exit;
}
...
if (!$validateResponse) {
    ...
    if ($reqtype == 0) {
        ...
        switch ($serviceId) {
            ...
            case '1501':
                if ($circle == 'KAR')
                    $response = 'Entertainment Unlimited at Rs.10/5days.Listen Unlimited Music,Gossip and download HT' . "\n" . 'Press 1 to subscribe' . "\n" . 'Press 2 to unsub' . "\n" . 'Reply';
                else if ($circle == 'MAH')
                    $response = 'Entertainment Unlimited at Rs.60/30days.Listen Unlimited Music,Gossip and download HT' . "\n" . 'Press 1 to subscribe' . "\n" . 'Press 2 to unsub' . "\n" . 'Reply';
                else
                    $response = 'Entertainment Unlimited at Rs.30/15days.Listen Unlimited Music,Gossip and download HT' . "\n" . 'Press 1 to subscribe' . "\n" . 'Press 2 to unsub' . "\n" . 'Reply';
                header("Menu code:" . $response);
                break;
        }
    ...
    if (($reqtype == '1' || $reqtype == '2') && !$planid) {
        ...
        elseif ($reqtype == '2') {
            ...
            $resData = "1 to Unsubscribe";
        }
        header('Path=/airtelSubUnsub');
        header('Content-Type: UTF-8');
        switch ($serviceId) {
            case '1517': if ($reqtype == '1')
