...
$logdate = date("Ymd");
$logPath = "/var/www/html/hungamawap/airtel/CCG/logs/sConsentResponse_" . $logdate . ".log";
...
$pubid=$affid_dgm_data[2];

if($afid=='1105' || $afid=='1114' || $afid=='1040')
{
	...
	if($randSwitch==1 || $randSwitch==2 || $randSwitch==3 || $randSwitch==4 || $randSwitch==5)
		...
		$DGMResponse='NOK';
			switch($afid)
				...
				case '1040': // tyro
						$randno=rand(1,9);
						$pubid=date('YmdHis').$randno;
						$dgmUrl="https://tda.tyroo.com/sdk/lead.php?cac=$Publisher_Ref_Id&leadid=$pubid&amount=0.0&status=confirmed";
						break;
			  }
				if (trim(strtoupper($msg)) == 'SUCCESS') {
					...
					if($afid=='1040')
					{
						$xml = simplexml_load_string($DGMResponse);
						$tyro_MessageID = $xml->MessageID;
						$tyro_Status = $xml->Status;
						$tyro_PiggybackStatus = $xml->PiggybackStatus;
						$DGMResponse=$tyro_MessageID."@@".$tyro_Status."@@".$tyro_PiggybackStatus;
					}
...
if (trim(strtoupper($msg)) == 'SUCCESS') {
		   ...
		   sleep(1);
		   $call_224 = "http://119.82.69.212/airtel/airtelWAPPostCCG.php?msisdn=".$MSISDN1."&amnt=".$amount;
...
$msisdnval_count_val2 = strlen($msisdnval2);
	...
	//save for Live MIS purpose start here-
$logPath_MIS="/var/www/html/hungamawap/airtel/CCG/logs/AllAirtelAfterCCGVisitorResponseMIS_".$logdate.".txt";
$logString_MIS = $zone_id . "|" . date('Y-m-d H:i:s')."|".$MSISDN1 . "|" . $Remote_add . "|" . $full_user_agent . "|" . $posturl . "|" . trim($msg) . "|" .$planid."|WAP|". $productId. "|117.239.178.108|" .urlencode($refererName)."|".$afid."|".urlencode($Publisher_Ref_Id)."|".urlencode($DGMResponse)."|".$dgmUrl."|".$old_sessionid."|".$circle."|WAPAirtelLDR|ccgresponse|"."\r\n";
error_log($logString_MIS, 3, $logPath_MIS);
$saveLiveMisWAPLogs = "http://119.82.69.212/kmis/services/hungamacare/2.0/wap/saveairtelLiveWAPlogs.php?zone_id=".$zone_id."&msisdn=".$MSISDN1."&msg=".urlencode($msg)."&afid=".$afid."&Publisher_Ref_Id=".urlencode($Publisher_Ref_Id)."&DGMResponse=".urlencode($DGMResponse)."&circle=".$circle."&service=WAPAirtelLDR&type=ccgresponse";
