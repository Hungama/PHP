...
$select_query = "select file_name, added_on, service_type, channel, price_point, upload_for,batch_id,service_id,amount,language,act_date,renew_date,smsFlag 
	from billing_intermediate_db.bulk_upload_history nolock where status=0 and upload_for in('topup_charging','event_active','event_unsub','autochurn_renewal','tryandbuy','ticket_chat','act_tnb') 
    ...
    for ($i = 0; $i < $file_size; $i++) {
        ...
        switch ($service_id) {
            case '1101': //MTSMU
				...
				$tnb_Act_subscriptionProcedure='mts_radio.RADIO_TNB_BULK';
                break;
            case '1102': //MTS 54646
                if ($upload_for == 'event_unsub' || $upload_for == 'autochurn_renewal')
				...
				$tnb_Act_subscriptionProcedure='mts_hungama.JBOX_TNB_BULK';
                break;
            ...
            case '1110': //MTS REDFM
				...
				$tnb_Act_subscriptionProcedure='mts_redfm.REDFM_TNB_BULK';
                break;
            case '1116': //MTSVA
                if ($upload_for == 'event_unsub' || $upload_for == 'autochurn_renewal')
				...
				$tnb_Act_subscriptionProcedure='mts_voicealert.VOICE_TNB_BULK';
                break;
            case '1113': //MTS MND
                if ($upload_for == 'event_unsub' || $upload_for == 'autochurn_renewal')
				...
				$tnb_Act_subscriptionProcedure='mts_mnd.MND_TNB_BULK';
                break;
            case '1111': //MTS BhaktiRas
				...
				$tnb_Act_subscriptionProcedure='dm_radio.DIGI_TNB_BULK';
                break;
            case '1123': //MTS Contest
				...
				$tnb_Act_subscriptionProcedure = 'Mts_summer_contest.CONTEST_TNB_BULK';
                break;
            case '1126': //MTS Regional
                if ($upload_for == 'event_unsub')
					...
					$tnb_Act_subscriptionProcedure='mts_Regional.REGIONAL_TNB_BULK';
                break;
            case '1125': //MTS JOKES
                if ($upload_for == 'event_unsub')
				...
				$tnb_Act_subscriptionProcedure='mts_JOKEPORTAL.JOKES_TNB_BULK';
                break;
        }
        if ($reqtype == 1) {
            $plan_id = $price;
        ...
        if ($reqtype == 3) {
		 ...
		 if ($reqtype == 4) {
            $plan_id = $price;
            $shortCode = getShortCode($service_id);
			$aniANDccg=explode("#",$msisdn);			
             $call = "call " . $tnb_Act_subscriptionProcedure . "('$aniANDccg[0]','$lang','$channel','$shortCode','$Newamount',$service_id,$price,'$batch_id','$aniANDccg[1]')";
			if (mysql_query($call, $dbConn)) {
                $res = 'SUCCESS';
            } else {
                //$res=mysql_error();
			...
			$log_file_path = "/var/www/html/hungamacare/bulkuploads/TNBUpload/tnbAct_" . date('Y-m-d') . ".txt";
            $LogString = $batch_id . "#" . $msisdn . "#" . $channel . "#" . date('H:i:s') . "#" . $price . "#" . $upload_for . "#" . $call . "#" . $res . "\n";
            error_log($LogString, 3, $log_file_path);
        }
    } // end of for 
