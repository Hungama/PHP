...
$uu_tf = array();
$uu_tf_query = "select 'C_UU_T_3',circle, count(distinct msisdn),'MTSIBD' as service_name,date(call_date) 
from mis_db.tbl_mtv_calllog nolock where date(call_date)='$view_date1' 
and dnis like '55789%' and chrg_rate=3 group by circle";
...
$uu_tf = array();
$uu_tf_query = "(select 'C_UU_T_3',circle, count(distinct msisdn),'MTSIBD' as service_name,date(call_date),status,'Non Active' as 'user_status' 
from mis_db.tbl_mtv_calllog where date(call_date)='$view_date1' and dnis like '55789%' and chrg_rate=3 and status in(-1,11,0) 
AND MSISDN  NOT IN ( select DISTINCT MSISDN from mis_db.tbl_mtv_calllog where date(call_date)='$view_date1' and dnis like '55789%' 
and chrg_rate=1 and status IN (1)) group by circle)";
$uu_tf_query .= "UNION (select 'C_UU_T_3',circle, count(distinct msisdn),'MTSIBD' as service_name,date(call_date),status,'Active' as 'user_status' 
from mis_db.tbl_mtv_calllog where date(call_date)='$view_date1' and dnis like '55789%' and chrg_rate=3 and status=1 group by circle)";
$uu_tf_result = mysql_query($uu_tf_query, $dbConn) or die(mysql_error());
$numRows4 = mysql_num_rows($uu_tf_result);
if ($numRows4 > 0) {
    while ($uu_tf = mysql_fetch_array($uu_tf_result)) {
...
$sec_tf = array();
$sec_tf_query = "select 'C_SEC_T_3',circle, count(msisdn),'MTSIBD' as service_name,date(call_date),sum(duration_in_sec) 
...
$sec_tf = array();
$sec_tf_query = "select 'C_SEC_T_3',circle, count(msisdn),'MTSIBD' as service_name,date(call_date),sum(duration_in_sec),status from 
mis_db.tbl_mtv_calllog nolock where date(call_date)='$view_date1' and dnis like '55789%' and chrg_rate=3 group by circle";
...
if ($numRows5 > 0) {
    while ($sec_tf = mysql_fetch_array($sec_tf_result)) {
        ...
        if ($sec_tf[6] == 1)
            $sec_tf[0] = 'C_L_SEC_T_3';
        elseif ($sec_tf[6] != 1)
            $sec_tf[0] = 'C_N_SEC_T_3';
...
$uu_tf = array();
$uu_tf_query = "select 'R_UU_T_3',circle, count(distinct msisdn),'MTSIBD' as service_name,date(call_date) 
from mis_db.tbl_reg_calllog nolock where date(call_date)='$view_date1' 
and dnis like '55789%' and chrg_rate=3 group by circle";
...
$uu_tf = array();
$uu_tf_query = "(select 'R_UU_T_3',circle, count(distinct msisdn),'MTSIBD' as service_name,date(call_date),status,'Non Active' as 'user_status' 
from mis_db.tbl_reg_calllog where date(call_date)='$view_date1' and dnis like '55789%' and chrg_rate=3 and status in(-1,11,0) 
AND MSISDN  NOT IN ( select DISTINCT MSISDN from mis_db.tbl_reg_calllog where date(call_date)='$view_date1' and dnis like '55789%' 
and chrg_rate=1 and status IN (1)) group by circle)";
$uu_tf_query .= "UNION (select 'R_UU_T_3',circle, count(distinct msisdn),'MTSIBD' as service_name,date(call_date),status,'Active' as 'user_status' 
from mis_db.tbl_reg_calllog where date(call_date)='$view_date1' and dnis like '55789%' and chrg_rate=3 and status=1 group by circle)";
$uu_tf_result = mysql_query($uu_tf_query, $dbConn) or die(mysql_error());
$numRows4 = mysql_num_rows($uu_tf_result);
if ($numRows4 > 0) {
    while ($uu_tf = mysql_fetch_array($uu_tf_result)) {
...
$sec_tf = array();
$sec_tf_query = "select 'R_SEC_T_3',circle, count(msisdn),'MTSIBD' as service_name,date(call_date),sum(duration_in_sec) 
...
$sec_tf = array();
$sec_tf_query = "select 'R_SEC_T_3',circle, count(msisdn),'MTSIBD' as service_name,date(call_date),sum(duration_in_sec),status from 
mis_db.tbl_reg_calllog nolock where date(call_date)='$view_date1' and dnis like '55789%' and chrg_rate=3 group by circle";
...
if ($numRows5 > 0) {
    while ($sec_tf = mysql_fetch_array($sec_tf_result)) {
        ...
        if ($sec_tf[6] == 1)
            $sec_tf[0] = 'R_L_SEC_T_3';
        elseif ($sec_tf[6] != 1)
            $sec_tf[0] = 'R_N_SEC_T_3';
...
$uu_tf_query = "select 'UU_TF',circle, count(distinct msisdn),'mtssu' as service_name,date(call_date) from mis_db.tbl_cricket_calllog 
where date(call_date)='$view_date1' and dnis like '52444%' and dnis not like '5244401%' group by circle";
...
if ($numRows4 > 0) {
    while ($uu_tf = mysql_fetch_array($uu_tf_result)) {
        ...
        $insert_uu_tf_data = "insert into mis_db.mtsDailyReport(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,total_sec) values('$view_date1', '$uu_tf[0]','$circle','0','$uu_tf[2]','','1108','NA','NA','NA')";
        $queryIns_uu = mysql_query($insert_uu_tf_data, $dbConn);
    }
}

$uu_tf = array();
$uu_tf_query = "(select 'UU_TF',circle, count(distinct msisdn),'mtssu' as service_name,date(call_date),status,'Non Active' as 'user_status' from mis_db.tbl_cricket_calllog where date(call_date)='$view_date1' and dnis like '52444%' and dnis not like '5244401%' and status in(-1,11,0) AND MSISDN  NOT IN( select DISTINCT MSISDN from mis_db.tbl_cricket_calllog where date(call_date)='$view_date1' and dnis like '52444%' and dnis not like '5244401%'  and status IN (1)) group by circle)";
$uu_tf_query .= "UNION (select 'UU_TF',circle, count(distinct msisdn),'mtssu' as service_name,date(call_date),status,'Active' as 'user_status' from mis_db.tbl_cricket_calllog where date(call_date)='$view_date1' and dnis like '52444%' and dnis not like '5244401%'  and status=1 group by circle)";
...
$uu_tf_query = "select 'UU_T_1',circle, count(distinct msisdn),'mtssu' as service_name,date(call_date) from mis_db.tbl_cricket_calllog 
where date(call_date)='$view_date1' and dnis like '5244401%' group by circle";
...
$uu_tf = array();
$uu_tf_query = "(select 'UU_T_1',circle, count(distinct msisdn),'mtssu' as service_name,date(call_date),status,'Non Active' as 'user_status' from mis_db.tbl_cricket_calllog where date(call_date)='$view_date1' and dnis like '5244401%' and status in(-1,11,0) AND MSISDN  NOT IN( select DISTINCT MSISDN from mis_db.tbl_cricket_calllog where date(call_date)='$view_date1' and dnis like '5244401%'  and status IN (1)) group by circle)";
$uu_tf_query .= "UNION (select 'UU_T_1',circle, count(distinct msisdn),'mtssu' as service_name,date(call_date),status,'Active' as 'user_status' from mis_db.tbl_cricket_calllog where date(call_date)='$view_date1' and dnis like '5244401%'  and status=1 group by circle)";
...
$sec_tf = array();
$sec_tf_query = "select 'SEC_TF',circle, count(msisdn),'MTSSU' as service_name,date(call_date),sum(duration_in_sec) from mis_db.tbl_cricket_calllog 
where date(call_date)='$view_date1' and dnis like '52444%' and dnis not like '5244401%' group by circle";
...
if ($numRows5 > 0) {
    while ($sec_tf = mysql_fetch_array($sec_tf_result)) {
        ...
        $insert_sec_tf_data = "insert into mis_db.mtsDailyReport(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,total_sec)
        values('$view_date1', '$sec_tf[0]','$circle','0','$sec_tf[5]','','1108','NA','NA','NA')";
        $queryIns_sec = mysql_query($insert_sec_tf_data, $dbConn);
    }
}

$sec_tf = array();
$sec_tf_query = "select 'SEC_TF',circle, count(msisdn),'MTSSU' as service_name,date(call_date),sum(duration_in_sec),status from mis_db.tbl_cricket_calllog 
where date(call_date)='$view_date1' and dnis like '52444%' and dnis not like '5244401%' group by circle,status";
...
$sec_tf = array();
$sec_tf_query = "select 'SEC_T_1',circle, count(msisdn),'MTSSU' as service_name,date(call_date),sum(duration_in_sec) from mis_db.tbl_cricket_calllog 
where date(call_date)='$view_date1' and dnis like '5244401%' group by circle";
...
$sec_tf = array();
$sec_tf_query = "select 'SEC_T_1',circle, count(msisdn),'MTSSU' as service_name,date(call_date),sum(duration_in_sec),status from mis_db.tbl_cricket_calllog 
where date(call_date)='$view_date1' and dnis like '5244401%' group by circle,status";
