...
$Days=$_REQUEST['Days'];
$duration_date=$_REQUEST['duration_date'];
switch($upfor)
{
...
break;
case 'mou':
if ($service_info == '1001'){
$type=$duration;
if($Days == '25'){
    $Days = $Days.'/'.$duration_date;
}
$duration=$Days;
    }else{
 $type=$duration;
...
mysql_query($update_query,$dbConn);	
					$insertquery="insert into master_db.tbl_sms_engagement(message,type,status,added_on,circle,scheduledDate,duration,category,service_id) values('".mysql_real_escape_string($message)."', '$type', '1', now(),'$circle_id', '$schedule_date', '$duration', '$category','$service_info')";
...
mysql_query($update_query,$dbConn);
 $insertquery="insert into master_db.tbl_sms_engagement(message,type,status,added_on,circle,scheduledDate,duration,category,service_id) values('".mysql_real_escape_string($message)."', '$type', '1', now(),'$circle', '$schedule_date', '$duration', '$category','$service_info')";
