...
include("config/dbConnect.php");
	...
	$select_query="select file_name, added_on, service_type, channel, price_point, upload_for,batch_id,service_id,amount,language from billing_intermediate_db.bulk_upload_history where status=0 and upload_for='topup_charging' and service_id in(1101,1102,1103,1111,1106,1110,1116,1113,1124) limit 1";
	...
	//}
		for($i=0;$i<$file_size;$i++)
		{
			...
			echo $call = "call " . $subscriptionProcedure . "('$msisdn','$lang','$channel','$shortCode','$Newamount',$service_id,$price,'$batch_id')";
			$qry1 = mysql_query($call,$dbConn) or die(mysql_error());
          
					$log_file_path="/var/www/html/hungamacare/log/bulk_topup_log_".date('Y-m-d').".txt";
					$LogString=$msisdn."#".$channel."#".date('H:i:s')."#".$price."#".$call."\n";
					error_log($LogString,3,$log_file_path);
				}
			
		} // end of for 
		echo $update_bulk_history="update billing_intermediate_db.bulk_upload_history set status=2 where batch_id=$batch_id and service_id=".$service_id;
		$bulk_update_result = mysql_query($update_bulk_history, $dbConn) or die(mysql_error());
