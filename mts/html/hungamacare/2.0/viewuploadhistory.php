...
//	$serviceId = $_GET['sid'];
	...
	$uploadvaluearray = array('active'=>'Activation','deactive'=>'Deactivation','topup'=>'Top-Up','renewal'=>'Renewal','topup_charging'=>'Top-Up','event_active'=>'Event Activation','event_unsub'=>'Event','autochurn_renewal'=>'AutoChurnRenewal','tryandbuy'=>'Try & Buy SPL','ticket_chat'=>'Ticket');
	//$uploadedby='uni.bulk';
...
$uploadeby_mts=$_SESSION['loginId_mts'];
	...
	if($uploadfor=='tryandbuy')
	{
	if($uploadeby_mts=='vikrant.garg')
	$cond=" where status=1";
	else
	$cond=" where added_by='".$uploadeby_mts."' and status=1";
	
	$get_query="select id,status,auto_rew_deact,service_id,tnb_days,tnb_mins,pre_sms,submode,circle,reqs_time,added_by from MTS_IVR.tbl_new_tnb_whitelisted_config $cond order by id desc";
	}
	else if($uploadfor=='get_balance')
	{
	$get_query="select id, batch_id,status,file_name,added_on,added_by,scheduletime from billing_intermediate_db.getBal_upload_history  order by added_on desc limit 20";
	}
	else
	{
	$get_query="select id,status,batch_id,channel,file_name,added_on,upload_for,total_file_count,service_id,price_point,success_count,failure_count,InRequest,amount,scheduledOn from billing_intermediate_db.bulk_upload_history where added_by='".$uploadeby_mts."' and upload_for='".$uploadfor."' order by added_on desc limit 20";
	}
...
if($uploadfor=='tryandbuy')
	{
	//echo $get_query;
	include("trynbuyconfighistory.php");
	exit;
	}
        
if($uploadfor=='get_balance')
{
	//echo $get_query;
	include("getbalancehistory.php");
	exit;
}
?>
...
<TABLE class="table table-condensed table-bordered">
  ...
  <th align="left"><?php echo TH_BATCHID;?></th>
	...
	<th align="left"><?php echo 'Scheduled FOR';?></th>
	<th align="left"><?php echo 'Action';?></th>
	
</TR>
 </thead>
<?php
	while(list($id,$status,$batchId, $channel,$file_name, $datetime, $upload_for, $totalCount,$serviceId,$planId,$success_count,$failure_count,$request_count,$getAmount,$scheduleDate) = mysql_fetch_array($query)) {
	...
	<TD bgcolor="#FFFFFF"><?php echo $Service_DESC[$servicename]['Name'];?></TD>
		...
		<?php if(isset($status) && $status==0)
			...
			if($upload_for=='deactive')
			{
				...
				$fileStatus='<span class="label label-info">Scheduled</span>';
...
<?php //echo $request_count?$request_count:"NA"; ?>
		...
		<TD><?php 
		if($status==11){
		echo date('j-M \'y g:i a',strtotime($scheduleDate));
		}
		else
		{
		echo 'NA';
