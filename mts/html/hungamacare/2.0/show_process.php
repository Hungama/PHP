...
if ($_REQUEST['service']) {
   ...
   $query = "select t.id,s.servicename,s.service_id,t.start_time,t.end_time,t.status,s.circle from master_db.tbl_doubleconsent s INNER JOIN master_db.tbl_doubleconsent_time t ON s.id=t.sId WHERE t.status!=5 and s.service_id='" . $serviceId . "'";
    if ($_REQUEST['circle']) {
