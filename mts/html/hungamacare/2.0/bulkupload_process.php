...
/************************ADDED for MTS GET BALANCE ACTIVITY ***********************/
if($_POST['upfor'] == 'get_balance')
{
 $qryBatch = mysql_query("select max(batch_id) from billing_intermediate_db.getBal_upload_history", $dbConn);
 ...
 $uploaddir = "/var/www/html/hungamacare/bulkuploads/getbalance/";
 ...
 if (move_uploaded_file($_FILES['upfile']['tmp_name'], $path)) {
        $thisTime = date("Y-m-d H:i:s");
		 ...
		 if($_POST['dpd3'])
		 $scheduletime=$_POST['dpd3'];
		 else
		 $scheduletime = date("Y-m-d H:i:s");
		 
        $Uploadquery = "insert into billing_intermediate_db.getBal_upload_history(batch_id,file_name,added_by,added_on,status,scheduletime) values('".$batchId."','" . $makFileName . "', '" . $uploadeby_mts . "', now(), 0,'" .$scheduletime. "')";
...
// added for tryandbuy interface option
    ...
    if ($batchId)
		...
		//check for correct plan_id/service id combination -
		if($_POST['upfor'] != 'deactive')
		{
$checkPid = mysql_query("select S_id from master_db.tbl_plan_bank nolock where S_id='".$serviceId."' and plan_id='".$planId."'", $dbConn);
		 list($S_id) = mysql_fetch_array($checkPid);
		 if($S_id)
		 {		 
		 }
		 else
		 {
		 echo "<div width=\"85%\" align=\"left\" class=\"txt\">
			<div class=\"alert alert-danger\">Wrong price point was selected. Please refresh the page and try again.</div></div>";
		 exit;		 
		 }
		 }
