...
function checkfield(type) { alert(type);
	$('#loading').hide();
	...
	if (service_info==0) {
		...
		return false;
	} else if (type=="form-event_unsub") { alert('here');
            ...
            if(add_scheduler == 1){
                var dpd2 =document.forms[type]["dpd2"].value;alert(add_scheduler);
                if(dpd2 == 'NULL' || dpd2==''){
...
document.getElementById('alert_placeholder').style.display='none';
 $('#loading').show();
    ...
    $.ajax({
        url: 'bulkupload_process_jyoti.php',
        type: 'POST',
        data: formData,
        async: false,
        success: function (data) {
