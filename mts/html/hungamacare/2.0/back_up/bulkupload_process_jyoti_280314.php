...
<div class=\"alert alert-danger\">Please upload file of less than " . number_format($bulklimit) . " numbers otherwise it will not process.</div></div>";
    ...
    $channel = $_POST['channel'];
    $upfor = $_POST['upfor'];
    if ($upfor == 'event_unsub') {
        $scheduler_date = $_POST['dpd2'];
        $status= 11;
    } else {
        $scheduler_date = '';
        $status= 0;
...
// added for tryandbuy interface option
    ...
    if (move_uploaded_file($_FILES['upfile']['tmp_name'], $path)) {
        ...
        $dbaccess = 'mts';
        $Uploadquery = "insert into billing_intermediate_db.bulk_upload_history(batch_id,                service_type,     channel,              price_point,       file_name,            added_by,                 added_on,         upload_for, status,operator,total_file_count,service_id,amount,language,act_date,renew_date,smsFlag,scheduler_date) 
                                                                         values('" . $batchId . "','" . $subtype . "', '" . $channel . "', '" . $planId . "', '" . $makFileName . "', '" . $uploadeby_mts . "', '" . $thisTime . "', '" . $upfor . "',$status,'" . $dbaccess . "','" . $file_size . "','" . $serviceId . "','" . $getAmount . "','" . $lang . "','" . $act_date . "','" . $renew_date . "','" . $tnb_prerenewsms . "','" . $scheduler_date . "')";
        $queryIns = mysql_query($Uploadquery, $dbConn);
