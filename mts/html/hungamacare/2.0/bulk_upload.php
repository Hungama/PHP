...
$currentTimeQuery = "SELECT DATE_FORMAT(now(), '%Y-%m-%d %H:%i:%s') as ctime";
$currentTime = mysql_query($currentTimeQuery,$dbConn);
while ($TimeData = mysql_fetch_array($currentTime)) {
    $ctime = $TimeData['ctime'];
}
...
/********************************Try & buy section********************************************/
$tryandbuyarray=array();
foreach ($serviceTryandBuy as $k => $v) { 
	if(in_array($k,$services)) {
		if($v!='') {
			$checkforEvent="SELECT id,type,service_id,value FROM master_db.tbl_interface_type_master WHERE Isenable=1 and value='tryandbuy' and service_id like '%$v%'";
			$total_event = mysql_query($checkforEvent,$dbConn);
			$row1_event= mysql_fetch_array($total_event);
			if($row1_event['value'] == 'tryandbuy') {
				$tryandbuyarray[$v] = $Service_DESC[$k]['Name'];
			}
		}
	}
}
...
/********************************ticket_chat section********************************************/
$ticket_chatarray=array(); 
foreach ($serviceticket_chat as $k => $v) { 
	if(in_array($k,$services)) {
		if($v!='') {
			$checkforEvent="SELECT id,type,service_id,value FROM master_db.tbl_interface_type_master WHERE Isenable=1 and value='ticket_chat' and service_id like '%$v%'";
			$total_event = mysql_query($checkforEvent,$dbConn);
			$row1_event= mysql_fetch_array($total_event);
			if($row1_event['value'] == 'ticket_chat') {
				$ticket_chatarray[$v] = $Service_DESC[$k]['Name'];
			}
		}
	}
}
...
asort($tryandbuyarray);
asort($ticket_chatarray);
...
function checkfield(type) {
	...
	if(type!="form-deactive") {
		var pricepoint=document.forms[type]["price"].value;
if(pricepoint=="" && (uploadfor=='active' || uploadfor=='renewal' || uploadfor=='topup' || uploadfor=='topup_charging' || uploadfor=='event_active' || uploadfor=='event_unsub' || uploadfor=='autochurn_renewal')) {
			bootstrap_alert.warning('<?php echo JS_NOPPSELECTED;?>');
			return false; 
		}
	}
	if (type=="form-event_unsub") { 
            //var add_scheduler=document.forms[type]["add_scheduler"].value;alert(add_scheduler);
            var add_scheduler = $('input:radio[name=add_scheduler]:checked').val(); 
            if(add_scheduler == 1){
                var dpd2 =document.forms[type]["dpd2"].value;
                if(dpd2 == 'NULL' || dpd2==''){
		bootstrap_alert.warning('Please select Scheduler Date');
                return false;
                }		
                }
	}
...
function checkfield_tnbspl(type) {
	$('#loading').hide();
	document.getElementById('alert_placeholder').style.display='inline';
	var price=document.forms[type]["price"].value;
...
function showRowsTable(type,sid) { //alert(type);alert(sid);
	var formname='form-'+type;
	//alert(type);
...
//tonePlanData toneMode renewal  event
	...
	if(type == 'deactive') {
		...
		if(type=='topup_charging' || type=='event_active' || type=='event_unsub' || type=='auto_churn' || type=='auto_renewal' || type=='tryandbuy')
		{
...
function deleteBulkFile(batchid,type)
{
    var answer = confirm("Are You Sure To Want Delete File");
        if(answer) {
        }else{
            return false;
        } 
...
xmlhttp.open("GET","deletebulkfile.php?batchid="+batchid+"&bulktype="+type,true);
xmlhttp.send();
}
function submitEditDeatil(batchid,type)
{
var edit_tnb_days = document.getElementById('edit_tnb_days').value; 
var edit_tnb_mins = document.getElementById('edit_tnb_mins').value;
var edit_pre_sms = document.getElementById('edit_pre_sms').value;
...
xmlhttp.open("GET","submitedifile.php?id="+batchid+"&bulktype="+type+"&tnb_days="+edit_tnb_days+"&tnb_mins="+edit_tnb_mins+"&pre_sms="+edit_pre_sms,true);
xmlhttp.send();
}
function editBulkFile(batchid,type) { 
                var xmlhttp;
                if (window.XMLHttpRequest) { // code for IE7+, Firefox, Chrome, Opera, Safari
                    xmlhttp=new XMLHttpRequest();
                } else { // code for IE6, IE5
                ...
                var url="editbulkfile.php?id="+batchid;
                xmlhttp.open("GET",url,true);
                xmlhttp.send();	
            }
...
<body onload="javascript:viewUploadhistory('deactive')">
...
<div class="container">
	<div class="row">
		...
		<div class="tabbable">
			<ul class="nav nav-pills">
				...
				<li class="active"><a href="#deactive" onclick="viewUploadhistory('deactive')" data-toggle="tab" data-act="deactivation">Deactivation</a></li>
				<?php
				if($_SESSION['loginId']=='vikrant.garg')
				{?>
				...
				<li class=""><a href="#autochurn_renewal" onclick="viewUploadhistory('autochurn_renewal')" data-toggle="tab" data-act="autochurn_renewal">Auto Churn-Renewal</a></li>
                                <li class=""><a href="#get_balance" onclick="viewUploadhistory('get_balance')" data-toggle="tab" data-act="get_balance">Get Balance</a></li>
				<!--li class=""><a href="#tryandbuy" onclick="viewUploadhistory('tryandbuy')" data-toggle="tab" data-act="tryandbuy">Try & Buy Spl</a></li-->
                 <!--li class=""><a href="#ticket_chat" onclick="viewUploadhistory('ticket_chat')" data-toggle="tab" data-act="ticket_chat">Ticket</a></li-->
				 <!--li class=""><a href="#act_tnb" onclick="viewUploadhistory('act_tnb')" data-toggle="tab" data-act="ticket_chat">TNB- Activation</a></li-->
	...
	<div id="deactive" class="tab-pane active">
	...
	<table class="table table-bordered table-condensed">
	  ...
	  <tr>
				...
				<span class="add-on"><i class="icon-calendar"></i></span>
                                <input type="text" name="schedule_date" id="schedule_date" value="<?php echo date("m/d/Y");?> - <?php echo date("m/d/Y");?>" />
                     </div>
                    </div>
                  </div>
                 </fieldset>
					...
					<tr>
						...
						<td><div id="pricepoint-event_unsub">
								<select name="price" id='price' class="in"></select>
                                            ...
                                            <tr>
                                                <td width="16%" height="32" align="left">Add Scheduler</td>
                                                <td>
                                                        <input type="radio" id="add_scheduler_yes" name="add_scheduler" value="1" checked="true" onchange="setScheduler(1)"/>&nbsp; Yes&nbsp;&nbsp;&nbsp;
                                                        <input type="radio" id="add_scheduler_no" name="add_scheduler" value="0" onchange="setScheduler(0)">&nbsp; No &nbsp;&nbsp;&nbsp;
                                                    </td>
                                            </tr>
<!--                                            <tr id="tr_scheduler_date">
                                                <td width="16%" height="32" align="left">Schedule Date</td>
                                                <td>
                                                    <input type="text" value="" name="scheduler_date" id="scheduler_date" placeholder="Click to set date">(mm/dd/yyyy)
                                                </td>
                                            </tr>-->
                                            <tr id="tr_scheduler_date">
                                            ...
                                            <td>
                                               <div id="dpd2" class="input-append date">
                                                ...
                                                <?php echo "Current Server Time-  <b>" . $ctime."</b>"; ?>
			...
			<div id="grid-autochurn_renewal"></div>
						...
						<!---for Try & buy interface-->
				<div id="tryandbuy" class="tab-pane">
				<form id="form-tryandbuy" name="form-tryandbuy" method="post" enctype="multipart/form-data">
				
					<table class="table table-bordered table-condensed">
					<tr>
						<td align="left" width="16%" height="32"><span>Service&nbsp;</span></td>
						<td><select name="service_info" id="service_info" onchange="SetPricePoint('tryandbuy',this.value);">
								<option value="0">Select Service</option>
								<?php foreach($tryandbuyarray as $s_id=>$s_val) { ?>
									<option value="<?php echo $s_id;?>"><?php echo $s_val;?></option>
								<?php } ?>
							</select>
						</td>
					...
					<tr>
						<td align="left" width="16%" height="32">Price Point</td>
						<td><div id="pricepoint-tryandbuy">
								<select name="price" id='price' class="in"></select>
							</div>
						</td>
					</tr>
					<tr>
						<td width="16%" height="32" align="left">Type</td>
						<td><div id="auto_types">
					 <input type="radio" name="auto_type" value="deact"  />&nbsp; Auto deactivation&nbsp;&nbsp;&nbsp;
                     <input type="radio" name="auto_type" value="renew" checked="true">&nbsp; Auto renewal &nbsp;&nbsp;&nbsp;
					</div> </td>
				</tr>
				<tr>
						<td width="16%" height="32" align="left">Days</td>
						<td>
						<input type="radio" name="tnb_days" value="1" checked="true" />&nbsp; 1 days&nbsp;
						<input type="radio" name="tnb_days" value="2"  />&nbsp; 2 days&nbsp;
					 <input type="radio" name="tnb_days" value="3" checked="true" />&nbsp; 3 days&nbsp;
					 <input type="radio" name="tnb_days" value="4" />&nbsp; 4 days&nbsp;
                     <input type="radio" name="tnb_days" value="5">&nbsp;5 days &nbsp;
					 <input type="radio" name="tnb_days" value="6" />&nbsp; 6 days&nbsp;
					 <input type="radio" name="tnb_days" value="7">&nbsp;7 days &nbsp;
					</td>
				</tr>
				<tr>
						<td width="16%" height="32" align="left">Minutes</td>
						<td>
						<input type="radio" name="tnb_mins" value="0" checked="true" />&nbsp; NA&nbsp;
					 <input type="radio" name="tnb_mins" value="10" />&nbsp; 10 mins&nbsp;
                     <input type="radio" name="tnb_mins" value="15">&nbsp;15 mins &nbsp;
					 <input type="radio" name="tnb_mins" value="20">&nbsp;20 mins &nbsp;
					</td>
				</tr>
				<tr>
						<td width="16%" height="32" align="left">Pre Renewal SMS</td>
						<td>
					 <input type="radio" name="tnb_prerenewsms" value="yes"  />&nbsp; Yes&nbsp;
                     <input type="radio" name="tnb_prerenewsms" value="no" checked="true">&nbsp;No &nbsp;
					 </td>
				</tr>
					...
					<tr>
						...
						<select name="circle_info" id="circle_info">
								<option value="0">Select Circle</option>
								<?php foreach($circle_info as $cid=>$cvalue) { ?>
									<option value="<?php echo $cid;?>"><?php echo $cvalue;?></option>
								<?php } ?>
							</select>
											
							<INPUT type="hidden" name="action" id='action' value="tryandbuy" class="in">
							<input id="upfor" type="hidden" value="tryandbuy" name="upfor">
		                  	<button class="btn btn-primary" style="float:right">Submit</button>
							<input type='hidden' name='usrId' value=<?php echo $user_id;?>>
						</td>
					</tr>
			...
			<div id="grid-tryandbuy"></div>
		</div>
                                                
             <div id="ticket_chat" class="tab-pane">
				<form id="form-ticket_chat" name="form-ticket_chat" method="post" enctype="multipart/form-data">
					<table class="table table-bordered table-condensed">
					<tr>
						<td align="left" width="16%" height="32"><span>Service&nbsp;</span></td>
						<td>
                                                    <select name="service_info" id="service_info" onchange="showRowsTable('ticket_chat',this.value);">
								<option value="0">Select Service</option>
<!--                                                                <option value="1102">MTS - 54646</option>-->
								<?php foreach($ticket_chatarray as $s_id=>$s_val) { ?>
									<option value="<?php echo $s_id;?>"><?php echo $s_val;?></option>
								<?php } ?>
						</td>
					</tr>
					<tr>
						<td width="16%" height="32" align="left">Mode</td>
						<td><div id="channel-ticket_chat"><select name="channel" id="channel" class="in"></SELECT></div></td>
					</tr>
					<tr>
						<td align="left" width="16%" height="32">Price Point</td>
						<td><div id="pricepoint-ticket_chat">
								<select name="price" id='price' class="in">
<!--                                                                    <option value="91">Rs. 10</option>-->
                                                                </select>
							</div>
						</td>
					</tr>
					<tr>
						<td align="left" width="16%" height="32"><span>Browse File To Upload </span></td>
						<td><INPUT name="upfile" id='upfile' type="file" class="in">
							<INPUT type="hidden" name="action" id='action' value="ticket_chat" class="in">
							<input id="upfor" type="hidden" value="ticket_chat" name="upfor">
		                  	<button class="btn btn-primary" style="float:right">Submit</button>
							<input type='hidden' name='usrId' value=<?php echo $user_id;?>>
						</td>
					</tr>
			...
			<div id="grid-ticket_chat"></div>
		</div>	
		
		
		
		<!---TNB bulk upload start here-->
			<div id="act_tnb" class="tab-pane">
				<form id="form-act_tnb" name="form-act_tnb" method="post" enctype="multipart/form-data">
				
					<table class="table table-bordered table-condensed">
					<tr>
						<td align="left" width="16%" height="32"><span>Service&nbsp;</span></td>
						<td><select name="service_info" id="service_info" onchange="showRowsTable('act_tnb',this.value);">
								<option value="0">Select Service</option>
								<?php foreach($tryandbuyarray as $s_id=>$s_val) { ?>
									<option value="<?php echo $s_id;?>"><?php echo $s_val;?></option>
								<?php } ?>
					...
					<tr>
						<td width="16%" height="32" align="left">Mode</td>
						<td><div id="channel-act_tnb"><select name="channel" id="channel" class="in"></SELECT></div></td>
					</tr>
					<tr>
						<td align="left" width="16%" height="32">Price Point</td>
						<td><div id="pricepoint-act_tnb">
								<select name="price" id='price' class="in"></select>
							</div>
						</td>
					</tr>
						...
						<td align="left" width="16%" height="32"><span>Browse File To Upload </span></td>
						<td><INPUT name="upfile" id='upfile' type="file" class="in">
							<INPUT type="hidden" name="action" id='action' value="act_tnb" class="in">
							<input id="upfor" type="hidden" value="act_tnb" name="upfor">
		                  	<button class="btn btn-primary" style="float:right">Submit</button>
							<input type='hidden' name='usrId' value=<?php echo $user_id;?>>
						</td>
					</tr>
			...
			<div id="grid-act_tnb"></div>
		</div>
		<!---TNB bulk upload end here-->
...
<!--Get Balance section start here added by rahul tripathi-->	
<div id="get_balance" class="tab-pane">
	<form id="form-get_balance" name="form-get_balance" method="post">
	<input type="hidden" value="<?php echo $bulklimit;?>" name="bulklimit"/>
	<table class="table table-bordered table-condensed">
<tr>
			<td align="left" width="16%" height="32"><span>Browse File To Upload </span></td>
			<td><INPUT name="upfile" id='upfile' type="file" class="in">
				<INPUT type="hidden" name="action" id='action' value="get_balance" class="in">
				<input id="upfor" type="hidden" value="get_balance" name="upfor">
                <button class="btn btn-primary" style="float:right">Submit</button>
				<input type='hidden' name='usrId' value=<?php echo $user_id;?>>
			</td>
		</tr>
                   <tr id="tr_scheduler_date">
                                            <td align="left" width="16%" height="32"><span>Schedule Date&nbsp;</span></td>
                                            <td>
                                               <div id="dpd3" class="input-append date">
                                                    <input data-format="yyyy-MM-dd hh:mm:00" type="text" name="dpd3" value="" placeholder="Click on icon to set date"></input>
                                                    <span class="add-on">
                                                        <i data-time-icon="icon-time" data-date-icon="icon-calendar">
                                                        </i>
                                                    </span>
	...
	<div id="grid-get_balance"></div>									
    </div>  
<!--Get Balance section End here--> 
...
$("form#form-tryandbuy").submit(function(){
var isok = checkfield_tnbspl('form-tryandbuy');
//var isok=true;
if(isok)
{
document.getElementById('alert_placeholder').style.display='none';
 $('#loading').show();
	var formData = new FormData($("form#form-tryandbuy")[0]);
    $.ajax({
   	...
   	document.getElementById('grid-tryandbuy').style.display='inline';
	document.getElementById('grid-tryandbuy').innerHTML=data;
	//resestForm('tryandbuy');
	viewUploadhistory('tryandbuy');
	        },
        cache: false,
        contentType: false,
        processData: false
...
$("form#form-act_tnb").submit(function(){
//var isok = checkfield('form-act_tnb');
var isok=true;
if(isok)
{
document.getElementById('alert_placeholder').style.display='none';
 $('#loading').show();
	var formData = new FormData($("form#form-act_tnb")[0]);
    $.ajax({
        url: 'bulkupload_process.php',
        type: 'POST',
        data: formData,
        async: false,
        success: function (data) {
   	document.getElementById('grid-act_tnb').style.display='inline';
	document.getElementById('grid-act_tnb').innerHTML=data;
	resestForm('act_tnb');
	viewUploadhistory('act_tnb');
	        },
        cache: false,
        contentType: false,
        processData: false
...
$("form#form-ticket_chat").submit(function(){ 
var isok = checkfield('form-ticket_chat');
//var isok=true;
if(isok)
{
document.getElementById('alert_placeholder').style.display='none';
 $('#loading').show();
	var formData = new FormData($("form#form-ticket_chat")[0]);
    $.ajax({
        url: 'bulkupload_process.php',
        type: 'POST',
        data: formData,
        async: false,
        success: function (data) {
   	document.getElementById('grid-ticket_chat').style.display='inline';
	document.getElementById('grid-ticket_chat').innerHTML=data;
	resestForm('ticket_chat');
	viewUploadhistory('ticket_chat');
	        },
        cache: false,
        contentType: false,
        processData: false
...
//add new code for get balance
$("form#form-get_balance").submit(function(){
//var isok = checkfield('form-get_balance');
var isok=true;
 if(isok)
{
document.getElementById('alert_placeholder').style.display='none';
 $('#loading').show();
    var formData = new FormData($("form#form-get_balance")[0]);
    ...
    $.ajax({
        ...
        success: function (data) {
        document.getElementById('loading').style.display='none';
	document.getElementById('grid-get_balance').style.display='inline';
		 document.getElementById('grid-get_balance').innerHTML=data;
		  resestForm('get_balance');
			viewUploadhistory('get_balance');
			//$('#loading').hide();
        },
        cache: false,
        contentType: false,
...
//end of the code for get balance
...
if(value == 1){
document.getElementById('tr_scheduler_date').style.display = 'table-row';		
}else{
document.getElementById('tr_scheduler_date').style.display = 'none';
}    
}
