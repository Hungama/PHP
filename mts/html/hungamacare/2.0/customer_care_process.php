...
$_SESSION['usrId'] = $_REQUEST['usrId'];
   ...
   if ($_POST['Submit'] == "Submit")
	...
	<TR height="30">
		...
		<TD><B><?php echo 'CGID';?></B></TD>
                <TD><B><?php echo 'Combo Pack';?></B></TD>
...
<?php
	...
	//	$select_query1= "select SUB_DATE, circle, MODE_OF_SUB, DNIS, STATUS from ";
		$select_query1= "select SUB_DATE, RENEW_DATE, circle, MODE_OF_SUB, DNIS, STATUS, DATEDIFF(RENEW_DATE,NOW()) as diff,DCccg_ID from ";
        	switch($service_info)
			{
			case '1102':
				...
				case '11011':
					...
					$select_query1.= "mts_mu.tbl_HB_subscription";
				break;
				case '1103':
				...
				case '1125':
               	...
               	break;
				case '1108':
                     $select_query1.= "MTS_cricket.tbl_cricket_subscription";
               	break;
				case '1116':
...
//list($TicketStatus) = mysql_fetch_array($ticket_query);
 ...
 list($SUB_DATE,$RENEW_DATE,$circle1,$MODE_OF_SUB,$DNIS,$subStatus,$daysLeft,$DCccg_ID) = mysql_fetch_array($querySubscription);
	...
	?></TD>
<td><?php echo $DCccg_ID;?></td>
<td><?php 
if (in_array($plan_id, $planData)){
switch ($plan_id) {
    case '120':
    case '121':
    case '122':
...
<button class="btn btn-mini btn-danger" type="button" onclick="do_Act_Deactivate(<?php echo $msisdn;?>,<?php echo $service_info;?>,'<?php echo $subsrv;?>','da','customer_care_process','all')" >Deactivate (All)</button>		
	...
	{?>		
	 <button class="btn btn-mini btn-danger" type="button" onclick="do_Act_Deactivate(<?php echo $msisdn;?>,<?php echo $service_info;?>,'<?php echo $subsrv;?>','da','customer_care_process','<?php echo $iscombo; ?>')" >Deactivate</button>
	<?php 
	}
	}
	elseif($subStatus=='')
...
<TR height="30">
	...
	<TD><?php if(!empty($DNIS_TC)){echo $DNIS_TC;} else {echo '-';} ?></TD>
	<TD><?php 
	echo $servicename;
	?>
	</TD>
	<TD><?php if(!empty($SUB_DATE_TC)){echo date('j-M \'y g:i a',strtotime($SUB_DATE_TC));} else {echo '-';}?></TD>
	<TD><?php 
	...
	{
	if(!empty($RENEW_DATE_TC)){echo date('j-M \'y g:i a',strtotime($RENEW_DATE_TC));} else {echo '-';}
	}
	?></TD>
	<TD><?php if(!empty($date_time)){echo date('j-M \'y g:i a',strtotime($date_time));} else {echo '-';}?></TD>
	...
	<TD><?php if(!empty($MODE_OF_SUB_TC)){echo $MODE_OF_SUB_TC;} else {echo '-';}?></TD>	
	<TD><?php
	if($SC=='546460' || $SC=='54644646' || $service_info==1102)
	{
	echo $TicketStatus1;
...
//if section end here	
	...
	elseif ($_GET['msisdn'] != "" && $_GET['act'] == "da")
		...
		elseif($_GET['service_info']==1101 || $_GET['service_info']==11011)
			...
			$Query1 = "call mts_mu.HB_UNSUB('$_GET[msisdn]', 'CC')";
		...
		elseif($_GET['service_info']==1108)
			$Query1 = "call MTS_cricket.CRICKET_UNSUB('$_GET[msisdn]', 'CC')";

		$combPackDeactReq=$_GET['catid'];
		$iscomboPack=explode("|",$combPackDeactReq);
		if($iscomboPack[0] == 'yes')
		{
		$comboPlanid=$iscomboPack[1];
		//explode
		$comboPackArray=array(1101,1111,1123);
		if(in_array($_GET['service_info'],$comboPackArray))
			{
			$Query1 = "call MTS_IVR.COMBO_PACK_UNSUB('$_GET[msisdn]', 'CC','".$comboPlanid."')";
			}
		}
		//echo $Query1."<br>";
