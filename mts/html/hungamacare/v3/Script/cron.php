...
$deleted_file1 = "/var/www/html/hungamacare/v3/Script/logs/processlog_" . date("Ymd", mktime(0, 0, 0, date("m"), date("d") - 2, date("Y"))) . ".txt";
unlink($deleted_file1);
$deleted_file2 = "/var/www/html/hungamacare/v3/Script/querylog_" . date("Ymd", mktime(0, 0, 0, date("m"), date("d") - 2, date("Y"))) . ".txt";
unlink($deleted_file2);
//$nowtime = '04:49 PM';
$date = date("dmy", mktime(0, 0, 0, date('m'), date('d'), date('Y')));
$qry = "select rule_id,time_slot,action_name,sms_cli,sms_sequence from master_db.tbl_rule_engagement_temp_process where status=0 and action_name='sms' limit 1";
echo $queryData = $qry . "\r\n";
error_log($queryData, 3, $querylog);
/* $qry = "select rule_id,time_slot,action_name,sms_cli,sms_sequence from master_db.tbl_rule_engagement_action where status=1 and
  TIME_FORMAT(time_slot, '%H:%i')=DATE_FORMAT(now(),'%h:%i %p') and  action_name='sms'";
 */
