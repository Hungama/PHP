...
$get_process_query = "select id,count,added_on,rule_id,type from master_db.tbl_new_engagement_data nolock";
        ...
        <div class="col-md-12">
            ...
            <TABLE class="table table-condensed table-bordered">
                ...
                while ($processData = mysql_fetch_array($processQuery)) {
                    ...
                    <TR height = "30">
                        ...
                        <TD>
                            <?php 
                               $circleDisplay = explode(",", $rule_data_array['circle']);
                               $circle_str = '';
            ...
            $lastProcessDate = date('dmy', strtotime($added_on));
                    ?>
                        ...
                        <TD>
                           <?php if($action_data_array['action_name'] == 'sms'){ ?>
                            <a href="xls_new_engagement.php?service_id=<?php echo $rule_data_array['service_id'] ?>&type=<?php echo $processData['type'] ?>&added_on=<?php echo $lastProcessDate ?>&rule_id=<?php echo $rule_id ?>">
                              <?}else{
                                  $curdate = date("d-m-Y");?>
                                <a href="http://119.82.69.212/kmis/services/hungamacare/MTS_NEW_ENGMNT/dndcheck/<?php echo $rule_id.'_'.$curdate.'.csv'?>" target="_blank">
                                <?php }?>
                                Download
                        </TD>
                    </TR>
