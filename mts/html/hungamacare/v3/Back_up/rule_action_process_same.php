...
$selRuleDetailsQry = "select circle,service_id,service_base,filter_base,scenerios from master_db.tbl_rule_engagement where id='".$rule_id[0]."'";
$selRuleDetailsQryEx = mysql_query($selRuleDetailsQry, $dbConn);
$details = mysql_fetch_array($selRuleDetailsQryEx);

echo $select_scenerios_qry = "select rule.id from master_db.tbl_rule_engagement rule, master_db.tbl_rule_engagement_action action 
                         where rule.circle='" . $details['circle'] . "' and rule.service_id='" . $details['service_id'] . "' 
                             and rule.service_base='" . $details['service_base'] . "' 
                         and rule.filter_base='" . $details['filter_base'] . "' and rule.scenerios='" . $details['scenerios'] . "' and rule.added_by='" . $loginid . "' 
...
echo $old_rule_id = mysql_fetch_row($select_scenerios_qryEx);
if ($old_rule_id) {
