...
if ($action_name == 'sms') {
    ...
    $insertquery = "insert into master_db.tbl_rule_engagement_action(rule_id,action_name,time_slot,email_user,cc_user,status,added_on,added_by) 
     values('$rule_id[0]', '$action_name','$time_slot', '$email_user', '$add_cc',1,now(),'')";
}
//die('here');
...
/* $obd_form_mob_file=$_FILES['upfile']['name'];
  ...
  VALUES ('".$createfilename."','".$uploadedby."',now(),'".$mode."','0','','".$totalcount."','".$obd_form_service."','".$ipaddress."','".$obd_form_menuid."','".$obd_form_ussdstr."','".$testingno."','".$menu_serialid."','".$obd_form_schedule_date."','".$obd_form_schedule_start_date."','".$obd_form_schedule_end_date."')";
