...
$serviceArray = array('1101' => 'MTSMU','1102' => 'MTS54646');
...
if ($_REQUEST['id'] && $_REQUEST['act']) {
    ...
    if ($actionType == 'dact') {
        $dacttQuery = "UPDATE MTS_IVR.tbl_ad_campaign SET status=0 WHERE id=" . $actionId;
        mysql_query($dacttQuery, $dbConn);
    } elseif ($actionType == 'act') {
        $actQuery = "UPDATE MTS_IVR.tbl_ad_campaign SET status=1 WHERE id=" . $actionId;
        mysql_query($actQuery, $dbConn);
    }
}

$get_query = "select id,S_id,ad_name,sc,circle,starttime,endtime,status,added_on from ";
$get_query .=" MTS_IVR.tbl_ad_campaign nolock order by id desc limit 20";
