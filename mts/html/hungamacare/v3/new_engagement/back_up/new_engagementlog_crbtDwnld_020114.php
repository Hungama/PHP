...
$query_1days = "select distinct a.ani,a.circle,b.status from mts_radio.tbl_crbtrng_reqs_log a, mts_radio.tbl_radio_subscription b
...
if ($result_row_1days > 0) {
    ...
    while ($details_1days = mysql_fetch_row($result_1days)) {
        ...
        $insert_query_1days = "insert into master_db.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status) 
            values (" . $details_1days[0] . ",'" . $details_1days[1] . "',now(),1101,'23','" . $details_1days[2] . "')";
...
$query_3days = "select distinct a.ani,a.circle,b.status from mts_radio.tbl_crbtrng_reqs_log a,mts_radio.tbl_radio_subscription b  where  date(date_time) 
                between DATE(NOW()-INTERVAL 5 DAY) and DATE(NOW()-INTERVAL 1 DAY) and a.status= 1 and b.status in(1,11) and a.ani=b.ani
...
if ($result_row_3days > 0) {
    ...
    while ($details_3days = mysql_fetch_row($result_3days)) {
        $insert_query_3days = "insert into master_db.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status)
                       values (" . $details_3days[0] . ",'" . $details_3days[1] . "',now(),1101,'24','" . $details_3days[2] . "')";
