...
where not exists(select a.ani from mts_radio.tbl_crbtrng_reqs_log a where date(a.date_time) between DATE(NOW()-INTERVAL 3 DAY) 
and DATE(NOW()-INTERVAL 1 DAY) and a.ani=b.ani and a.status = 1 ) and
...
if ($result_row_3days > 0) {
    echo $delete_query = "delete from master_db.tbl_new_engagement_number where date(added_on) = date(now()) and type = '25' and service_id=1101";
...
where not exists(select a.ani from mts_radio.tbl_crbtrng_reqs_log a where date(a.date_time) between DATE(NOW()-INTERVAL 5 DAY) 
and DATE(NOW()-INTERVAL 1 DAY) and a.ani=b.ani and a.status = 1 ) and
...
if ($result_row_5days > 0) {
    echo $delete_query = "delete from master_db.tbl_new_engagement_number where date(added_on) = date(now()) and type = '26' and service_id=1101";
    mysql_query($delete_query, $dbConn);
...
where not exists(select a.ani from mts_radio.tbl_crbtrng_reqs_log a where date(a.date_time) between DATE(NOW()-INTERVAL 7 DAY) 
and DATE(NOW()-INTERVAL 1 DAY) and a.ani=b.ani and a.status = 1 ) and
...
if ($result_row_7days > 0) {
    echo $delete_query = "delete from master_db.tbl_new_engagement_number where date(added_on) = date(now()) and type = '27' and service_id=1101";
    mysql_query($delete_query, $dbConn);
...
where not exists(select a.ani from mts_radio.tbl_crbtrng_reqs_log a where date(a.date_time) between DATE(NOW()-INTERVAL 15 DAY) 
and DATE(NOW()-INTERVAL 1 DAY) and a.ani=b.ani and a.status = 1 ) and
...
if ($result_row_15days > 0) {
    echo $delete_query = "delete from master_db.tbl_new_engagement_number where date(added_on) = date(now()) and type = '28' and service_id=1101";
    mysql_query($delete_query, $dbConn);
