...
$actionquery = "select sms_cli from master_db.tbl_rule_engagement_action nolock where rule_id='" . $rule_id . "'";
...
if ($msg != '') {
    ...
    and Type='" . $type . "' and service_id='" . $service_id . "' and " . $status_info . " and circle='" . $circle[$i] . "'";
        ...
        while ($smsRecord = mysql_fetch_row($result)) {
            ...
            if ($response == 'ND') {
                ...
                $procedureCall = "call master_db.SENDSMS_BULK_DND(" . $smsRecord[0] . ",'$msg','" . $sms_cli . "','sms-promo','1')";
    ...
    if ($msisdn_count == 0) {
     ...
     values('0',now(), '$ruleData[service_id]', 9,'$ruleData[scenerios]','$rule_id')";
    ...
    $insertquery2 = "insert into master_db.tbl_new_engagement_data(count,added_on,service_id,status,type,rule_id)
     values('0',now(), '$ruleData[service_id]', 10,'$ruleData[scenerios]','$rule_id')";
