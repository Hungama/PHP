...
echo $query = "select distinct a.msisdn,b.circle,b.status from mts_radio.tbl_radio_subscription b ,mis_db.tbl_radio_calllog a
...
if ($result_row > 0) {
    ...
    while ($details = mysql_fetch_row($result)) {
        $insert_query = "insert into master_db.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status)
						   values (" . $details[0] . ",'" . $details[1] . "'," . "now(),1101,'1','" . $details[2] . "')";
...
echo $query_1030MOU = "select distinct a.msisdn,b.circle,b.status from mts_radio.tbl_radio_subscription b ,mis_db.tbl_radio_calllog a where a.dnis like '52222%' 
...
if ($result_row_1030MOU > 0) {
    ...
    while ($details_1030MOU = mysql_fetch_row($result_1030MOU)) {
        $insert_query_1030MOU = "insert into master_db.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status)
						   values (" . $details_1030MOU[0] . ",'" . $details_1030MOU[1] . "',now(),1101,'2','" . $details_1030MOU[2] . "')";
...
echo $query_G30MOU = "select distinct a.msisdn,b.circle,b.status from mts_radio.tbl_radio_subscription b ,mis_db.tbl_radio_calllog a 
...
if ($result_row_G30MOU > 0) {
    ...
    while ($details_G30MOU = mysql_fetch_row($result_G30MOU)) {
        $insert_query_G30MOU = "insert into master_db.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status) 
            values (" . $details_G30MOU[0] . ",'" . $details_G30MOU[1] . "',now(),1101,'3','" . $details_G30MOU[2] . "')";
...
echo $query_L5 = "select distinct a.msisdn,b.circle,b.status from mts_radio.tbl_radio_subscription b ,mis_db.tbl_radio_calllog a
            where a.dnis like '52222%' and a.operator in ('mtsm') and date(a.call_date) = DATE(NOW()-INTERVAL 1 DAY) and a.msisdn=b.ani
...
if ($result_row_L5 > 0) {
    ...
    while ($details_L5 = mysql_fetch_row($result_L5)) {
        $insert_query_L5 = "insert into master_db.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status)
						   values (" . $details_L5[0] . ",'" . $details_L5[1] . "',now(),1101,'4','" . $details_L5[2] . "')";
...
echo $query_G20 = "select distinct a.msisdn,b.circle,b.status from mts_radio.tbl_radio_subscription b ,mis_db.tbl_radio_calllog a
...
if ($result_row_G20 > 0) {
    ...
    while ($details_G20 = mysql_fetch_row($result_G20)) {
        $insert_query = "insert into master_db.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status)
						   values (" . $details_G20[0] . ",'" . $details_G20[1] . "',now(),1101,'5','" . $details_G20[2] . "')";
        mysql_query($insert_query, $dbConn);
    }
}
