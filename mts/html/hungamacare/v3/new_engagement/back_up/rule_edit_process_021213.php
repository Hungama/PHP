...
if ($action_name == 'sms') {
    $time_slot = $_REQUEST['sms_edit_time_slot'];
    $min = $_REQUEST['sms_edit_min'];
    $sec = $_REQUEST['sms_edit_sec'];
} else {
    $time_slot = $_REQUEST['email_edit_time_slot'];
    $min = $_REQUEST['email_edit_min'];
    $sec = $_REQUEST['email_edit_sec'];
}
//$time_slot = $_REQUEST['edit_time_slot'];
//$min = $_REQUEST['edit_min'];
//$sec = $_REQUEST['edit_sec'];

$upload_file = $_REQUEST['upload_edit_file'];
//$dnd_scrubbing = $_REQUEST['dnd_scrubbing'];
$loginid = $_SESSION['loginId'];
echo $update_query = "update master_db.tbl_rule_engagement set  circle='" . $circle . "', service_base='" . $service_base . "', filter_base='" . $filter_base . "' , scenerios='" . $scenarios . "', dnd_scrubbing='" . $edit_dnd_scrubbing . "' where id='" . $rule_id . "'";
...
//    $ipaddress = $_SERVER['REMOTE_ADDR'];
  ...
  <div class=\"alert alert-danger\">Please upload file of less than 25,000 numbers otherwise it will not process.</div></div>";
        ...
        echo $insertquery = "update master_db.tbl_rule_engagement_action set  sms_sequence='" . $sms_sequence . "' ,
            sms_cli='" . $sms_cli . "' , upload_file='" . $createfilename . "' , time_slot='" . $time_slot . "', min='" . $min . "'
                , sec='" . $sec . "' where rule_id=" . $rule_id . "";
    } else {
        echo $insertquery = "update master_db.tbl_rule_engagement_action set  sms_sequence='" . $sms_sequence . "' ,
            sms_cli='" . $sms_cli . "' , time_slot='" . $time_slot . "' , min='" . $min . "'
                , sec='" . $sec . "' where rule_id=" . $rule_id . "";
    }
} else {
    echo $insertquery = "update master_db.tbl_rule_engagement_action set email_user='" . $email_user . "' ,
            cc_user='" . $cc_user . "' , time_slot='" . $time_slot . "'  , min='" . $min . "'
                , sec='" . $sec . "' where rule_id=" . $rule_id . "";
}
