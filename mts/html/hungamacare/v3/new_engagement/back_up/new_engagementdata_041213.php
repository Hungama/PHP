...
if ($result_row > 0) {
    $delete_query = "delete from master_db.tbl_new_engagement_data where date(added_on) = date(now())";
    mysql_query($delete_query, $dbConn);
    while ($details = mysql_fetch_row($result)) {
        $insert_query = "insert into master_db.tbl_new_engagement_data (count,added_on,service_id,status,type,duration) values (" . $details[0] . ",now()," . $details[1] . ",0,'" . $details[2] . "','" . $details[4] . "')";
