...
//    $ipaddress = $_SERVER['REMOTE_ADDR'];
  ...
  <div class=\"alert alert-danger\">Please upload file of less than 25,000 numbers otherwise it will not process.</div></div>";
        ...
        $insertquery = "insert into master_db.tbl_rule_engagement_action(rule_id,action_name,sms_sequence,sms_cli,time_slot,upload_file,status,added_on,added_by,min,sec) 
     values('$rule_id[0]', '$action_name','$sms_sequence', '$sms_cli', '$time_slot', '$createfilename',1,now(),'$loginid','$min','$sec')";
...
//     values('$rule_id[0]', '$action_name','$sms_sequence', '$sms_cli', '$time_slot', '$upload_file',1,now(),'')";
    ...
    $insertquery = "insert into master_db.tbl_rule_engagement_action(rule_id,action_name,time_slot,email_user,cc_user,status,added_on,added_by,min,sec) 
     values('$rule_id[0]', '$action_name','$time_slot', '$email_user', '$add_cc',1,now(),'$loginid','$min','$sec')";
}
