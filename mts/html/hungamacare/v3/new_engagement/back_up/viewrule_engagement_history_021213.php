    ...
    <TABLE class="table table-condensed table-bordered">
        ...
        echo date('j-M h:i A', strtotime($added_on));
            ?>
                ...
                <TD><?php
//            $get_query = "select time_slot from  master_db.tbl_rule_engagement_action nolock where rule_id ='" . $summarydata['id'] . "'";
//            $query = mysql_query($get_query, $dbConn);
//            $time_slot_detail = mysql_fetch_array($query);
//            $time_slot_detail['time_slot'];
//            //$myTime = '19:30';
////            if (date('H:i:s') >= date('H:i:s', strtotime($myTime))) {
//            if (date('H') >= date('H', strtotime($time_slot_detail['time_slot']))) {
//                $next_date = date('y-m-d') + 1;
//            } else {
//                $next_date = date('y-m-d');
//            }
//            $next_date = $next_date . " " . $time_slot_detail['time_slot'];
//            //echo "<br/>";
//            $added_on = $summarydata['added_on'];
//            echo date('j-M h:i A', strtotime($next_date));
                    ?></TD>
                <TD>
