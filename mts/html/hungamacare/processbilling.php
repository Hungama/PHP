...
$select_query = "select file_name, added_on, service_type, channel, price_point, upload_for,batch_id,service_id,amount,language,act_date,renew_date from billing_intermediate_db.bulk_upload_history nolock where status=0 and upload_for not in('event_active','event_unsub','tryandbuy','ticket_chat','act_tnb') and service_id in(1101,1102,1103,1111,1106,1110,1116,1113,1124,1125,1123,1126) limit 1";
    ...
    for ($i = 0; $i < $file_size; $i++) {
        ...
        switch ($service_id) {
            ...
            case '1125': $subTable = "mts_JOKEPORTAL.tbl_jokeportal_subscription";
                ...
                break;
			case '1108': $subTable = "MTS_cricket.tbl_cricket_subscription";
						$tableName = 'MTS_cricket.tbl_jbox_blacklist';
                break;
        }
        ...
        if (strlen(trim($msisdn)) == 10 && $BList <= 0) {
            if ($reqtype == 2) {
                ...
                $getCircle = "select circle from master_db.tbl_valid_series nolock where series=substring($msisdn,1,4) and length(series)=4";
				...
				$sub_check = "select ANI from " . $subTable . " nolock where ANI='$msisdn' and MODE_OF_SUB!='TRCV'";
				$qry_check = mysql_query($sub_check,$dbConn);
			//	$rows_check = mysql_fetch_row($qry_check);
				$rows_check = mysql_num_rows($qry_check);
//echo $sub_check."<br>";
        if ($rows_check <= 0) {
...
//echo $insertResubRequest."<br>";
		...
		{
		$insertResubRequest='Number Already in Subscription Table';
		}
                $log_file_path = "/var/www/html/hungamacare/log/bulk_resub_log_" . date('Y-m-d') . ".txt";
				$LogString = $batch_id."#".$msisdn . "#" . $channel . "#" . date('H:i:s') . "#" . $price . "#" . $insertResubRequest . "\n";
                error_log($LogString, 3, $log_file_path);
				$log_file_path2 = "/var/www/html/hungamacare/log/bulk_resub_logSub_" . date('Y-m-d') . ".txt";
				$LogString2 = $batch_id."#".$insertSub."#" . date('H:i:s') ."\n";
                error_log($LogString2, 3, $log_file_path2);
