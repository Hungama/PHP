        ...
        <center><div width="85%" align="left" class="txt">&nbsp;&nbsp;<FONT COLOR="#FF0000"> <B>Billing details for <?php echo $_GET['msisdn']; ?></B></FONT><br/>&nbsp;&nbsp;&nbsp;<i>Displaying last 30 transactions </i></div><center><br/>
                <TABLE width="95%" align="center" bgcolor="#0369b3" border="0" cellpadding="1" cellspacing="1" class="txt">
                    <TBODY>
                        ...
                        $query1 = "select * from ( select billing_ID,trans_id, event_type, date_time, amount,aval_amount,chrg_amount,MODE as mode,circle from master_db.tbl_billing_success where msisdn='" . $_GET[msisdn] . "' and service_id='" . $serviceId . "' and plan_id IN (" . implode(",", $planData) . ") ";

                        $query1 .=" UNION select billing_ID,trans_id, event_type, date_time, amount,aval_amount,chrg_amount,MODE as mode,circle from master_db.tbl_billing_success_backup where msisdn='" . $_GET[msisdn] . "' and service_id='" . $serviceId . "' and plan_id IN (" . implode(",", $planData) . ")";

                        $query1 .=" UNION select billing_ID,trans_id, event_type, date_time, amount,aval_amount,chrg_amount,MODE as mode,circle from master_db.tbl_billing_success_backup1 where msisdn='" . $_GET[msisdn] . "' and service_id='" . $serviceId . "' and plan_id IN (" . implode(",", $planData) . ") UNION ";

                        $query1 .= "select '' as billing_id,'' as trans_id,'UNSUB' as event_type,unsub_date as date_time,'0' as amount,'0' as aval_amount,'0' as chrg_amount,unsub_reason as 'mode',circle from ";
                        ...
                        switch ($_REQUEST['service_info']) {
                            ...
                            case '1116':
                                ...
                                break;
                            case '1123':
                                $query1.= "Mts_summer_contest.tbl_contest_unsub";
                                break;
                            case '1124':
                                $query1.= "mts_radio.tbl_AudioCinema_unsub";
                                break;
                            case '1125':
                                $query1.= "mts_JOKEPORTAL.tbl_jokeportal_unsub";
                                break;
                            case '1126':
                                $query1.= "mts_Regional.tbl_regional_unsub";
                                break;
                        }
...
//		$query1 .=" limit 2";
                        ...
                        while (list($billing_ID, $trans_id, $event_type, $date_time, $amount, $aval_amount, $chrg_amount, $MODE, $circle) = mysql_fetch_array($query)) {
                            if ($MODE != 'Insufficient Balance') {
                                ?>
                                <TR height="30">
                                    ...
                                    <TD bgcolor="#FFFFFF" align="center">&nbsp;<?php if ($circle == 'RAJ' and strtolower($event_type) == 'event') {
                        echo 'Pay Per Use';
                    } else {
                        echo $event_type;
                    } ?></TD>
                                    ...
                                    if ($MODE == 'null')
                                        $MODE = '';
                                    ?>
                                    <TD bgcolor="#FFFFFF" align="center">&nbsp;<?php if ($MODE == 'push') {
                                        $MODE = 'OBD1';
                                    } elseif ($MODE == 'push2') {
                                        $MODE = 'OBD2';
                                    } echo $MODE; ?></TD>
