...
$query = "select msisdn,operator,circle,date_time,mode from master_db.tbl_billing_success nolock where response_time between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '".$DateFormat[0]."'  and (mode IN ('OBD-MS','IVR-MS','net-MS','USSD-MS') OR SC='5464630') and event_type='SUB'"; 
...
while($row1=mysql_fetch_array($result)) {
	...
	if(strtoupper($mode) == 'OBD-MS') {
		$url="http://183.82.99.137/Hungama_OBD_MTS/index.php?mobno=".$msisdn."&operator=".$operator."&circle=".$circle."&fname=".$msisdn. "_&uname=vgtSub&pwd=HunSubApi";
	} else {	
