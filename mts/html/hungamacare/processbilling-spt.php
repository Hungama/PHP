...
include("config/dbConnect.php");
	...
	$select_query="select file_name, added_on, service_type, channel, price_point, upload_for,batch_id,service_id,amount,language from billing_intermediate_db.bulk_upload_history where status=0 and service_id in(1101,1102,1103,1111,1106,1110,1116,1113) limit 1";
	...
	while(list($file_name, $datetime, $service_type, $channel, $price, $upload_for,$batch_id,$service_id,$Newamount,$lang) = mysql_fetch_array($query)) 
	{
		...
		for($i=0;$i<$file_size;$i++)
		{
			...
			echo $getBlacklist="select count(*) from ".$tableName ." where ani=".$msisdn;
