...
$get_process_query = "select process.id,process.count,process.added_on,process.rule_id,process.type,rule.rule_name,rule.action_base,rule.service_id,rule.circle,process.filepath
    from honeybee_sms_engagement.tbl_new_engagement_data process join honeybee_sms_engagement.tbl_rule_engagement rule ON rule.id = process.rule_id
...
$get_rule_query = "SELECT distinct process.rule_id,rule.rule_name FROM honeybee_sms_engagement.tbl_new_engagement_data process join honeybee_sms_engagement.tbl_rule_engagement rule 
...
//                    $rule_get_query = "SELECT rule_name,service_id,circle,added_on FROM honeybee_sms_engagement.tbl_rule_engagement where id='" . $rule_id . "'";
//                    $ruleResult = mysql_query($rule_get_query, $dbConn);
//                    $rule_data_array = mysql_fetch_array($ruleResult);
                    $action_get_query = "SELECT action_name FROM honeybee_sms_engagement.tbl_rule_engagement_action where rule_id='" . $rule_id . "'";
					...
					$action_base = $processData['action_base'];
                    ...
                    $format_process_date = date('d-m-Y', strtotime($added_on));
                        ...
                        <TD>
                            ...
                            if ($processData['count'] == 0) {
                                ...
                                if ($action_base == 'pre_defined_time') {
                                    $filePath = 'SMS_Rule_Execution_' . $rule_id . '_' . $format_process_date . '.zip';
									$filePathurl="http://10.130.14.107/hungamacare/honey-bee/Script/ruledownloadlogs/".$filePath;
                                    ?>
                                    <a href="<?php echo $filePathurl ?>" target="_blank">
                                        <?php
                                    } else {
                                         ?>
                                        <a href="<?php echo $filePathurl ?>" target="_blank">
