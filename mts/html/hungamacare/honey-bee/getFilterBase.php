...
echo "SELECT Fid,description FROM honeybee_sms_engagement.tbl_filter_base WHERE status=1  and service_base like'%" . $service_base . "%' order by description";
$Result = mysql_query("SELECT Fid,description FROM honeybee_sms_engagement.tbl_filter_base WHERE status=1  and service_base like'%" . $service_base . "%' order by description", $dbConn);
