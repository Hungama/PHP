...
//////////////////// code start for Less than 10 MOUS in last 10 days /////////////////
    ...
    if ($result_row > 0) {
        $delete_query = "delete from honeybee_sms_engagement.tbl_new_engagement_number where date(added_on) = date(now()) and type = '1' and service_id='" . $s_id . "'";
        mysql_query($delete_query, $dbConn);
        while ($details = mysql_fetch_row($result)) {
            $insert_query = "insert into honeybee_sms_engagement.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status)
...
////////////// code start for More than 10, But less than 30 MOUS  in last 10 days ///////////
    ...
    $result_1030MOU = mysql_query($query_1030MOU, $dbConn);

    $result_row_1030MOU = mysql_num_rows($result_1030MOU);

    if ($result_row_1030MOU > 0) {
        $delete_query = "delete from honeybee_sms_engagement.tbl_new_engagement_number where date(added_on) = date(now()) and type = '2' and service_id='" . $s_id . "'";
        mysql_query($delete_query, $dbConn);
        while ($details_1030MOU = mysql_fetch_row($result_1030MOU)) {
            $insert_query_1030MOU = "insert into honeybee_sms_engagement.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status)
...
///////////////// code start for More than 30 MOUS in last 10 days //////////////
    ...
    $result_G30MOU = mysql_query($query_G30MOU, $dbConn);

    $result_row_G30MOU = mysql_num_rows($result_G30MOU);

    if ($result_row_G30MOU > 0) {
        $delete_query = "delete from honeybee_sms_engagement.tbl_new_engagement_number where date(added_on) = date(now()) and type = '3' and service_id='" . $s_id . "'";
        mysql_query($delete_query, $dbConn);
        while ($details_G30MOU = mysql_fetch_row($result_G30MOU)) {
            $insert_query_G30MOU = "insert into honeybee_sms_engagement.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status) 
...
/////////////////////// code start for Usage less than 5 mins in last 1 day /////////////////
    ...
    $result_L5 = mysql_query($query_L5, $dbConn);

    $result_row_L5 = mysql_num_rows($result_L5);

    if ($result_row_L5 > 0) {
        $delete_query = "delete from honeybee_sms_engagement.tbl_new_engagement_number where date(added_on) = date(now()) and type = '4' and service_id='" . $s_id . "'";
        mysql_query($delete_query, $dbConn);
        while ($details_L5 = mysql_fetch_row($result_L5)) {
            $insert_query_L5 = "insert into honeybee_sms_engagement.tbl_new_engagement_number (ANI,circle,added_on,service_id,type,status)
...
$query_G20 = "select distinct a.msisdn,b.circle,b.status from " . $subscription_db . "." . $subscription_table . " b ," . $calllog_db . "." . $calllog_table . " a
    ...
    $result_G20 = mysql_query($query_G20, $dbConn);

    $result_row_G20 = mysql_num_rows($result_G20);

    if ($result_row_G20 > 0) {
        $delete_query = "delete from honeybee_sms_engagement.tbl_new_engagement_number where date(added_on) = date(now()) and type = '5' and service_id='" . $s_id . "'";
        mysql_query($delete_query, $dbConn);
