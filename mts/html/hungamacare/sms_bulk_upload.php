...
<?php 
	 ...
	 if($_SERVER['REQUEST_METHOD']=="POST" && isset($_FILES['upfile']) && !empty($_FILES['upfile']['name'])) 
	{
		...
		if(isAllowedExtension($file['name'])) {			
			...
			$file_to_read="http://10.130.14.107/hungamacare/smsbulkuploads/".$_POST['service_info']."/".$makFileName;
			...
			$Uploadquery="insert into master_db.bulk_message_history(batch_id,file_name,msg_id,added_by,added_on,upload_for,status,operator,total_file_count, service_id, ip) values('".$batchId."', '".$dbMakFileName."', '".$msg_id."', '".$_SESSION[loginId]."', '".$thisTime."','active',0,'".$_SESSION[dbaccess]."','".$file_size."','".$serviceId."','".$remoteAdd."')";
			$queryIns = mysql_query($Uploadquery);
			
			/*$file_name1=file($file_to_read);
			$file_size1=sizeof($file_name1);
			$count = count($file_name1);
			for($i=0;$i<$count;$i++)
			{
				$insert_query="INSERT INTO master_db.bulk_msgupload_data (batch_id, msg_id, msisdn, service_id) VALUES ('".$batchId."', '".$msg_id."', '".$file_name1[$i]."', '".$_POST[service_info]."')";
				$query = mysql_query($insert_query) or die(mysql_error());
			}			
			$msg .="Data insert successfully<br/><br/>";*/

			$msg = "File <b>$makFileName<b> uploaded successfully.<br/><br/><a href='selectservice.php'>
			<div align='center'><B><FONT COLOR='#FF0000'>Home</FONT></a></B></div>"; 
...
<TABLE width="80%" align="center" border="0" cellpadding="0" cellspacing="0" class="txt">
      ...
      <TR height="30">
        <TD bgcolor="#FFFFFF"><a href='selectservice.php'>
			...
			|| <B><a href="smsviewhistory.php?s_id=<?php echo $_GET['service_info']?>" class=\"blue\"><FONT COLOR="#FF0000">View Upload SMS History</font></a></B> 
		</TD>
      </TR></TABLE>	  
