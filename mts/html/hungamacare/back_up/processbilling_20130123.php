...
include("config/dbConnect.php");
	...
	$select_query="select file_name, added_on, service_type, channel, price_point, upload_for,batch_id,service_id from billing_intermediate_db.bulk_upload_history where status=0 and service_id in(1101,1102,1103,1111,1106,1110,1116,1113) limit 1";
		...
		switch($upload_for)
		{
			case 'active':
				...
				if($service_id==1116) {
					...
					if($paramData[0]!='N') $cat1=$paramData[0]; 
					else $cat1=""; 
					if($paramData[1]!='N') $cat2=$paramData[1];
					else $cat2="";
					if($paramData[2]!='N') $lang=$paramData[2];
					else $lang="";
				}
			break;
			case 'deactive':
		...
		for($i=0;$i<$file_size;$i++)
		{
			...
			if(strlen(trim($msisdn))==10 && $BList<=0)
			{
				...
				{				
					...
					$billing_url .="&mode=".$channel."&reqtype=".$reqtype."&planid=".$price."&serviceid=".$service_id."&ac=0";
					if($service_id==1116) {
						$billing_url .="&cat1=".$cat1."&cat2=".$cat2."&lang=".$lang;
					}
					$billing_url .="&subchannel=bulk&rcode=100,101,102&batchid=".$batch_id;
					//echo $billing_url;
					$url_response=file_get_contents($billing_url);
				...
				elseif($reqtype=='topup') {
					...
					$insertToppupRequest="insert into master_db.tbl_billing_reqs values('',".trim($msisdn).",'TOPUP',now(),$price,0,'$lang',$sc,'$mode','$circle[0]','MTSM',$service_id,0,$plan_id)";
				...
				elseif($reqtype == "resub") {
					...
					$insertResubRequest="insert into master_db.tbl_billing_reqs values('','".$msisdn."','RESUB',now(),$amount,0,'01','".$shortCode."','$channel','$circle','MTSM',$service_id,$batch_id,$plan_id)";
					...
					$insertSub="insert into ".$subTable." (ANI,SUB_DATE,RENEW_DATE,DEF_LANG,STATUS,MODE_OF_SUB,DNIS,USER_BAL,SUB_TYPE,plan_id,circle, chrg_amount) value ('".$msisdn."', '".$subDate."', '".$renewDate."', 01, 0, '".$channel."', '".$shortCode."', '0', 'bulk', 
	'".$plan_id."','".$circle."','".$amount."')";
					mysql_query($insertSub);
