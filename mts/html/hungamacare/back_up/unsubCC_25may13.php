...
$result = mysql_query($Query1,$dbConn) or die(mysql_error());
	...
	header("location:displayMsg.php?msgCode=1");
	exit;
}
...
elseif ($_GET['msisdn'] != "" && $_GET['act'] == "mu") 
{
	...
	if($getPlanRecord[0]==38)
	{
		...
		header("location:displayMsg.php?msgCode=4");
		exit;
	}
	else
	{
		...
		header("location:displayMsg.php?msgCode=5");
		exit;
	}
}
