...
$serviceData = mysql_query("select distinct(service_id),event_type from " . $billibngTbl . " nolock where msisdn='" . $msisdn . "' and event_type in('SUB','EVENT','RESUB')");
$query="select distinct(service_id),event_type from " . $billibngTbl . " nolock where msisdn='" . $msisdn . "' and event_type in('SUB','EVENT','RESUB')";
//echo $query."<br>";
$countData = mysql_num_rows($serviceData);

if (!$countData) {

    $serviceData = mysql_query("select distinct(service_id),event_type from master_db.tbl_billing_success_backup nolock where msisdn='" . $msisdn . "' and event_type in('SUB','EVENT','RESUB')");
    $countData = mysql_num_rows($serviceData);
