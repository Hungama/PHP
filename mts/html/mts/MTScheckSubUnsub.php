...
*/
    $serviceData = mysql_query("select service_id,event_type from master_db.tbl_billing_success nolock where msisdn='" . $msisdn . "' and event_type in('SUB','EVENT','RESUB','SUB_RETRY') UNION (select service_id,event_type from master_db.tbl_billing_success_backup nolock where msisdn='" . $msisdn . "' and event_type in('SUB','EVENT','RESUB','SUB_RETRY')) ORDER BY service_id");
    $countData = mysql_num_rows($serviceData);
...
if ($countData) {
    ...
    while ($row = mysql_fetch_array($serviceData)) {
	  ...
	  if ($count != $serviceId)
	  {
		...
		$billingData = mysql_query("select date_format(date_time,'%d-%m-%Y %H:%i:%s') as date_time, chrg_amount 
		from master_db.tbl_billing_success nolock where msisdn='" . $msisdn . "' 
		and event_type = 'RESUB' and service_id='" . $serviceId . "' and response_time>='".$noDetail['actDate']."' 
		UNION select date_format(date_time,'%d-%m-%Y %H:%i:%s') as date_time, chrg_amount 
		from master_db.tbl_billing_success_backup nolock where msisdn='" . $msisdn . "' 
		and event_type = 'RESUB' and service_id='" . $serviceId . "' and response_time>='".$noDetail['actDate']."'");
       ...
       if ($noDetail['status'] == 1 || $noDetail['status'] == 11) {
			...
			echo "<CHARGEAMOUNT>" . $chAmt . "</CHARGEAMOUNT>\n";			
            echo "</SERVICE>\n";
        }
		}
