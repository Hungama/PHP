...
if ($reqType != '3') {
    ...
    $dataBalance.="<SERVICE_ID>$serviceid</SERVICE_ID>";
    $dataBalance.="<ISV_ID>4</ISV_ID>";
    $dataBalance.="<REQUEST_NAME>BC</REQUEST_NAME>";
    $dataBalance.="<MSISDN>$msisdn</MSISDN>";
    $dataBalance.="<SERVICE_TYPE>browsing</SERVICE_TYPE>";
    $dataBalance.="<RATE_PLAN_ID>$RPid</RATE_PLAN_ID>";
    ...
    $BALANCE_RESULT = $xml->BALANCE_CHECK_RESULT;
    error_log($BALANCE_RESULT_CODE . "#" . $BALANCE_RESULT_DESC . "#" . $BALANCE_MSISDN . "#" . $BALANCE_RESULT . "\n", '3', $logBalFilePath);
    
    if ($BALANCE_RESULT == 'ENOUGH_CREDIT' || $BALANCE_RESULT_CODE == '3106' || $BALANCE_RESULT_CODE == '5030') {
        
    }else{
        $respresult = 'sessionvalidity=0';
        echo $respresult;
        exit();
    }
}
