...
$logPath ="/var/www/html/MTS/logs/MTS/crbt/crbt_log_kar_".date("Ymd").".txt";

$logPath1 = "/var/www/html/MTS/logs/MTS/crbt/crbt_checklog_".date("Ymd").".txt";
switch($obdShed)
{
	case '10':
		$catId=2;
...
if($catId)
{
	$getQuery="select * from  mts_voicealert.tbl_voice_category where status=1 and obd_status=0 and IsDND=0 and cat_id=".$catId." and circle ='KAR'";
...
function curl1($Ani,$Circle)
{
	...
	curl_exec($ch);
	//echo "honey".$response;
	//$logData1 = $HitUrl."#1#".date("Y-m-d H:i:s")."\n";
