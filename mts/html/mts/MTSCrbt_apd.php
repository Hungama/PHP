...
$logPath ="/var/www/html/MTS/logs/MTS/crbt/crbt_log_apd_".date("Ymd").".txt";

$logPath_new = "/var/www/html/MTS/logs/crbt_checklog_".date("Ymd").".txt";
switch($obdShed)
{
	case '9':
		$catId=9;
...
if($catId)
{
	$getQuery="select * from  mts_voicealert.tbl_voice_category where status=1 and obd_status=0 and IsDND=0 and cat_id=".$catId." and circle ='APD'";
...
function curl1($Ani,$Circle)
{
	...
	//error_log($logData1,3,$logPath_new);
	return '100';	
}
echo 'done';
?>
