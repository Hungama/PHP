...
if($catId)
{
	$getQuery="select * from  mts_voicealert.tbl_voice_category where status=1 and obd_status=0 and IsDND=0 and cat_id=".$catId;
	...
	while($RecordRow = mysql_fetch_row($Record))
	{
		...
		$insertQuery="insert into mts_voicealert.tbl_OBD_category_com values(".$RecordRow[0].",".$RecordRow[3].",0,".$RecordRow[15].",".$RecordRow[16].",'".$RecordRow[17]."',now(),'".$RecordRow[11]."')";
