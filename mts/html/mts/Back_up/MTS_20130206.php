...
else
{
	...
	switch($seviceId)
	{
		...
		case '11011':			
			...
			if($batchId) $subscriptionProcedure="RADIO_SUB_BULK";
		...
		case '1111':
			...
			if($batchId) $subscriptionProcedure="DIGI_SUB_BULK";
			else $subscriptionProcedure="DIGI_SUB";
...
if($rows1[0] <=0)
{
	if($planid==11 || $planid==12 || $planid==13 || $planid==19) {
		...
		if($catId1 || $catId2) {
			$qry="call ".$DB.".". $subscriptionProcedure." ('".$msisdn."','".$lang."','".$mode."','".$sc."','".$amount."',".$s_id.", ".$planid.", '".$catId1."', '".$catId2."'";
		} else {
...
if($UCT != "")
		fwrite($file,$msisdn . "#" . $mode . "#".$reqtype."#".$planid."#".$subchannel."#".date('H:i:s') ."#uct->" .$UCT. "#" . $rcode . "\r\n" );
	else
		fwrite($file,$msisdn."#".$StartTime."#".$mode."#".$reqtype."#".$planid."#".$subchannel."#".$qry."#".date('H:i:s')."#".$rcode."\r\n" );
