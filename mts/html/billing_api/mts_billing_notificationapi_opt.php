...
$mts_serid=trim($_GET['mts_serid']);

$logDir="/var/www/html/billing_api/log/";
$logFile=date('dmY')."_Notificationlog";
$logFilePath=$logDir.$logFile.".txt";
$filePointer=fopen($logFilePath,'a+');

if($mts_serid!='null' && $mts_serid!='' && $mts_serid!='0')
  $serviceid=$mts_serid;
...
$rpIdMU_VA_MPD_TELECALLING_Array=array(90302519,90602511,90902511,91202511,91502514,91802511,92102508,92402511,92702511,93002514,93302500,93602502,93902500,94202502,94502500,94802502,95102500,95402502,95702500,96002502,96302500,96602500,96902500,97202500,97502500,97802500,98102500,98402500,98702500,99002500);
...
$rpIdBS_RP_54646_TELECALLING_Array=array(90302520,90602512,90902512,91202512,91502515,91802512,92102509,92402512,92702512,93002515,93302501,93602503,93902501,94202503,94502501,94802503,95102501,95402503,95702501,96002503,96302501,96602501,96902501,97202501,97502501,97802501,98102501,98402501,98702501,99002501);
...
$rpIdCZ_BS_TELECALLING_Array=array(90202515,90402514,90602515,90802512,91002513,91202515,91402512,91602512,91802515,92002515,92202512,92402515,92602512,92802512,93002518,93202503,93402503,93602506,93802503,94002503,94202506,94402503,94602503,94802506,95002503,95202503,95402506,95602503,95802503,96002506);
...
switch($reqName)
{
	case 'ASR': //XML Add subscription request:
		$data1="<RealTimeOperationRequest xmlns='xmltypes.cs.cvp.hp.com'>";
		$data1.="<ISV_ID>1</ISV_ID>";
		$data1.="<REQUEST_NAME>ASR</REQUEST_NAME>";
		$data1.="<MSISDN>$msisdn</MSISDN>";
		$data1.="<SERVICE_ID>$serviceid</SERVICE_ID>";
		$data1.="<SERVICE_START_TIME>$starttime</SERVICE_START_TIME>";
		$data1.="<SERVICE_END_TIME>$endtime</SERVICE_END_TIME>";
		$data1.="</RealTimeOperationRequest>";
		break;
	case 'USR': //XML Update subscription request:
		$data1="<RealTimeOperationRequest xmlns='xmltypes.cs.cvp.hp.com'>";
		$data1.="<ISV_ID>1</ISV_ID>";
		$data1.="<REQUEST_NAME>USR</REQUEST_NAME>";
		$data1.="<MSISDN>$msisdn</MSISDN>";
		$data1.="<SERVICE_ID>$serviceid</SERVICE_ID>";
		$data1.="<SERVICE_START_TIME>$starttime</SERVICE_START_TIME>";
		$data1.="<SERVICE_END_TIME>$endtime</SERVICE_END_TIME>";
		$data1.="</RealTimeOperationRequest>";
		break;
	case 'DSR': //XML Delete subscription request:
		$data1="<RealTimeOperationRequest xmlns='xmltypes.cs.cvp.hp.com'>";
		$data1.="<ISV_ID>1</ISV_ID>";
		$data1.="<REQUEST_NAME>DSR</REQUEST_NAME>";
		$data1.="<MSISDN>$msisdn</MSISDN>";
		$data1.="<SERVICE_ID>$serviceid</SERVICE_ID>";
		$data1.="</RealTimeOperationRequest>";
		break;
