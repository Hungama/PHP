...
//echo $sQuery="select SongUniqueCode from tbl_song_details where TT_ID='".$ringId."'";
//$sResult = mysql_query($sQuery) or die(mysql_error());
//echo mysql_num_rows($sResult);
//$row = mysql_fetch_array($sResult);		

//echo $row['SongUniqueCode'];
...
if($msisdn)
{
	switch($type) {
	case 'tt': $planid="71"; 
			  echo $sQuery = "select SongUniqueCode from uninor_myringtone.tbl_song_details where TT_ID='".$ringId."'";
			   $sResult = mysql_query($sQuery) or die(mysql_error());
			   echo mysql_num_rows($sResult);
			   //$rows1 = mysql_fetch_array($sResult);   echo $songId = $rows1[0];
			   //echo list($songId) = mysql_fetch_array($sResult); 
			   
			   //echo "2: ".$songId =$rows1['SongUniqueCode'];			   
		break;
	case 'mt': $planid="70"; 			   
			   $songId="";		
		break;
