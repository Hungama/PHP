...
if (($msisdn == "") || ($mode == "") || ($reqtype == "") || ($planid == "")) {
	...
	case 'uninor_jyotish':
        ...
        case 'uninor_rt':
            ...
            $subscriptionTable = "tbl_jyotish_subscription";
            $subscriptionProcedure = "RINGTONE_BULK_PUSH_new";
            $unSubscriptionProcedure = "Jyotish_UNSUB";
            $unSubscriptionTable = "tbl_Jyotish_unsub";
            $lang = '01';
			$songid='701_Hun-13-78193';
			$type='tt';
            break;
        default:
    ...
    if ($reqtype == 3) {
        ...
        if ($rows1[0] <= 0) {
            if($servicename=='uninor_rt')
                $call = "call " . $dbname . "." . $subscriptionProcedure . "($msisdn,'$lang','$mode','$sc','$amount',$s_id,$planid,'$ringid','$songid','$type',0)";
            else
