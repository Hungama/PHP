	...
	$.fn.smartCart = function(options) {
		...
		var defaults = {
			...
			// Text Labels
    			...
    			addedLabel: 'Song Added',	// text to show message when an item is added
    			removedLabel: 'Song Removed',	// text to show message when an item is removed
    			emptyCartLabel: '',	// text or empty cart (can be even html)
        		// Css Classes
		...
		return this.each(function(index) {  
			...
			function addCartItemEvent(e) {
				...
				if(scItemList.children("option").length >= 10) return false;
			        var prodId   = lastAddedItemId = $(this).attr('rel');
  				var prodName = $(options.itemNameSelectorPrefix+prodId).html();          
  				var $option = $("<option></option>").text(prodName).val(prodId).attr("selected", true); 
      ...
      function resetCartData() {
			...
			function resetListItem() {
				...
				if(scItemList.children("option").length > 0){
			            scItemList.children("option").each(function(n) {
			                var tmpOpt = $(this);
        				var $removeLink = $("<a><img src='/newportlet/images/delete.gif'/></a>")
        					.attr("href", "#")
                  ...
                  var itemId = itemVal;
                  var itemText = "<table width='100%'><tr><td>"+tmpOpt.html()+"</td><td width='70px'></td></tr></table>";
