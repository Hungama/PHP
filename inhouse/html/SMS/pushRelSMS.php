...
if($_GET['cli']) { $cli = trim($_GET['cli']); }
else $cli = 'RM-HNGAMA';

$con = mysql_connect("192.168.100.224","webcc","webcc");
		
$fileName="/var/www/html/SMS/log/Reliance/rel_log_".date("Y-m-d").".txt"; 
...
if((strlen($ani) == 10 || strlen($ani) == 12) && is_numeric($ani)) {
	if($message) {
		$sndMsgQuery = "CALL master_db.SENDSMS_NEW('".$ani."','".$message."','".$cli."','RELC','RELSMS',1)";
