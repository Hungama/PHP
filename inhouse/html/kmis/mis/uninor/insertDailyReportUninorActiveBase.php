...
if ($numRowsRIA > 0) {
    while (list($countGL, $circle) = mysql_fetch_array($activeBaseQuery1)) {
        ...
        elseif ($circle1 == "PUN")
            $circle1 = "PUB";
        $insert_data = "insert into mis_db.dailyReportUninor(report_date,type,circle,charging_rate,total_count,mode_of_sub,sub_type, mous,pulse,total_sec, service_id) values('$view_date1','Active_Base' ,'$circle1','NA','$countGL','NA','NA','NA','NA','NA',1413)";
        $queryIns = mysql_query($insert_data, $dbConn);
    }
}
...
$file_process_status = '************Script start for uninor desi beats Active Base**********' . ' #datetime#' . date("Y-m-d H:i:s") . "\r\n";
//error_log($file_process_status, 3, $processlog);
$getActiveBase = "select count(*),circle from misdata.tbl_base_active nolock where service='UninorReg' and status='Active' and date(date)='$view_date1' group by circle";
$activeBaseQuery1 = mysql_query($getActiveBase, $LivdbConn) or die(mysql_error());
...
if ($numRowsDB > 0) {
    while (list($countGL, $circle) = mysql_fetch_array($activeBaseQuery1)) {
        ...
        elseif ($circle1 == "PUN")
            $circle1 = "PUB";
        $insert_data = "insert into mis_db.dailyReportUninor(report_date,type,circle,charging_rate,total_count,mode_of_sub,sub_type, mous,pulse,total_sec, service_id) values('$view_date1','Active_Base' ,'$circle1','NA','$countGL','NA','NA','NA','NA','NA',1441)";
        $queryIns = mysql_query($insert_data, $dbConn);
    }
}
$file_process_status = '*******Script end for UninorDesi Beats Active Base**********' . ' #datetime#' . date("Y-m-d H:i:s") . "\r\n";
