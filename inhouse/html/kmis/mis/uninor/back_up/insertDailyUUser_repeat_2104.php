...
dnis like '52888%' and operator ='unim')temp ON item.msisdn= temp.msisdn where date(item.call_date)='$view_date1' and item.dnis like '52888%' and item.operator ='unim' group by item.circle";
$uu_tf_result = mysql_query($uu_tf_query, $dbConn) or die(mysql_error());
$numRows4 = mysql_num_rows($uu_tf_result);
if ($numRows4 > 0) {
    $uu_tf_result = mysql_query($uu_tf_query, $dbConn) or die(mysql_error());
