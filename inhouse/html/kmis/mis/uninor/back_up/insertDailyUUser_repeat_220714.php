...
if ($numRows4 > 0) {
    ...
    while ($uu_total = mysql_fetch_array($uu_total_result)) {
        ...
        echo $repeat_query = "select  count(distinct item.msisdn)
from mis_db.tbl_bollyalerts_calllog item INNER JOIN(  SELECT msisdn  FROM mis_db.mis_db.tbl_bollyalerts_calllog  where date(call_date) between DATE_SUB('$view_date1', INTERVAL 7 DAY) and DATE_SUB('$view_date1', INTERVAL 1 DAY) and operator ='unim'
