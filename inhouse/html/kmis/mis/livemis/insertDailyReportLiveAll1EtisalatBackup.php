...
//Activation
$get_activation_query="select count(msisdn),chrg_amount,service_id,plan_id from master_db.tbl_billing_success nolock where response_time between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]' and service_id in (2121) and event_type in('SUB') group by service_id,chrg_amount,plan_id"; 
...
if ($numRows > 0)
{
	...
	{
		if($count) {
			if($plan_id=="125" || $plan_id =="126") $circle="EPL";
			...
			echo $insert_data="insert into misdata.livemis(Date,Service,Circle,Type,Value,Revenue) values('$DateFormat[0]', '$service_name','".$circle."','$activation_str','$count',$revenue)";
...
//Mode Activation
$get_mactivation_query="select count(msisdn),chrg_amount,service_id,plan_id,mode from master_db.tbl_billing_success nolock where response_time between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]' and service_id in(2121) and event_type in('SUB') group by service_id,chrg_amount,plan_id,mode"; 
...
if ($numRows > 0)
{
	while(list($count,$charging_amt,$service_id,$plan_id,$mode) = mysql_fetch_array($get_mactivation_result))
	{
		if($count) {
			...
			$insert_data="insert into misdata.livemis(Date,Service,Circle,Type,Value,Revenue) values('$DateFormat[0]', '$service_name','".$circle."','$activation_str','$count',0)";
...
// Renewal
$get_renewal_query="select count(msisdn),chrg_amount,service_id,plan_id from master_db.tbl_billing_success nolock where response_time between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]' and service_id in(2121) and event_type in('RESUB') group by service_id,chrg_amount,plan_id"; 
...
if ($numRows > 0)
{
	while(list($count,$charging_amt,$service_id,$plan_id) = mysql_fetch_array($get_renewal_result))
	{
		if($count) {
			...
			$insert_data="insert into misdata.livemis(Date,Service,Circle,Type,Value,Revenue) values('$DateFormat[0]', '$service_name','".$circle."','$renewal_str','$count','$revenue')";
...
// Mode Deactivation
$unsubBase = "select count(*),'SPL',UNSUB_REASON FROM etislat_hsep.tbl_sfp_subscription_log WHERE  sub_date between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]'
UNION
select count(*),'EPL',UNSUB_REASON FROM etislat_hsep.tbl_epl_subscription_log WHERE  sub_date between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]'
UNION
select count(*),'Fun',UNSUB_REASON FROM etislat_hsep.tbl_funnews_subscription_log WHERE  sub_date between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]'
UNION
select count(*),'Jokes',UNSUB_REASON FROM etislat_hsep.tbl_jokes_subscription_log WHERE  sub_date between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]'
UNION
select count(*),'Hollywood',UNSUB_REASON FROM etislat_hsep.tbl_hollywood_subscription_log WHERE  sub_date between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]'";
