...
$service_array = array('MTSContest');
//$s_id='1101,1124,1102,1125,1103,1111,1106, 1110,1116,1113,1126,1123';
$s_id='1101';
...
//echo $DateFormat[0] = '2014-02-27 01:00:00';
//and date>'".$view_date1." 00:00:00'
//exit;

$DeleteQuery = "delete from misdata.livemis where date(date)='".$view_date1."'  and service IN ('" . implode("','", $service_array) . "')  and (type not like 'CALLS%' and type not like 'PULSE%' and type not like 'UU%' and type not like 'SEC%' and type not like 'MOU%' and type not like 'CNS_%') and type not in('Active_Base','Pending_Base')";
...
and service_id in($s_id) and event_type in('SUB','RESUB','TOPUP','EVENT','Event') group by circle,service_id,chrg_amount,event_type,plan_id,hour(response_time)";
echo $get_activation_query."<br>";
...
$get_mode_activation_query .=" where date(response_time)='".$view_date1."' 
 and service_id in($s_id) 
 and event_type in('SUB','EVENT','Event') group by circle,service_id,event_type,mode order by event_type,plan_id,hour(response_time)";
...
$get_deactivation_base = "select count(ani),circle,'1101' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_radio.tbl_radio_unsub 
where date(unsub_date)='".$view_date1."' and plan_id!=29 group by circle";
/*
union
select count(ani),circle,'1124' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_radio.tbl_AudioCinema_unsub
where date(unsub_date)='".$view_date1."' and status=1 group by circle
union
select count(ani),circle,'11012' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_radio.tbl_radio_unsub 
where date(unsub_date)='".$view_date1."' and plan_id=29 group by circle
union
select count(ani),circle,'1102' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_hungama.tbl_jbox_unsub 
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1125' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_JOKEPORTAL.tbl_jokeportal_unsub 
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1123' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from Mts_summer_contest.tbl_contest_unsub 
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1126' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_Regional.tbl_regional_unsub 
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1103' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_mtv.tbl_mtv_unsub 
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1111' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from dm_radio.tbl_digi_unsub
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1106' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_starclub.tbl_jbox_unsub
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1110' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_redfm.tbl_jbox_unsub
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1116' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_voicealert.tbl_voice_unsub
where date(unsub_date)='".$view_date1."' group by circle
union
select count(ani),circle,'1113' as service_name,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_mnd.tbl_character_unsub1
where date(unsub_date)='".$view_date1."' group by circle";
*/
...
$get_deactivation_base = "select count(ani),circle,'1101' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_radio.tbl_radio_unsub 
where date(unsub_date)='".$view_date1."' and plan_id!=29 group by circle,unsub_reason";
/*
union
select count(ani),circle,'1124' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_radio.tbl_AudioCinema_unsub 
where date(unsub_date)='".$view_date1."' and status=1 group by circle,unsub_reason
union
select count(ani),circle,'11012' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_radio.tbl_radio_unsub 
where date(unsub_date)='".$view_date1."' and plan_id=29 group by circle,unsub_reason
union
select count(ani),circle,'1002' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_hungama.tbl_jbox_unsub 
where date(unsub_date)='".$view_date1."' group by circle,unsub_reason
union
select count(ani),circle,'1003' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_mtv.tbl_mtv_unsub 
where date(unsub_date)='".$view_date1."' group by circle,unsub_reason
union
select count(ani),circle,'1111' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from dm_radio.tbl_digi_unsub
where date(unsub_date)='".$view_date1."' group by circle,unsub_reason
union
select count(ani),circle,'1106' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_starclub.tbl_jbox_unsub
where date(unsub_date)='".$view_date1."' group by circle,unsub_reason
union
select count(ani),circle,'1110' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_redfm.tbl_jbox_unsub
where date(unsub_date)='".$view_date1."' group by circle,unsub_reason
union
select count(ani),circle,'1116' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_voicealert.tbl_voice_unsub
where date(unsub_date)='".$view_date1."' group by circle,unsub_reason
union
select count(ani),circle,'1113' as service_name,unsub_reason,adddate(date_format(unsub_date,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) from mts_mnd.tbl_character_unsub1
where date(unsub_date)='".$view_date1."' group by circle,unsub_reason";
*/
$deactivation_base_query = mysql_query($get_deactivation_base, $dbConn) or die(mysql_error());
$numRows = mysql_num_rows($deactivation_base_query);
