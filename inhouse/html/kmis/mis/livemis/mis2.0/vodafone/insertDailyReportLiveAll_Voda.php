...
$kpi_process_status = '***************Script start for insertDailyReportLiveAll_Voda******************' . ' #datetime#' . date("Y-m-d H:i:s") . "\r\n";
...
$get_mode_activation_query = "select count(msisdn),circle,service_id,event_type,mode,sum(chrg_amount) as revenue from master_db.tbl_billing_success nolock ";
...
if ($numRows1 > 0) {
    while (list($count, $circle, $service_id, $event_type, $mode, $revenue) = mysql_fetch_array($db_query)) {
        $service_name = getServiceName($service_id);
        if ($circle_info[strtoupper($circle)] == '')
            $circle_info[strtoupper($circle)] = 'Other';
        if ($event_type == 'SUB') {
            $activation_str1 = "Mode_Activation_" . $mode;
            $insert_data1 = "insert into misdata.livemis(Date,Service,Circle,Type,Value,Revenue) values('$DateFormat[0]', '$service_name','" . $circle_info[strtoupper($circle)] . "','$activation_str1','$count','$revenue')";
        }
...
$get_deactivation_base = "select count(ani),circle,'1301' as service_name from vodafone_radio.tbl_radio_unsub where unsub_date between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]' group by circle";
...
$get_deactivation_base = "select count(ani),circle,'1301' as service_name,unsub_reason from vodafone_radio.tbl_radio_unsub where unsub_date between DATE_SUB('$DateFormat[0]', INTERVAL 1 HOUR) and '$DateFormat[0]' group by circle,unsub_reason";
...
$kpi_process_status = '***************Script end for insertDailyReportLiveAll_Voda******************' . ' #datetime#' . date("Y-m-d H:i:s") . "\r\n";
error_log($kpi_process_status, 3, $Allkpiprocessfile);
echo "generated";
?>