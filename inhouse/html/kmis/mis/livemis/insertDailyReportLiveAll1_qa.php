...
and service_id in(1001,1002,1003,1009,1013,1202,1203,1208,1402,1403,1410,1601,1602,1603,1605,1607,1609,1613,1801,1409,1809,1813,1010,1610,1810,1416,1408,1418,1423,1430,1431,1432,1433,1434) 
...
in (1001,1002,1003,1009,1013,1202,1203,1208,1402,1403,1410,1601,1602,1603,1605,1607,1609,1613,1801,1813,1409,1809,1010,1610,1810,1416,1408,1418,1423,1430,1431,1432,1433,1434) 
...
and service_id IN (1001,1002,1003,1009,1013,1202,1203,1208,1402,1403,1410,1601,1602,1603,1605, 1607,1609,1613,1801,1813,1409,1809, 1010,1610,1810,1416,1408,1418,1423,1430,1431,1432,1433,1434) 
...
$get_activation_query5 .= " where date(response_time)='" . $view_date1 . "' and service_id in(1410,1409,1402,1009,1609,1208,1430,1431,1432,1433,1434) and event_type in('EVENT','Event') group by circle,service_id,chrg_amount,plan_id,hour(response_time)";
...
$get_mode_activation_query61 = "select count(msisdn),circle,service_id,mode,plan_id,hour(response_time),adddate(date_format(response_time,'%Y-%m-%d %H:00:00'),INTERVAL 1 HOUR) as realTime from master_db.tbl_billing_success nolock ";
$get_mode_activation_query61 .=" where date(response_time)='" . $view_date1 . "' and service_id in(1410,1409,1009,1010,1402,1609,1208,1430,1431,1432,1433,1434) and event_type in('EVENT','Event') group by circle,service_id,mode order by plan_id,hour(response_time)";
...
$date_nexthour = date('H', strtotime('+1 hour'));
$DeleteQuery = "delete from misdata.livemis where date(date)='" . $view_date1 . "' and hour(date)>='" . $date_nexthour . "' and service IN ('" . implode("','", $service_array) . "')";
//$deleteResult12 = mysql_query($DeleteQuery, $LivdbConn) or die(mysql_error());
