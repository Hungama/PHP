...
$circle_info=array('DEL'=>'Delhi','GUJ'=>'Gujarat','WBL'=>'WestBengal','BIH'=>'Bihar','RAJ'=>'Rajasthan','UPW'=>'UP WEST','MAH'=>'Maharashtra','APD'=>'Andhra Pradesh','UPE'=>'UP EAST','ASM'=>'Assam','TNU'=>'Tamil Nadu','KOL'=>'Kolkata','NES'=>'NE','CHN'=>'Chennai','ORI'=>'Orissa','KAR'=>'Karnataka','HAY'=>'Haryana', 'PUN'=>'Punjab','MUM'=>'Mumbai','MPD'=>'Madhya Pradesh','JNK'=>'Jammu-Kashmir','PUB'=>"Punjab",'KER'=>'Kerala','HPD'=>'Himachal Pradesh','UND'=>'Others',''=>'Others');
...
//echo $DateFormat[0] = '2012-06-05 09:00:00';

///////// start the code to insert the data of activation Airtel 54646,MTV,GL,VH1 ////////////////

//delete data --------------
$service_array = array('EndlessMusic','Airtel54646','MTVAirtel','AirtelGL','VH1Airtel','RIAAirtel','AirtelPD','AirtelMND','AirtelComedy','AirtelSE', 'AirtelDevo','AirtelPK','AirtelRegTN','AirtelRegKK');
...
if ($numRows > 0)
{
	while(list($count,$circle,$service_id) = mysql_fetch_array($deactivation_base_query))
	{
		if($circle_info[strtoupper($circle)]=='')
			$circle_info[strtoupper($circle)]='Other';
		$service_name=getServiceName($service_id);
		$insert_data3="insert into misdata.livemis(Date,Service,Circle,Type,Value,Revenue) values(date_format('$DateFormat[0]','%Y-%m-%d %H'),'$service_name','".$circle_info[strtoupper($circle)]."','Deactivation_30','$count',0)";
		$queryIns = mysql_query($insert_data3, $LivdbConn);
	}
}
...
if ($numRows > 0)
{
	while(list($count,$circle,$service_id,$unsub_reason) = mysql_fetch_array($deactivation_base_query))
	{
		...
		elseif($unsub_reason == '546461' || strtoupper($unsub_reason) == 'BULK' || strtoupper($unsub_reason) == 'SELF_REQS') $unsub_reason ='IVR';
		else $unsub_reason = $unsub_reason;
