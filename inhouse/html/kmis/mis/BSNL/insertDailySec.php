...
$sec_t = array();
$sec_t_query = "select 'SEC_T',circle, count(msisdn),'BSNLPKP' as service_name,date(call_date),sum(duration_in_sec) from mis_db.tbl_mnd_calllog 
...
if ($numRows6 > 0) {
    ...
    while ($sec_t = mysql_fetch_array($sec_t_result)) {
        $insert_sec_t_data = "insert into mis_db.dailyReportBsnl(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,
        total_sec) values('$view_date1', '$sec_t[0]','$sec_t[1]','0','$sec_t[5]','','2213','NA','NA','NA')";
        $queryIns_sec = mysql_query($insert_sec_t_data, $dbConn);
    }
}
...
$sec_t = array();
$sec_t_query = "select 'SEC_T',circle, count(msisdn),'BSNLPKP' as service_name,date(call_date),sum(duration_in_sec),status 
from mis_db.tbl_mnd_calllog where date(call_date)='$view_date1' and dnis='546464'  and operator in('bsnl') group by circle,status";
