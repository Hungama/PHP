...
$pulse_t = array();
$pulse_t_query = "select 'PULSE_T',circle, sum(ceiling(duration_in_sec/60)),'BSNLPKP' as service_name,date(call_date),
sum(ceiling(duration_in_sec/60)) as pulse from mis_db.tbl_mnd_calllog where date(call_date)='$view_date1'  and dnis='546464'
and operator ='bsnl' group by circle";
...
if ($numRows31 > 0) {
	...
	$pulse_tRev=$pulse_t[5]*3;//NA
        $insert_pulse_t_data = "insert into mis_db.dailyReportBsnl(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,
        total_sec,Revenue) values('$view_date1', '$pulse_t[0]','$pulse_t[1]','3','$pulse_t[5]','','2213','NA','$pulse_t[5]','NA','$pulse_tRev')";
        $queryIns_pulseT = mysql_query($insert_pulse_t_data, $dbConn);
    }
}
...
$pulse_t = array();
$pulse_t_query = "select 'PULSE_T',circle, sum(ceiling(duration_in_sec/60)),'BSNLPKP' as service_name,date(call_date),
sum(ceiling(duration_in_sec/60)) as pulse,status from mis_db.tbl_mnd_calllog where date(call_date)='$view_date1' 
and dnis='546464' and operator ='bsnl' group by circle,status";
...
if ($numRows31 > 0) {
    ...
    while ($pulse_t = mysql_fetch_array($pulse_t_result)) {
        ...
        elseif ($pulse_t[6] != 1)
            $pulse_t[0] = "N_PULSE_T";
        $insert_pulse_t_data = "insert into mis_db.dailyReportBsnl(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,
        total_sec) values('$view_date1', '$pulse_t[0]','$pulse_t[1]','0','$pulse_t[5]','','2213','NA','$pulse_t[5]','NA')";
        $queryIns_pulseT = mysql_query($insert_pulse_t_data, $dbConn);
    }
}
