...
totalcount_sms=`cat $filePath | grep -c 'sms-promo'`
totalcount_nocall=`cat $filePath | grep -c 'no_call_promo'`
totalcount_conteng=`cat $filePath | grep -c 'CONT-ENG'`
...
$SQL -h$mysql_host -u$mysql_user  -p$mysql_password $mysql_db -e "insert into $mydb (totalcount,operator,message_type,added_on) values('$totalcount_conteng','UNIM','conteng','$added_date');"	 
