...
if ($numRows > 0) {
    while (list($count, $circle, $unsub_reason) = mysql_fetch_array($deactivation_base_query)) {
        ...
        if ($unsub_reason == "155223")
            $unsub_reason = "IVR";
        elseif (strtoupper($unsub_reason) == "CCI" || strtoupper($unsub_reason) == "CC" || strtoupper($unsub_reason) == "CCARE")
            $unsub_reason = "CCI";
...
// end code to insert the Deactivation base into the MIS database Docomo GL
//////////////////////// start code to insert the Deactivation Base into the MIS database TataDoCoMoMND @jyoti.porwal ////////////////////////////////////////////

$get_deactivation_base = "select count(*),circle,unsub_reason from docomo_mnd.tbl_character_unsub1
where date(unsub_date)='$view_date1' and plan_id='106' group by circle,unsub_reason";
...
if ($numRows > 0) {
    while (list($count, $circle, $unsub_reason) = mysql_fetch_array($deactivation_base_query)) {
        ...
        $deactivation_str1 = "Mode_Deactivation_" . $unsub_reason;
        $insert_data = "insert into mis_db.daily_report(report_date,type,circle,charging_rate,total_count,mode_of_sub,mous,pulse,total_sec,service_id) values('$view_date1', '$deactivation_str1','$circle','$chrg_amount','$count','$unsub_reason','NA','NA','NA',1013)";
        $queryIns = mysql_query($insert_data, $dbConn);
    }
}
//////////////////////// end code to insert the Deactivation Base into the MIS database TataDoCoMoMND @jyoti.porwal ///////////////////////////////////////
////////////////////////////////////////////// Start code to insert the Deactivation Base into the MIS database TataDoCoMoMND @jyoti.porwal//////////////////////

$get_deactivation_base = "select count(*),circle from docomo_mnd.tbl_character_unsub1 where date(unsub_date)='$view_date1' and plan_id='106' group by circle";
...
if ($numRows > 0) {
    while (list($count, $circle) = mysql_fetch_array($deactivation_base_query)) {
        ...
        values('$view_date1', 'Deactivation_30','$circle','NA','$count','NA','NA','NA','NA',1013)";
        $queryIns = mysql_query($insert_data, $dbConn);
    }
}
////////////////////////////////////////////// end code to insert the Deactivation base into the MIS database TataDoCoMoMND @jyoti.porwal//////////////////////
