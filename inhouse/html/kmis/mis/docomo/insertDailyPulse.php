...
$pulse_tf = array();
$pulse_tf_query = "select 'PULSE_TF',circle, sum(ceiling(duration_in_sec/60)),'endless' as service_name,date(call_date),sum(ceiling(duration_in_sec/60)) as pulse,status from mis_db.tbl_radio_calllog where date(call_date)='$view_date1' and dnis like '59090%' and dnis not like '590902%' and dnis not like '590905%' and operator in('TATM') group by circle,status";
...
// toll calling in enless

$pulse_tf = array();
$pulse_tf_query = "select 'PULSE_T_6',circle, sum(ceiling(duration_in_sec/60)),'endless' as service_name,date(call_date),sum(ceiling(duration_in_sec/60)) as pulse,status from mis_db.tbl_radio_calllog where date(call_date)='$view_date1' and (dnis like '590902%' or dnis like '590905%') and operator in('TATM') group by circle,status";
...
if ($numRows3 > 0) {
    while ($pulse_tf = mysql_fetch_array($pulse_tf_result)) {
        ...
        elseif (strtoupper($pulse_tf[1]) == 'HAR')
            $pulse_tf[1] = 'HAY';
        if ($pulse_tf[6] == 1)
            $pulse_tf[0] = 'L_PULSE_T_6';
        if ($pulse_tf[6] != 1)
            $pulse_tf[0] = 'N_PULSE_T_6';
...
$pulse_tf = array();
$pulse_tf_query = "select 'PULSE_T_6',circle, sum(ceiling(duration_in_sec/60)),'endless' as service_name,date(call_date),sum(ceiling(duration_in_sec/60)) as pulse from mis_db.tbl_radio_calllog where date(call_date)='$view_date1' and (dnis like '590902%' or dnis  like '590905%') and operator in('TATM') group by circle";
...
if ($numRows3 > 0) {
    while ($pulse_tf = mysql_fetch_array($pulse_tf_result)) {
        ...
        elseif (strtoupper($pulse_tf[1]) == 'HAR')
            $pulse_tf[1] = 'HAY';
        $insert_pulse_tf_data = "insert into mis_db.daily_report(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,
                total_sec) values('$view_date1', '$pulse_tf[0]','$pulse_tf[1]','6','$pulse_tf[5]','','1001','NA','$pulse_tf[5]','NA')";
        $queryIns_pulse = mysql_query($insert_pulse_tf_data, $dbConn);
    }
}
...
$pulse_tf = array();
$pulse_tf_query = "select 'PULSE_T_6',circle, sum(ceiling(duration_in_sec/60)),'Docomo54646' as service_name,date(call_date),sum(ceiling(duration_in_sec/60)) as pulse,status 
from mis_db.tbl_54646_calllog where date(call_date)='$view_date1' and  (dnis = '54646' or dnis like '546464%' or dnis like '546465%' or dnis like '546466%' or dnis like '546467%' or dnis like '546468%') and dnis not in('5464669','5464668') and dnis not like '5464669%' and dnis not like '%P%' 
...
$pulse_tf = array();
$pulse_tf_query = "select 'PULSE_T_6',circle, sum(ceiling(duration_in_sec/60)),'Docomo54646' as service_name,date(call_date),sum(ceiling(duration_in_sec/60)) as pulse
 from mis_db.tbl_54646_calllog where date(call_date)='$view_date1' and  (dnis = '54646' or dnis like '546464%' or dnis like '546465%' or dnis like '546466%' or dnis like '546467%' or dnis like '546468%') and dnis not in('5464669','5464668') and dnis not like '5464669%' and dnis not like '%P%' and operator ='tatm' group by circle";
...
if ($numRows3 > 0) {
    while ($pulse_tf = mysql_fetch_array($pulse_tf_result)) {
        ...
        elseif (strtoupper($pulse_tf[1]) == 'HAR')
            $pulse_tf[1] = 'HAY';
        $insert_pulse_tf_data = "insert into mis_db.daily_report(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,
                total_sec) values('$view_date1', '$pulse_tf[0]','$pulse_tf[1]','6','$pulse_tf[5]','','1002','NA','$pulse_tf[5]','NA')";
        $queryIns_pulse = mysql_query($insert_pulse_tf_data, $dbConn);
    }
}
...
/////////////////////////////////////////start code to insert the data for PULSE_T for the Tata Docomo 54646 /////////////////////////////////////////
$pulse_tf1 = array();
$pulse_tf_query1 = "select 'PULSE_T_1',circle, sum(ceiling(duration_in_sec/60)),'DocomoMissRiya' as service_name,date(call_date), sum(ceiling(duration_in_sec/60)) as pulse, dnis, status 
from mis_db.tbl_54646_calllog where date(call_date)='$view_date1' and dnis in ('5464668') and operator ='tatm' group by circle,dnis,status";
...
$pulse_tf1 = array();
$pulse_tf_query1 = "select 'PULSE_T_1',circle, sum(ceiling(duration_in_sec/60)),'DocomoMissRiya' as service_name,date(call_date), sum(ceiling(duration_in_sec/60)) as pulse, dnis 
from mis_db.tbl_54646_calllog where date(call_date)='$view_date1' and dnis in('5464668') and operator ='tatm' group by circle,dnis";
...
$pulse_tf1 = array();
$pulse_tf_query1 = "select 'PULSE_T_6',circle, sum(ceiling(duration_in_sec/60)),'DocomoMissRiya' as service_name,date(call_date), sum(ceiling(duration_in_sec/60)) as pulse, dnis, status 
from mis_db.tbl_54646_calllog where date(call_date)='$view_date1' and dnis like '5464669%' and operator ='tatm' group by circle,dnis,status";
...
if ($numRows31 > 0) {
    while ($pulse_tf1 = mysql_fetch_array($pulse_tf_result1)) {
        ...
        elseif (strtoupper($pulse_tf1[1]) == 'HAR')
            ...
            if ($pulse_tf1[7] == 1)
                $pulse_tf1[0] = 'L_PULSE_T_6';
            if ($pulse_tf1[7] != 1)
                $pulse_tf1[0] = 'N_PULSE_T_6';
...
$pulse_tf1 = array();
$pulse_tf_query1 = "select 'PULSE_T_6',circle, sum(ceiling(duration_in_sec/60)),'DocomoMissRiya' as service_name,date(call_date), sum(ceiling(duration_in_sec/60)) as pulse, dnis 
from mis_db.tbl_54646_calllog where date(call_date)='$view_date1' and dnis like '5464669%' and operator ='tatm' group by circle,dnis";
...
if ($numRows31 > 0) {
    while ($pulse_tf1 = mysql_fetch_array($pulse_tf_result1)) {
        ...
        $insert_pulse_tf_data1 = "insert into mis_db.daily_report(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,
                total_sec) values('$view_date1', '$pulse_tf1[0]','$pulse_tf1[1]','6','$pulse_tf1[5]','','1009','NA','$pulse_tf1[5]','NA')";
        $queryIns_pulse1 = mysql_query($insert_pulse_tf_data1, $dbConn);
    }
}
...
////////////////////////////////////////start code to insert the data for PULSE_TF for the TataDoCoMoMND SErvice @jyoti.porwal /////////////////////////////////
$pulse_tf = array();
$pulse_tf_query = "select 'PULSE_TF',circle, sum(ceiling(duration_in_sec/60)),'TataDoCoMoMND' as service_name,date(call_date),sum(ceiling(duration_in_sec/60)) as pulse 
...
if ($numRows3 > 0) {
    while ($pulse_tf = mysql_fetch_array($pulse_tf_result)) {
        ...
        elseif (strtoupper($pulse_tf[1]) == 'HAR')
            $pulse_tf[1] = 'HAY';
        $insert_pulse_tf_data = "insert into mis_db.daily_report(report_date,type,circle,charging_rate,total_count,mode_of_sub,service_id,mous,pulse,
                total_sec) values('$view_date1', '$pulse_tf[0]','$pulse_tf[1]','0','$pulse_tf[5]','','1013','NA','$pulse_tf[5]','NA')";
        $queryIns_pulse = mysql_query($insert_pulse_tf_data, $dbConn);
    }
}

$pulse_tf = array();
$pulse_tf_query = "select 'PULSE_TF',circle, sum(ceiling(duration_in_sec/60)),'TataDoCoMoMND' as service_name,date(call_date),sum(ceiling(duration_in_sec/60)) as pulse,status 
...
////////////////////////////////////////end code to insert the data for PULSE_TF for the TataDoCoMoMND SErvice @jyoti.porwal ///////////////////////////
?>