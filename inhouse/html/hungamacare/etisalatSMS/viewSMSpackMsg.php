...
function deleteMessage(msgid){
  var answer = confirm("Are You Sure To Want Delete Selected message?");
                if(answer){ 
                }else{ 
                    return false;
                }
...
<tr height="30">
	...
	<td bgcolor="#FFFFFF">&nbsp;&nbsp;<select name='service' id="service" onchange="showHidemore()">
			...
			<option value='DAP' <?php if($_REQUEST['service']=='DAP') echo "SELECTED";?>>Anxiety</option>
			<option value='CCP' <?php if($_REQUEST['service']=='CCP') echo "SELECTED";?>>Career Counselling</option>
			<option value='HBP' <?php if($_REQUEST['service']=='HBP') echo "SELECTED";?>>Hygiene</option>
			<option value='LENTEN' <?php if($_REQUEST['service']=='LENTEN') echo "SELECTED";?>>LENTEN</option>
			<option value='LENTN' <?php if($_REQUEST['service']=='LENTN') echo "SELECTED";?>>LENTN</option>
			<option value='LNT' <?php if($_REQUEST['service']=='LNT') echo "SELECTED";?>>LNT</option>
			<option value='PRAYER' <?php if($_REQUEST['service']=='PRAYER') echo "SELECTED";?>>PRAYER</option>
			<option value='LENT' <?php if($_REQUEST['service']=='LENT') echo "SELECTED";?>>LENT</option>
		</select></td>
</tr>
<!--<tr height="30">
		...
		if($_REQUEST['service']) {
			...
			<table class="txt2" bgcolor="#e6e6e6" border="0" cellpadding="0" cellspacing="1" width="80%" align='center'>
				<tr align='center'>
					...
					<?php } ?>
						<th style="padding-left: 5px;" bgcolor="#ffffff" height="35">Action</th>
				</tr>
			<?php while($row = mysql_fetch_array($result)) { ?>
				<tr>
					<td style="padding-left: 5px;" bgcolor="#ffffff" height="35" align='center'><?php echo $i; ?></td>
					<td style="padding-left: 5px;" bgcolor="#ffffff" height="35" align='center'><?php echo date($row['date']); ?></td>
					...
					<?php } ?>
				<td style="padding-left: 5px;" bgcolor="#ffffff" height="35" align='center'><a href="#" onclick="deleteMessage('<?php echo $row['msg_id']; ?>')">Delete</a></td>
				</tr>
			<?php $i++;} ?>
			</table>
			<?php } else { ?>
