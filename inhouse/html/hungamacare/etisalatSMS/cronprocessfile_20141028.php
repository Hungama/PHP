...
//$checkfiletoprocess=mysql_query("select batchid,servicename,serviceId, filename, date_time,status,circle,plainid,flagvalue from etislat_hsep.tbl_sms_history_etisalat where status=0 order by batchid ASC limit 1");
...
//$mytime=date("Y-m-d H:i:s",strtotime($smstime));
...
//$mytime=date("m-d-Y H:i:s",strtotime($smstime));
//m d y
$datetime=date("Y-m-d", mktime(0, 0, 0,$explodedDate[1],$explodedDate[2],$explodedDate[0]));
$smsmessage=str_replace("\"", "", trim($data[2]));
$smsmessage=str_replace("’", "'", $smsmessage);
