...
<?php
	if($_SERVER['REQUEST_METHOD']=="POST")
	{
		...
		if($_REQUEST['service']) {
			...
			if($service == 'AST') $selectList = "date_time as date,message,sun_sign";
			else $selectList = "date_time as date,message";
			...
			<?php while($row = mysql_fetch_array($result)) { ?>
				<tr>
					...
					<td style="padding-left: 5px;" bgcolor="#ffffff" height="35"><?php echo $row['message']; ?></td>
					<?php if($service == 'AST') { ?>
