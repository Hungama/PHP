...
{ 
	...
	if( $name == "user-agent" )
	{
		...
		include_once "/var/www/html/hungamacare/wapstream/hungamawap/hs_capture.php"; // Handset Identification Mechanism 
		...
		if(strlen(trim($model)) <= 0 || trim($model)=="")
		{
			...
			$handset_missing_log="/var/www/html/hungamacare/wapstream/hungamawap/logs/mobile/".date("Ymd")."_hs_not_found.txt"; // Missing Handset Details Log
			...
			if($ref1[0]=='apn')
				$apn=$ref1[1];
                         ...
                         if($ref1[0]=='imsi')
                        {
							...
							$imsi_logs = "/var/www/html/hungamacare/wapstream/hungamawap/logs/".date('Ymd')."_aircel_imsi.txt";
...
if(strlen($msisdn)<=0)
{
	...
	$header_log = "/var/www/html/hungamacare/wapstream/hungamawap/logs/empty_msisdn_headerlog_".$date.".txt";	
	
	echo $empty_msisdn =$time_stamp."|".$header_str."&".$_SERVER['REQUEST_URI'];
