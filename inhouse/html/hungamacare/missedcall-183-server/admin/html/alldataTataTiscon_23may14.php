...
$getDashbord_TargettdOBD=mysql_query("select  distinct ANI from Hungama_Tatasky.tbl_tataobd_success_fail_details nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!=''",$con);
...
/*$getDashbord_MaxMin=mysql_query("select ceil(max(duration)/60) as maxDuration from Hungama_Tatasky.tbl_tataobd_success_fail_details nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' ",$con);
list($maxDuration1) = mysql_fetch_array($getDashbord_MaxMin); 
$maxDuration=$maxDuration1;
*/

$getDashbord_MaxAvgDuration=mysql_query("select max(duration) as MaxDuration, ceil(AVG(duration)) as AvgDuration from Hungama_Tatasky.tbl_tataobd_success_fail_details nolock  
where date(date_time) between '2014-04-19' and '2014-05-19' and ANI!='' and status=2 ",$con);
...
$getDashbord_OBD_Ad=mysql_query("select sum(tiscon_addcount) as totalcount from Hungama_Tatasky.tbl_tiscon_subscription",$con);
list($totalAdCount) = mysql_fetch_array($getDashbord_OBD_Ad); 
...
$allnewvsuniquevisit=mysql_query("select distinct(ANI) as dani,(select count(1) from Hungama_Tatasky.tbl_tata_pushobd where ANI=dani) as total
from Hungama_Tatasky.tbl_tata_pushobd nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!='' ",$con);
$totalUniqueVisit=0;
