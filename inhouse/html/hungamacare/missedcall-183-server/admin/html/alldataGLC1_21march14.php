...
$getDashbord_TargettdOBD=mysql_query("select  distinct ANI from hul_hungama.tbl_hulobd_success_fail_details nolock  where date(date_time)>='2014-02-28' and service='HUL' and ANI!=''",$con);
$totalOBD_targettd=mysql_num_rows($getDashbord_TargettdOBD);
//date(date_time)>='2014-02-28' and
///date(date_time) between '".$StartDate."' and '".$EndDate."' and 

//for section 7,8,9
//Max min spent by unique user
$getDashbord_MaxMin=mysql_query("select max(duration) as maxDuration from hul_hungama.tbl_hulobd_success_fail_details nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and service in ('HUL')",$con);
...
$getDashbord_OBD=mysql_query("select count(ANI) from hul_hungama.tbl_hulobd_success_fail_details nolock where date(date_time)>='2014-02-28' and service='HUL_PROMOTION' and ANI!='' ",$con);
list($totalOBD_attended_Promotional) = mysql_fetch_array($getDashbord_OBD); 
$getDashbord_OBD_content=mysql_query("select count(ANI) from hul_hungama.tbl_hulobd_success_fail_details nolock  where date(date_time)>='2014-02-28' and service='HUL' and ANI!=''",$con);
...
from hul_hungama.tbl_hul_pushobd_sub nolock where date(date_time)=date(now()) and ANI!=''",$con);
...
$getDashbord_data_allnew="select count(*) as total, count(distinct(ANI)) as total_unique from hul_hungama.tbl_hul_pushobd_sub nolock 
 where date(date_time)>='2014-02-28' and ANI!='' ";
...
from hul_hungama.tbl_hul_pushobd_sub nolock  where ANI!='' group by date(date_time) order by day(date_time) ASC";
...
//New Visits
$getDailyVisits_data="select ANI from hul_hungama.tbl_hul_pushobd_sub nolock  where date(date_time)=date(now()) and ANI!='' ";
...
from hul_hungama.tbl_hul_pushobd_sub nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!='' group by ANI";
...
from hul_hungama.tbl_hul_pushobd_sub nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!='' group by date(date_time) order by day(date_time) ASC";
