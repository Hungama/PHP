...
require_once("alldataTataTiscon_q.php");

$getDashbordChart_missedCallNewChart="select ANI as total, day(date_time) as day,Month(date_time) as month,Year(date_time) as year
from Hungama_Tatasky.tbl_tata_pushobd nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!=''
 union all select count(*) as total, day(date_time) as day,Month(date_time) as month,Year(date_time) as year
from Hungama_Tatasky.tbl_tata_pushobd_backup_25dec nolock  
where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!='' group by date(date_time) order by year, month,day ASC";
$query_missedCallNewChart = mysql_query($getDashbordChart_missedCallNewChart,$con);

$getDashbordChart_UniqueCallNewChart="select count(distinct(ANI)) as total_unique,day(date_time) as day,Month(date_time) as month,Year(date_time) as year
from Hungama_Tatasky.tbl_tata_pushobd_unique nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!='' group by date(date_time) order by year, month,day ASC";
...
from Hungama_Tatasky.tbl_tata_pushobd nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!=''
union all select count(*) as total, hour(date_time) as hour
from Hungama_Tatasky.tbl_tata_pushobd_backup_25dec nolock  where date(date_time) between '".$StartDate."' and '".$EndDate."' and ANI!='' group by hour(date_time) order by hour(date_time) ASC";
...
$year=$data_missedCallNewChart['year'];
$month=$data_missedCallNewChart['month']-1;
$total_missed_callNew=$data_missedCallNewChart['total'];
?>
{ x: new Date(<?php echo $year; ?>, <?php echo $month;?>, <?php echo $day;?>), y: <?php echo $total_missed_callNew;?>},
<?php
}
?>
  ]
...
$year1=$data_uniqueCallNewChart['year'];
$total_unique_callNew=$data_uniqueCallNewChart['total_unique'];
?>
{ x: new Date(<?php echo $year1; ?>, <?php echo $month1;?>, <?php echo $day1;?>), y: <?php echo $total_unique_callNew;?>},
<?php
}
?>
  ]
       ...
       <!--Body content-->
        <div id="content" class="clearfix">
            <div class="contentwrapper"><!--Content wrapper-->
              ...
              <!--h3>Dashboard</h3--> 
<div class="col-lg-8">					
<form id="rangepicker" action="dashboardTataTiscon_q.php" method="post">					
                    ...
                    <div class="col-lg-8">
                        <div class="centerContent">
                             <ul class="bigBtnIcon">
								...
								<!--li>
                                    <a href="#" title="Total Pulse consumed through OBD" class="pattern tipB">
                                        <span class="icon icomoon-icon-basket"></span>
										<span class="txt">Total Pulses Consumed</span>
                                        <span class="notification"><?php echo number_format($totalOBD_attendedPulse_Content);?></span>
                                    </a>
                                </li-->
...
<a href="exportData.php?StartDate=<?php echo $StartDate;?>&EndDate=<?php echo $EndDate;?>&type=missedcall"><span class="icon icomoon-icon-download-2"></span></a>
<input type="hidden" value="89" class="greenCircle" />
</div>

<div class="circular-item tipB" title="Usage of content per MDN on OBD">
<a href="exportData.php?StartDate=<?php echo $StartDate;?>&EndDate=<?php echo $EndDate;?>&type=content"> <span class="icon icomoon-icon-download"></span></a>
<input type="hidden" value="97" class="blueCircle" />
</div>
<div class="circular-item tipB" title="Tata Tiscon Automation Report">
<a href="http://182.72.55.117/hungamacare/missedcall/admin/html/excel/contents-display.php"> <span class="icon icomoon-icon-download"></span></a>
<input type="hidden" value="97" class="blueCircle" />
</div>
...
<div class="row">
 <div class="col-lg-8">
						...
						<div class="panel-heading">
                                <h4>
 <select onchange="javascript:setChartType(this.value)">
 <option value="0">Please Select Chart Type</option>
  <option value="1">Missed Calls  Vs Total unique Visitors</option>
  <option value="2">Missed Calls - Hour wise</option>
</select>
</h4>
</div>
</div>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="panel panel-default chart gradient" id="visitor_chart1">
                            <div class="panel-heading">
                                <h4>
                       ...
                       <ul class="chartShortcuts">
                                    <li>
                                        <a href="#">
                                            ...
                                            <span class="number"><?php echo number_format($total_user);?></span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <span class="head">Total Unique Visitor</span>
                                            <span class="number"><?php echo number_format($total_unique_user);?></span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#">
                                            <span class="head">Today's New Visitor(s)</span>
                                            <span class="number"><?php echo number_format($totalNewUniqueVisitToday);?></span>
                                        </a>
                                    </li>
						 ...
						 <div class="panel panel-default chart gradient" id="visitor_chart2">
                            <div class="panel-heading">
                                <h4>
                                    <span class="icon16 icomoon-icon-bars"></span>
                                    <span>Visitors Chart Hour Wise</span>
                                </h4>
                                <a href="#" class="minimize">Minimize</a>
                            </div>
                            <div class="panel-body" style="padding-bottom:0;">
                        	
							    <div id="chartContainer_hourwise" style="height:230px;width:100%;margin-top:15px; margin-bottom:15px;"></div>
				...
				<span class="sparkLine_newVisitor"></span>% Of New Visitor: <span class="number"><?php echo $totalnewvisist_percetage_outofuniqueuser;?></span>
								...
								<!--li>Tata Tiscon Ad Impression : <span class="number"><?php echo $totalAdCount;?></span></li-->
							</ul>
