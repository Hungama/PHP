...
$subStatus=-1;
	...
	<a href="javascript:void(0);" onclick="viewbillinghistory(<?php echo $msisdn;?>,<?php echo $service_info;?>,'<?php echo $subsrv;?>')">	
	<?php echo $date_time; ?></a>
	</TD>
	...
	<?php if($service_info=='1208') { ?>
		...
		if (($subStatus == 1 || $subStatus == 5 || $subStatus == 2) && !in_array($_REQUEST['usrId'],$blacklistID)) { 
			if($_SESSION['usrId']!=265) { ?>
				<!--a href="customer_care.php?msisdn=<?php echo $msisdn;?>&act=da&service_info=<?php echo $service_info;?>&subsrv=<?php echo $subsrv;?>" style="text-decoration:none">
				<b><span class="label label-important">Deactivate</span></b></a-->
		<a href="javascript:void(0);" onclick="do_Act_Deactivate(<?php echo $msisdn;?>,<?php echo $service_info;?>,'<?php echo $subsrv;?>','da','customer_care_reliance')" style="text-decoration:none">
...
<?php
        } else
        {
			if($_SESSION['usrId'] == 24) {  //echo "hello:".$_POST['msisdn'];?>
				...
				<FONT COLOR="#0033FF" size='2px'><span class="label label-info">Check Sub-Retry</span></FONT></a></div>
			<?php  }
			$flag=1;
