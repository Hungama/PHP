...
document.getElementById("langDiv").innerHTML=myAjax.responseText;
	...
	var url="Content.Live_langValue.php?circle="+circleCode+"&case=1&service="+service;
	//alert(url);
...
function showDevoContent_main(cat,circle,lang,clip,rel,navlang) {document.getElementById('ac1Div').style.display = 'none';
document.getElementById("showinfo").innerHTML='<img src="assets/img/loading-circle-48x48.gif" border="0"/>';
	myAjax.onreadystatechange=function() {
		if (myAjax.readyState==4 && myAjax.status==200) {
		clearTimeout(xhrTimeout); 
