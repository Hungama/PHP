...
foreach ($contentid_array as $key => $value) {
    ...
    foreach ($array['files']['file'] as $val) {
        ...
        if ($ext == 'mp4') {
           ...
           // $url = "http://mdn.hungama.com/download/" . $value . "/" . $contentFileType_details1[0] . "/" . $subType_details1[0] . "?duration=PT0H0M30S&cdn=akamai&agent=application&cms=ms2&protocol=filedl";
            ...
            echo $qry = "update uninor_summer_contest.question_bank_wapcontest set video='" . $output . "' where content_id='" . $value . "'";
