...
<div class="row">
  ...
  <button type="button" class="btn btn-success"><a href="downloadData.php?S_id=<?=$sid?>">Download TSV</a></button>
</div><br/>
    </div>
    </div>
	...
	<form id="form-SMS" name="form-SMS" method="post" enctype="multipart/form-data">
	  <table class="table table-bordered table-condensed">
	   ...
	   <tr>
		  ...
		  <select name="msg_type" data-width="auto">
		   <option value="0">Select KCI Type</option>
				...
				echo "<option value=".$dpvalue.">".ucfirst($row['msg_type'])."</option>";
				}
				?>
	        	</select></td>
        </tr>
         ...
         <tr>
                                ...
                                <td align="left" colspan="2"><select name="priority" data-width="auto">
                                        ...
                                        <?php foreach ($Priority as $prid) {
										if($prid=='5') {$selected='selected';}
                                            ?>
                                            <option value="<?php echo $prid; ?>" <?php echo $selected;?>><?php echo $prid; ?></option>
                                        <?php } ?>
                                    </select></td>
...
{
 $('#loading_sms').show();
 //document.getElementById('loading_sms').style.display='block';
    ...
    $.ajax({
		...
		if(data=='NOK') {var msg='Please try again.';} else {var msg='Message has been saved successfully.';}
	document.getElementById('grid-SMS').style.display='inline';
	document.getElementById('grid-SMS').innerHTML='<div width="85%" align="left" class="txt"><div class="alert alert-success"><a class="close" data-dismiss="alert">&times;</a>'+msg+'</div></div>';
	//document.getElementById('loading_sms').style.display='none';
	resestForm('SMS');   //
	$('#loading_sms').hide();
	//getMessage(data);
        ...
        function showMessageBox() {
		 ...
		 function deleteMsg(msg_id) {
            ...
            xmlhttp.onreadystatechange=function() {
				...
				//var res = response.split("#");alert(response);
				if(response!='NOK')
