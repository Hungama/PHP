...
//code for balance uses
else if ($_REQUEST['type'] == 'balance_uses') {
    $data_query = "select ANI,BNI,duration,patchstart_date_time,patchstop_date_time from hul_hungama.tbl_GLC_CallPatch order by patchstop_date_time desc";
} 
...
if ($result_row > 0) {
    ...
    elseif($_REQUEST['type'] == 'balance_uses')
    {
        echo "ANI,BNI,Duration,Patchstart_date_time,Patchstop_date_time" . "\r\n";
        while ($mis_array = mysql_fetch_array($data)) {
            echo $mis_array['ANI'] . "," . $mis_array['BNI'] . "," . $mis_array['duration'] . "," . $mis_array['patchstart_date_time'] . "," . $mis_array['patchstop_date_time'] . "\r\n";
        }
