...
where date(date_time)='".$PDate."' and service='GLC' order by Data_Hour desc";
...
$message .= '<table rules="all" style="border-color: #666;font-size:12px;width:100%" border="0" cellpadding="2">';
$message .= "<tr bgcolor='#000000' style='color:#ffffff'><td>OBD Application</td><td>Time</td><td>Missed Calls</td><td>OBD's Connected</td><td>Average Success %</td><td>OBD Failed</td><td>Average Response Time (sec)</td><td>Min Response  Time (sec)</td><td>Max Response  Time (sec)</td></tr>";

while ($row = mysql_fetch_array($data)) {    
$message .= "<tr bgcolor='#F5DEB3'><td>".$row['service']."</td><td>".$row['timeToshow']."</td><td>".$row['TotalMissedCall']."</td><td>".$row['OBDConnected']."</td><td>".$row['AVGSuccess']."</td><td>".$row['OBDFailed']."</td><td>".$row['AVG_RESP_TIME']."</td><td>".$row['MIN_RESP_TIME']."</td><td>".$row['MAX_RESP_TIME']."</td></tr>";
}
$message .= "</table>";

$message .="</body></html>";
echo $message;
$htmlfilename = 'emailcontentGLC_' . date('Y_m_d') . '.html';
