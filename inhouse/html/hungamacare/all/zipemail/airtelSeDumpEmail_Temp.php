...
$sql_getreferdata = mysql_query("select ANI,friendANI,referDate,endDate,service_id,referfrom,userCircle,status,chrgAmount,optServiceId,date(chrgDate) as chrgDate from master_db.tbl_refer_ussdData where (date(referDate) between date(date_sub(now(),interval 7 day)) and date(date_sub(now(),interval 6 day)) 
and userCircle='UPW' and referfrom='Retailer'  and date(chrgDate)= date(date_sub(now(),interval 4 day))) or (date(referDate) = date(date_sub(now(),interval 4 day)) and userCircle='UPW' and referfrom='Retailer')");
