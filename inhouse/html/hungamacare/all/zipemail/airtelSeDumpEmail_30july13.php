...
$prevdate = date("Y-m-d", time() - 60 * 60 * 24);
	$sql_getreferdata = mysql_query("select ANI,friendANI,referDate,endDate,service_id,referfrom,userCircle,status,chrgAmount,optServiceId from master_db.tbl_refer_ussdData where date(referDate)='".$prevdate."' and userCircle='UPW'",$con);
	$totalnoofrecords= mysql_num_rows($sql_getreferdata);
	$getdatafile='Retailer ANI'."#".'Refer ANI'."#".'Refer Date'."#".'EndDate'."#".'Service Id'."#".'Refer From'."#".'Circle'."#".'Status'."#".'Charge Amount'."#".'OptServiceId'."\r\n";
//error_log($getdatafile,3,$path);
$fp=fopen($path,'a+');
fwrite($fp,'Retailer ANI'.','.'Refer ANI'.','.'Refer Date'.','.'EndDate'.','.'Service Id'.','.'Refer From'.','.'Circle'.','.'Status'.','.'Charge Amount'.','.'OptServiceId'."\r\n");
...
//error_log($getdatafile,3,$path);	
		fwrite($fp,$result_data['ANI'].','.$result_data['friendANI'].','.$result_data['referDate'].','.$result_data['endDate'].','.$result_data['service_id'].','.$result_data['referfrom'].','.$result_data['userCircle'].','.$result_data['status'].','.$result_data['chrgAmount'].','.$result_data['optServiceId']."\r\n");
				}
mysql_close($con);
