...
echo $DeleteQuery="delete from misdata.livemis where date(date)='".$view_date1."' and service IN ('".implode("','",$service_array)."')";
...
$get_activation_query1 .= " nolock where date(response_time)='".$view_date1."' and response_time<'".$DateFormat[0]."'  and service_id in(1402,1403,1410,1409,1416,1408,1418,1409) and plan_id!=95 and event_type in('RESUB','TOPUP') group by circle,service_id,chrg_amount,event_type,plan_id,hour(response_time)"; 

$fileDumpfile="resubtopup_".date('ymd').'txt';
$refileDumpPath1=$fileDumpPath.$fileDumpfile;

$query1 = mysql_query($get_activation_query1, $dbConn) or die(mysql_error());
$numRows1 = mysql_num_rows($query1);
if ($numRows1 > 0)
{
	unlink($refileDumpPath1);
	while(list($count,$circle,$charging_amt,$service_id,$event_type,$plan_id,$hour,$DateFormat1) = mysql_fetch_array($query1))
	{
		...
		$ResubTopData=$DateFormat1."|".$service_name."|".$circle_info[strtoupper($circle)]."|".$charging_str1."|".$count."|".$revenue."\r\n";
		error_log($ResubTopData,3,$refileDumpPath1) ;
	}
	$insertDump7= 'LOAD DATA LOCAL INFILE "'.$refileDumpPath1.'" INTO TABLE misdata.livemis FIELDS TERMINATED BY "|" LINES TERMINATED BY "\n" 				(Date,Service,Circle,Type,Value,Revenue)';
	$queryIns = mysql_query($insertDump7,$LivdbConn);
}
