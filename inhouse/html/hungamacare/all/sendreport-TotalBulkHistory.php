...
//MTS
$get_email_mts="select email from master_db.live_user_master where access_service like '%mts%' and ac_flag=1 and access_sec like '%revenue%' and alert_billing=1";
$email_list_mts = mysql_query($get_email_mts,$dbConn_224) or die(mysql_error());
while($row_email = mysql_fetch_array($email_list_mts))
{
if($row_email['email']!='anuj.bajpai@hungama.com' and $row_email['email']!='albert.almeida@hungama.com')
{
$emailMTSArray[]= $row_email['email'];
}
}
/*
//Airtel
$get_email_airtel="select email from master_db.live_user_master where access_service like '%airtel%' and ac_flag=1 and access_sec like '%revenue%' and alert_billing=1";
$email_list_airtel = mysql_query($get_email_airtel,$dbConn_224) or die(mysql_error());

while($row_email = mysql_fetch_array($email_list_airtel))
{
if($row_email['email']!='anuj.bajpai@hungama.com' and $row_email['email']!='albert.almeida@hungama.com')
{
$emailAirtelArray[]= $row_email['email'];
}
}
//Tata
...
//Reliance
$get_email_reliance="select email from master_db.live_user_master where access_service like '%Rel%' and ac_flag=1 and access_sec like '%revenue%' and alert_billing=1";
$email_list_reliance = mysql_query($get_email_reliance,$dbConn_224) or die(mysql_error());
while($row_email = mysql_fetch_array($email_list_reliance))
{
if($row_email['email']!='anuj.bajpai@hungama.com' and $row_email['email']!='albert.almeida@hungama.com')
{
$emailRelArray[]= $row_email['email'];
}
}
*/
...
$message = '<html><body>';
		    $message .= '<table rules="all" style="border-color: #666;font-size:12px;width:100%" border="0" cellpadding="2">';
			$message .= "<tr style='background: #eee;'><td><strong>BatchId</strong> </td><td><strong>Added On</strong></td><td><strong>Scheduled For</strong></td><td><strong>Uploaded For</strong></td><td><strong>Status</strong> </td><td><strong>Service Id</strong></td><td><strong>Total File Count</strong> </td><td><strong>Success Count</strong> </td><td><strong>Failure Count</strong> </td><td><strong>InRequest</strong></td><td><strong>Alert For</strong></td></tr>";
}			
if($operator=='airtel')
{
...
$getdatasql=mysql_query("select batch_id,added_on,status,service_id,total_file_count,success_count,failure_count,InRequest,upload_for,scheduledOn from billing_intermediate_db.bulk_upload_history where date(added_on) =date(now()) or date(added_on)=date(DATE_SUB(CONCAT(CURDATE(), ' 00:00:00'), INTERVAL 1 DAY)) order by id DESC",$con);
$noofrecords=mysql_num_rows($getdatasql);
mysql_close($con);
//close database connection here
while($result=mysql_fetch_array($getdatasql))
{
$upfor=$result['upload_for'];
if(empty($result[total_file_count]))
{
$TC='TC_0';
$style='background: #ADD8E6;color:#000000';
...
else if($result['upload_for']=='event_active')
{
$upfor='TopUp Charging';
$style='background: #ADD8E6;color:#000000';
}
else if(empty($result[success_count]) and  empty($result[failure_count]))
{
...
if($result['upload_for']=='event_active')
{
$upfor='TopUp Charging';
$style='background: #ADD8E6;color:#000000';
}
$message .= "<tr style='".$style."'><td><strong>".$result['batch_id']."</strong> </td><td>" .$result['added_on']."</td><td>".$result['scheduledOn']."</td><td>".$upfor."</td><td>".$result['status']."</td><td>".$result['service_id']."</td><td>".$TC."</td><td>".$SC."</td><td>".$FC."</td><td>".$IR."</td><td style='font-weight:bold'>MTS</td></tr>";
...
$getdatasql=mysql_query("select (CASE WHEN status=2 THEN 'PROCESSED' WHEN status=1 THEN 'IN_PROCESS'  ELSE 'NOT_PROCESSED' END) status,added_on ,batch_id ,file_name ,added_by ,service_id ,total_file_count ,block ,Already_subscribed ,in_process ,in_success ,in_failure ,gap_per,upload_for  from master_db.bulk_upload_history_triggered where service_id like '14%'  and date(added_on)>=date(subdate(now(),2))  order by added_on desc",$con);
$noofrecords=mysql_num_rows($getdatasql);
mysql_close($con);
//close database connection here
...
while($result=mysql_fetch_array($getdatasql))
{
if($result['gap_per']>10 and $result['in_process']>0)
{
...
$message .= "<tr style='".$style."'><td><strong>".$result['batch_id']."</strong> </td><td>" .$result['added_on']."</td><td>" .$result['added_by']."</td><td>".$result['upload_for']."</td><td>".$result['status']."</td><td>".$result['service_id']."</td><td>".$result['total_file_count']."</td><td>".$result['block']."</td><td>".$result['Already_subscribed']."</td><td>".$result['in_process']."</td><td>".$result['in_success']."</td><td>".$result['in_failure']."</td><td>".$result['gap_per']."</td><td style='font-weight:bold'>Uninor</td></tr>";
}
...
{
$con = mysql_connect("192.168.100.224","webcc","webcc");
$getdatasql=mysql_query("select (CASE WHEN status=2 THEN 'PROCESSED' WHEN status=1 THEN 'IN_PROCESS'  ELSE 'NOT_PROCESSED' END) status,added_on ,batch_id ,file_name ,added_by ,service_id ,total_file_count ,block ,Already_subscribed ,in_process ,in_success ,in_failure ,gap_per ,upload_for from master_db.bulk_upload_history_triggered  where service_id like '10%' or service_id like '18%' or service_id like '16%' order by added_on desc",$con);
$noofrecords=mysql_num_rows($getdatasql);
mysql_close($con);
//close database connection here
while($result=mysql_fetch_array($getdatasql))
...
$message .= "<tr style='".$style."'><td><strong>".$result['batch_id']."</strong> </td><td>" .$result['added_on']."</td><td>" .$result['added_by']."</td><td>".$result['upload_for']."</td><td>".$result['status']."</td><td>".$result['service_id']."</td><td>".$result['total_file_count']."</td><td>".$result['block']."</td><td>".$result['Already_subscribed']."</td><td>".$result['in_process']."</td><td>".$result['in_success']."</td><td>".$result['in_failure']."</td><td>".$result['gap_per']."</td><td style='font-weight:bold'>Tata</td></tr>";
}
}
*/
else if($operator=='Uninor')
...
*/
$getdatasql=mysql_query("select batch_id,added_on,added_by,status,service_id,total_file_count,success_count,failure_count,InRequest,upload_for,blocked ,Already_subscribed from master_db.bulk_upload_history where (date(added_on) =date(now()) or date(added_on)=date(DATE_SUB(CONCAT(CURDATE(), ' 00:00:00'), INTERVAL 1 DAY))) and service_id like '14%' and upload_for in('active','deactive','topup','event_active','doubleConsentUninor')
union
select batch_id,date_time as added_on,username as added_by,status,serviceID as service_id,total_count as total_file_count,success_count,failure_count,InRequest,'content_charging','0' ,'0'
...
$logdata='Operator'."#".$operator.'#Time'."#".date("H:i:s")."\r\n";
	...
	mail('manu.sharma@hungama.com', $subject, $message, $headers);
			...
			{
			$logdata="*************Email Send Successfully For Airtel Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			else
			{
			$logdata="*************Email Send Error For Airtel Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			error_log($logdata,3,$path_log);
	    }
	 ...
	 mail('manu.sharma@hungama.com', $subject, $message, $headers);
			...
			if(mail($cc_dev, $subject, $message, $headers))
			{
			$logdata="*************Email Send Successfully For Voda Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			else
			{
			$logdata="*************Email Send Error For Voda Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			error_log($logdata,3,$path_log);
	 }
	 else if($operator=='MTS')
	 {
		...
		mail('dheeraj.goel@hungama.com', $subject, $message, $headers);
		mail('rajneesh.srivastava@hungama.com', $subject, $message, $headers);
		mail('gagandeep.singh@hungama.com', $subject, $message, $headers);
	 		  foreach($emailArray as $email)
				{
				mail($email, $subject, $message, $headers);
				}
			...
			if(mail($cc_dev, $subject, $message, $headers))
			{
			$logdata="*************Email Send Successfully For MTS Service*************"."#".$noofrecords."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			else
			{
			$logdata="*************Email Send Error For MTS Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			error_log($logdata,3,$path_log);
	 }
	...
	else if($operator=='Uninor')
	 {
		 foreach($emailArray as $email)
			...
			if(mail($cc_dev, $subject, $message, $headers))
			{
			$logdata="*************Email Send Successfully For Uninor Service*************"."#".$noofrecords."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			else
			{
			$logdata="*************Email Send Error For Uninor Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			error_log($logdata,3,$path_log);
	    }
	 else if($operator=='TATA')
	 {
		  ...
		  //  mail($to, $subject, $message, $headers);	
			...
			//mail($cc_sd, $subject, $message, $headers);
				if(mail($cc_dev, $subject, $message, $headers))
			{
			$logdata="*************Email Send Successfully For Tata Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			else
			{
 ...
 else if($operator=='Reliance')
	 {
			...
			if(mail($cc_dev, $subject, $message, $headers))
			{
			$logdata="*************Email Send Successfully For Reliance Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
			else
			{
			$logdata="*************Email Send Error For Reliance Service*************"."#".date("H:i:s")."\r\n";;
			error_log($logdata,3,$path_log);
			}
