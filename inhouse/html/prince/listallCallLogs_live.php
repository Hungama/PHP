...
$daterange="CALLDATE = '$ToDate'";
	...
	//	$sql_getmsisdnlist = mysql_query("select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,SONGID $getcount from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  (CALLDATE) = '$displaydate' $sqlwhere order by CALLTIME desc");
//echo "select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,SONGID $getcount from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  $daterange $sqlwhere order by CALLTIME desc";
		$sql_getmsisdnlist = mysql_query("select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,DNIS,SONGID $getcount from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  $daterange $sqlwhere order by CALLTIME desc");
	}
	else
	{
	$sql_getmsisdnlist = mysql_query("select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,DNIS,SONGID from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  date(CALLDATE) = '$today' order by CALLTIME desc");	
	$displaydate=$today;
 }
...
<div id="main">
	...
	<div id="middle" >
		...
		<div id="center-column1" style="width:650px">
			...
			<option value="unique" <?php if($stype=='unique') { echo "selected";}?>><?php echo 'Unique Caller';?></option>
			<option value="repeat" <?php if($stype=='repeat') { echo "selected";}?>><?php echo 'Repeat Caller';?></option>
			</select>
				</td>
...
<tr>
					...
					<td colspan="7"><?php echo $status['CallMinuteUsed'];?></td>
