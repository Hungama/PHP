...
include("db.php");
    ...
    $sql_getmsisdnlist = mysql_query("select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,DNIS,SMOKERSTATUS,CALLDROPPOINT,DISCONNECT_REASON,SONGID $getcount from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  $daterange $sqlwhere order by CALLTIME desc");
    } else {
        $sql_getmsisdnlist = mysql_query("select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,DNIS,SMOKERSTATUS,CALLDROPPOINT,DISCONNECT_REASON,SONGID from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  date(CALLDATE) = '$today' order by CALLTIME desc");
  ...
  width: 15%
        ...
        <body>
            ...
            <div id="main">
                ...
                <div id="middle" >
                    <!--div id="left-column"-->
    <?php include('new_side_bar.php'); ?>	
                    <!--/div-->
					
                    <div id="center-column1" style="width:850px">
                        <div class="top-bar">
                            <h1>Call Log IVR</h1>
                        </div>
    ...
    <?php } ?>
        ...
        echo "selected";
                                        ...
                                        <td colspan="14"><strong>Select Date &nbsp;&nbsp;&nbsp;</strong>
...
//if(0)
        ?>
                                ...
                                <tr>
                                    ...
                                    <th colspan="12">Call Sec Used</th>
                                ...
                                <tr>
                                    ...
                                    <td colspan="12"><?php echo $status['CallMinuteUsed']; ?></td>
    ...
    if ($totalrecord > 0) {
        ?>
                                    <tr><th colspan="15">Total no of <?= $totalrecord; ?> records found of date <?= $displaydate; ?>.</th></tr>
                                    <tr>
										...
										<?php //if($_SESSION["id"]=='393') {?>
										<th><strong>DISCONNECT_REASON</strong></th>
										<th><strong>CALLDROPPOINT</strong></th>
										<?php //}?>
		...
		/******************start for repeat/unique user*************************/
		 while ($result_list = mysql_fetch_array($sql_getmsisdnlist)) {
			...
			$sql_getcalllogs = mysql_query("select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,DNIS,SMOKERSTATUS,CALLDROPPOINT,DISCONNECT_REASON,SONGID  from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  $daterange and APARTY='".$result_list['APARTY']."' order by CALLTIME desc");
		while ($result_list = mysql_fetch_array($sql_getcalllogs)) 
			...
			?>
                                            ...
                                            <td><?= $i; ?></td>
                                                ...
                                                <td><?php if (!empty($result_list['CIRCLE'])) {echo $circle_info[strtoupper($result_list['CIRCLE'])];} else { echo "--";} ?></td>
												...
												<td><?php if (!empty($result_list['SMOKERSTATUS'])) { echo $result_list['SMOKERSTATUS'];} else {echo "--";} ?></td>
												<td><?php if (!empty($result_list['LEGTYPE'])) { echo $result_list['LEGTYPE'];} else {echo "--";} ?></td>
												...
												<?php //if($_SESSION["id"]=='393') {?>
												<td><?php if (!empty($result_list['CALLDROPPOINT'])) { echo $result_list['CALLDROPPOINT'];} else { echo "--";} ?></td>
												<td><?php if (!empty($result_list['DISCONNECT_REASON'])) { echo $result_list['DISCONNECT_REASON'];} else { echo "--";} ?></td>
												<?php //} ?>
												<td><?php if (!empty($result_list['DNIS'])) {echo $dnismapped[$result_list['DNIS']];} else {echo "--";} ?></td>
                                            </tr>
        ...
        ?>
                                    <tr><th colspan="12">No records found.</th></tr>
        <?php
    }
    ?></table>
                            <p>&nbsp;</p>
