...
if($_POST['action'] == 1) {
	...
	//echo "select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,SONGID $getcount from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  $daterange $sqlwhere order by CALLTIME desc";
		$sql_getmsisdnlist = mysql_query("select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,SONGID $getcount from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  $daterange $sqlwhere order by CALLTIME desc");
	}
	else
	{
	$sql_getmsisdnlist = mysql_query("select APARTY,CALLDATE,CALLTIME,CIRCLE,CALLDURATION,RECORDEDSTATUS,RECIEVERID,CFCOMPLETE,LEGTYPE,SONGID from Hungama_PRINCEIVR.tbl_pinceivr_details  WHERE  date(CALLDATE) = '$today' order by CALLTIME desc");	
	$displaydate=$today;
 }
...
<div id="main">
	...
	<div id="middle" >
		...
		<table class="listing" cellpadding="0" cellspacing="0" style="width:700px" >
				<form action="listallCallLogs_demo.php" method="post">
				<tr class="bg">
				<td>
...
//if(0)
{?>
				<a href="xls_alldata.php?sdate=<?=$displaydate?>&stype=<?=$stype?>" title="Click to download file.">
