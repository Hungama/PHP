...
$checkfiletoprocess=mysql_query("select batchid,odb_filename,circle, startdate, enddate from tbl_obdrecording_log where status IN ( 1, 0) and prcocess_status='free' order by batchid ASC limit 1");
...
foreach ($allani as $allani_no => $msisdn)
 {
//$msisdn;
fwrite($file,$msisdn . "#" . $obd_form_startdate . "#" . $obd_form_enddate . "#" . $status . "#" . $obd_form_circle."\r\n" );
 }
fclose($file);
...
$sql_obdinfo= 'LOAD DATA LOCAL INFILE "'.$obd_anifile_path.'" 
        ...
        LINES TERMINATED BY "\n" 
        (ANI,StartDate,EndDate,Status,Circle)';
