...
$getMsisdnId="select id from master_db.tbl_recharged nolock where status=0 order by id ASC limit 50";
...
//error_log($rechargeStatus,3,$reponseLog);	
	...
	if($mobikiwi_status=='SUCCESS')
		...
		$status='FAILURE';
	else if($mobikiwi_status=='SUCCESSPENDING')
		$status='Success';
	else
		$status='FAILURE';
	...
	$CallBackResponse = curl_exec($ch);
			...
			//send email alert if Response not SUCCESS		
			$subject = 'Recharge Notification Issue';
			$message2="Hi Team, Unable to connect 192.168.100.218. Response- .".$CallBackResponse."<br>";
			$message2.=$callBackUrl;
			
			$email_url="http://192.168.100.212/hungamacare/all/sendEmailAlert.php?stype=RechargeFail&msg=".urlencode($message2);
			if($CallBackResponse!='Success')
			{
			$response_email = file_get_contents($email_url);
			}

	$message .=$responseTime."|".$msisdn."|".$operator_id."|".$circle_id."|".$amount."|".trim($apiresponse)."|".$callBackUrl."|".$CallBackResponse."|".$conntype."|".$mobikiwi_status."\r\n";	
	echo $message;
