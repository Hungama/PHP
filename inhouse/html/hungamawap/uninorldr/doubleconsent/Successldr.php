...
$logFile="logs/sdpResponseLdr_".date('Ymd');
$logPath=$logFile.".txt";
...
$saveLiveMisWAPLogs = "http://192.168.100.212/kmis/services/hungamacare/2.0/wap/saveLiveWAPlogs.php?zone_id=".$zoneid."&msisdn=".$msisdn."&msg=".urlencode($RESULT)."&afid=".$afid."&circle=".$circle."&service=WAPUninorLDR1&type=ccgresponse";
...
//home page or sucess page or thanks
        if($REQ_TYPE=='TOPUP')	
		{
		$redirectUrl = "http://u.ldr.mobi/site/topupcallback?mobile_no=" . $MSISDN1 . "&content_id=" . $content_id;
		}
		else if($REQ_TYPE=='Subcription')
		 {
		 $redirectUrl = "http://u.ldr.mobi/site/usersubscription_callback?mobile_no=" . $MSISDN1 . "&content_id=" . $content_id."&afid=".$afid;
		 }	
		//save logs for MIS 
