...
if($msisdn)
{
	if($reqtype=='RiyaWap' || $reqtype=='RiyaWap10Event')
	{
		if($reqtype=='RiyaWap10Event')
			$chargingUrl1="http://192.168.100.211/billing/Docomo/docomoCharging.php?msisdn=".$msisdn."&moh=WAP&rt=event&scode=5464626t10&online=y";
		else
			$chargingUrl1="http://192.168.100.211/billing/Docomo/docomoCharging.php?msisdn=".$msisdn."&moh=WAP&rt=sub&scode=546462630&online=y";
		...
		$max=count($response1);
		$data = $response1[$max-1];
	
		if($pos = strrpos(strtolower($data),"success")) 
		{
			$allData = explode("#",$data);
			$insertQuery = "insert into mis_db.tbl_wapRequest_data (msisdn, mode, datetime,response,status,operator,circle,service,chrg_amount) VALUES ('".$msisdn."' ,'WAP',NOW(),'".$response."','success','TATM','".$circle."','1009','".$allData[3]."')";
		}
		 else 
		{
			$insertQuery = "insert into mis_db.tbl_wapRequest_data (msisdn, mode, datetime,response,status,operator,circle,service,chrg_amount) VALUES ('".$msisdn."' ,'WAP',NOW(),'".$response."','failure','TATM','".$circle."','1009','0')";
		}
