...
$logFile_Wapdump="ccgWapdump_".date('Ymd');
$logPath=$logDir.$logFile_dump.".txt";
$logPath_WAPDUMP=$logDir.$logFile_Wapdump.".txt";
$curdate = date("Ymd");
...
switch($productId)
		...
		case 'GSMPKP':
				...
				$sc='55001';
				$s_id='1013';
				$subscriptionProcedure="MND_SUB_WAP";
				$planid=106;
				$lang='01';
				$subscriptionProcedure_ccg="MND_DC_SUBREQUEST";
				break;
		...
		case 'GSMDEIVA30':
				$dbname="docomo_hungama";				
				$subscriptionProcedure_ccg="DEV_DC_SUBREQUEST";
				break;	
		case 'GSMMissriacontent10':
				//$dbname="Hungama_RaginiMMS";
				$dbname="Celebrity_ChatRoom";
		...
		error_log($logData,3,$logPath2);
			...
			if($mode=='WAP')
			{
			//get affid & update ccg id Hungama_WAP_Logging.tbl_wap_logs 
			
			if($CGPResult=='SUCCESS' && $TPCGID!='')
			{
				$getAffID="select afid,zoneid from Hungama_WAP_Logging.tbl_wap_logs nolock where service='".$productId."' and msisdn='".$msisdn."' and trnxid='".$transID."' and operator='TATM'";
				$result=mysql_query($getAffID,$con);
				list($responseAfid,$responseZoneid)=mysql_fetch_array($result);
				//$responseZoneid=0;//zoneid
				$sqlupdate="update Hungama_WAP_Logging.tbl_wap_logs set ccgid='".$TPCGID."',status=2,response='SUCCESS' where service='".$productId."' and msisdn='".$msisdn."' and trnxid='".$transID."' and operator='TATM'";
				mysql_query($sqlupdate,$con);
			}
			
			
			$amt = mysql_query("select iAmount from master_db.tbl_plan_bank nolock where Plan_id='$planid'",$con);
			List($row1) = mysql_fetch_row($amt);
			$amount = $row1;
			$call="call ".$dbname.".".$subscriptionProcedure."('$msisdn','$lang','$mode','$sc','$amount',$s_id,$planid,'$TPCGID','$responseAfid','$responseZoneid')";
			if($CGPResult=='SUCCESS' && $TPCGID!='')
				{
				//stopped all wap request because of some issue
			
			$logData_Park=$responseAfid."#query-WAPSuccess#".$call."#Success#".date("Y-m-d H:i:s")."\n";
			if(mysql_query($call, $con))
			{
			$logData=$responseAfid."#query-WAP#".$call."#Success#".date("Y-m-d H:i:s")."\n";
			}
			else
			{
			$error= mysql_error();
			$logData=$responseAfid."#query-WAP#".$call."#Failure#Error-".$error."#".date("Y-m-d H:i:s")."\n";
			}
			}		
			else
			{
			$logData_Park="#query-WAPWitoutCCG#".$call."#Success#".date("Y-m-d H:i:s")."\n";

			$logData="#query-WAP_notcall#".$call."#".$CGPResult."#".date("Y-m-d H:i:s")."\n";
			}
			if($_REQUEST['test']==1)
			echo $logData_Park;
			error_log($logData_Park,3,$logPath_WAPDUMP);
			error_log($logData,3,$logPath2);
			echo 'SUCCESS';
			exit;
			}
		...
		sleep(3);
		if($productId=='GSMMissriacontent10')
		...
		{
		$error= mysql_error();
		$logData="#query#".$call."#Failure#Error-".$error."#".date("Y-m-d H:i:s")."\n";
		}
		}
		else
		{
