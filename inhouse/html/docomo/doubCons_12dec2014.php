...
switch($productId)
		...
		error_log($logData,3,$logPath2);
			...
			$saveAfIDdata="http://192.168.100.212/kmis/services/hungamacare/2.0/wap/updateWapAffiddata.php";
			$saveAfIDdata.="?msisdn=$msisdn&transid=$transID&ccgid=$TPCGID&product=$productId&op=TATM";
			$afiddata_result=curl_init($saveAfIDdata);
			curl_setopt($afiddata_result,CURLOPT_RETURNTRANSFER,TRUE);
			$responseAfid= curl_exec($afiddata_result);
			curl_close($afiddata_result);

			
			$amt = mysql_query("select iAmount from master_db.tbl_plan_bank where Plan_id='$planid'",$con);
			List($row1) = mysql_fetch_row($amt);
			$amount = $row1;
	$call="call ".$dbname.".".$subscriptionProcedure."('$msisdn','$lang','$mode','$sc','$amount',$s_id,$planid,'$TPCGID','$responseAfid')";
