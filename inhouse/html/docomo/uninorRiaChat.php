...
if (!is_numeric("$planid")) {
    echo $abc[1];
    $log_file_path = "logs/uninor/subscription/" . $servicename . "/chatEvent_" . $curdate . ".txt";
    ...
    if (strlen($msisdn) == 12 || strlen($msisdn) == 10) {
        if (strlen($msisdn) == 12) {
            if (substr($msisdn, 0, 2) == 91) {
                ...
                if ($reqtype == 2) {
                    $log_file_path = "logs/uninor/unsubscription/" . $servicename . "/chatEvent_" . $curdate . ".txt";
                    $file = fopen($log_file_path, "a");
                    fwrite($file, $msisdn . "#" . $mode . "#" . $reqtype . "#" . $planid . "#" . $lang . "#" . $subchannel . "#" . date('H:i:s') . "#" . $abc[1] . "\r\n");
                    fclose($file);
                }
...
if (($msisdn == "") || ($mode == "") || ($reqtype == "") || ($planid == "")) {
    ...
    switch ($servicename) {
        case 'uninor_manchala':
            $sc = '5646428';
            $s_id = '1409';
            $dbname = "uninor_manchala";
            $subscriptionTable = "tbl_riya_celebchat_subscription";
            $subscriptionProcedure = "RIYA_CHAT_SUB";
    ...
    if ($reqtype==4) {
        ...
        echo $sub = "select count(*) from " . $dbname . "." . $subscriptionTable . " where ANI='$msisdn'";
		$qry1 = mysql_query($sub);
        $rows1 = mysql_fetch_row($qry1);
        if ($rows1[0] <= 0) {
            echo $call = "call " . $dbname . "." . $subscriptionProcedure . "($msisdn,'$lang','$mode','$sc','$amount',$s_id,$planid)";
