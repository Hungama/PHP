...
include("dbConnect.php");
	...
	$select_query="select file_name, added_on, service_type, channel, price_point, upload_for,batch_id,service_id from vodafone_hungama.bulk_upload_history where status=0 order by added_on desc limit 1";
	...
	while(list($file_name, $datetime, $service_type, $channel, $price, $upload_for,$batch_id,$service_id) = mysql_fetch_array($query)) 
	{
		...
		$file_to_read="http://203.199.126.129/hungamacare/bulkuploads/".$service_id."/".$file_name.".txt";
		...
		for($i=0;$i<$file_size;$i++)
		{
			...
			if(strlen(trim($msisdn))==10)
			{
				/*if($reqtype==1)
				{
					...
					$callProcedure1="call master_db.BULK_ACTIVATION($msisdn,99,'$channel',54646,$amount,$service_id,$price)";
					$callProcedure= mysql_query($callProcedure1);
				}
				else
				{*/
					$billing_url = "http://203.199.126.129/vodafone/vodafone.php?msisdn=".trim($msisdn);
					...
					echo $logData = $msisdn."#".$billing_url."#".$url_response."#".date('Y-m-d H:i:s')."\n";
