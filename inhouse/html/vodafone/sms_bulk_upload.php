...
<?php 
	 ...
	 if($_SERVER['REQUEST_METHOD']=="POST" && isset($_FILES['upfile']) && !empty($_FILES['upfile']['name'])) 
	{
		...
		if(isAllowedExtension($file['name'])) {
			...
			$file_to_read="http://192.168.100.212/hungamacare/smsbulkuploads/".$_POST['service_info']."/".$makFileName;
			...
			$UploadMsgQuery="insert into master_db.tbl_message_data(message, added_on, service_id) values('".$message."', '".$currTime."', '".$_POST[service_info]."')";
			...
			$Uploadquery="insert into master_db.bulk_message_history(batch_id,file_name,msg_id,added_by,added_on,upload_for,status,operator,total_file_count, service_id, ip) values('".$batchId."', '".$dbMakFileName."', '".$msg_id."', '".$_SESSION[loginId]."', '".$currTime."','active',0,'".$_SESSION[dbaccess]."','".$file_size."','".$_POST[service_info]."','".$remoteAdd."')";
			$queryIns = mysql_query($Uploadquery);
			
			/*for($i=0;$i<$file_size;$i++)
			{
				$number = trim($file_data[$i]);
				$insert_query="INSERT INTO master_db.bulk_msgupload_data (batch_id, msg_id, msisdn, service_id) VALUES ('".$batchId."', '".$msg_id."', '".$number."', '".$_POST[service_info]."')";
...
<TABLE width="80%" align="center" border="0" cellpadding="0" cellspacing="0" class="txt">
      ...
      <TR height="30">
        <TD bgcolor="#FFFFFF"><a href='selectservice.php'>
			...
			<!--|| <B><a href="smsviewhistory.php" onclick=\"openWindow()\" class=\"blue\">
