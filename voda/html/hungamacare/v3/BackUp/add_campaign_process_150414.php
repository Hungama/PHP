...
foreach ($circle as $key => $circleValue) {
    $flag = 1;
    $selectQuery = "SELECT starttime,endtime FROM Vodafone_IVR.tbl_ad_campaign WHERE S_id='" . $service . "' and circle = '" . $circleValue . "' and sc='" . $sc . "'";
    ...
    if ($flag == 1) {
        $insertquery = "insert into Vodafone_IVR.tbl_ad_campaign(S_id,ad_name,circle,sc,isSkip,added_on,added_by,status,operator,starttime,endtime,ipaddress) 
     values('$service', '$add_name','$circleValue', '$sc', '$is_skip',now(),'$loginid','1','VODAM','$timestamp','$timestamp1','$remoteAdd')";
