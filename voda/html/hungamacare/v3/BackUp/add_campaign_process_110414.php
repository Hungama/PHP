...
foreach ($circle as $key => $circleValue) {
    $flag = 1;
    $selectQuery = "SELECT starttime,endtime FROM Inhouse_IVR.tbl_ad_campaign WHERE S_id='" . $service . "' and circle = '" . $circleValue . "' and sc='" . $sc . "'";
    ...
    while ($details = mysql_fetch_array($querySel)) {
     ...
     if ($flag == 1) {
        $insertquery = "insert into Inhouse_IVR.tbl_ad_campaign(S_id,ad_name,circle,sc,isSkip,added_on,added_by,status,operator,starttime,endtime,ipaddress) 
