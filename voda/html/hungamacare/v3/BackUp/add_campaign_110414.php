...
if ($sel_opt == 'unim') {
    $serviceArray1 =  array('1402' => 'Uninor54646', '1408' => 'UninorSU', '1423' => 'UninorContest', '1409' => 'RIAUninor',
        '1416' => 'UninorAstro', '1410' => 'REDFMUninor', '1418' => 'UninorComedy', '1412' => 'UninorRT');
    $circle_info = array('DEL' => 'Delhi', 'WBL' => 'WestBengal', 'BIH' => 'Bihar', 'RAJ' => 'Rajasthan', 'MAH' => 'Maharashtra', 'APD' => 'Andhra Pradesh', 'UPE' => 'UP EAST', 'ASM' => 'Assam', 'TNU' => 'Tamil Nadu', 'KOL' => 'Kolkata', 'NES' => 'NE', 'CHN' => 'Chennai', 'ORI' => 'Orissa', 'KAR' => 'Karnataka', 'MUM' => 'Mumbai', 'JNK' => 'Jammu-Kashmir', 'PUB' => 'Punjab', 'HPD' => 'Himachal Pradesh');
} else {
    $serviceArray1 = array('1002' => 'TataDoCoMo54646', '1602' => 'TataIndicom54646', '1003' => 'MTVTataDoCoMo', '1603' => 'MTVTataIndicom',
        '1010' => 'REDFMTataDoCoMo', '1610' => 'REDFMTataDoCoMocdma', '1009' => 'RIATataDoCoMo', '1609' => 'RIATataDoCoMocdma', '1005' => 'TataDoCoMoFMJ'
        , '1605' => 'TataDoCoMoFMJcdma', '1013' => 'TataDoCoMoMND', '1613' => 'TataDoCoMoMNDcdma', '1001' => 'TataDoCoMoMX', '1601' => 'TataDoCoMoMXcdma');
                                ...
                                <div id="active" class="tab-pane active">
                                    ...
                                    <form id="tstest" name="tstest" method="post" enctype="multipart/form-data">
                                        <table class="table table-bordered table-condensed">
                                            <tr>
                                                <td align="left" width="16%" height="32"><span>Operator Name</span></td>
                                                <td align="left" colspan="2">
                                                    <select name="opeartor" data-width="30%" onchange="setService(this.value);">
                                                        <?php $select_str = 'selected="selected"';?>
                                                        <option value="0">Select Operator</option>
                                                        <option value="unim" <?php if($sel_opt == 'unim'){ echo $select_str;}?>>Uninor</option>
                                                        <option value="tatm" <?php if($sel_opt == 'tatm'){ echo $select_str;}?>>Tata</option>
                                                    </select>
                                                </td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                <td align="left" width="16%" height="32"><span>Sort Code</span></td>
                                                <td align="left" colspan="2">
                                                    <?php
                                                     $selectQuery = "SELECT SC FROM master_db.tbl_sc_base WHERE status=1 and service_id = " . $sel_service . " ";
                                                      ...
                                                      ?>
                                                    <select name="sc" data-width="30%" onchange="setScenarios(this.value)">
...
<!--                                                        &nbsp;<a href="javascript:toggle('circle')" id="all-circle">[All]</a><a href="javascript:toggle('circle')" id="none-circle" style="display:none">[X]</a>-->
                                                ...
                                                <tr>
                                                    ...
                                                    <td align="left" colspan="2">
                                                        <?php
                                                     $selectQuery = "SELECT ad_name FROM Inhouse_IVR.tbl_ad_nameMap WHERE status=1";
                                                        $querySel = mysql_query($selectQuery, $dbConn);
                                                      ?>
                                                ...
                                                <tr>
                                                    <td align="left" width="16%" height="32">Do You Want to Skip</td>
                                                    <td>
                                                        <label class="radio">
            ...
            <script>
                    ...
                    if(isok)
                    {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                bootstrap_alert.warning('<?php echo 'Campaign has been added successfully'; ?>');
                                resestForm('tstest');    
                                viewUploadhistory();
                            },
                            cache: false,
                ...
                function checkfield(type) { 
                    var opeartor = document.forms["tstest"]["opeartor"].value;//alert(opeartor);
                    var service = document.forms["tstest"]["service"].value;//alert(service);
                    var sc = document.forms["tstest"]["sc"].value;//alert(sc);
                    var timestamp = document.forms["tstest"]["timestamp"].value;//alert(timestamp);
                    var timestamp1 = document.forms["tstest"]["timestamp1"].value;//alert(timestamp1);
                    var add_name = document.forms["tstest"]["add_name"].value;//alert(add_name);
                    
                    $('#loading').hide();
                    showMessageBox();
                    if (opeartor=='0' || opeartor== '') {
                        bootstrap_alert.warning('<?php echo 'Please select operator' ?>');
                        return false;
            ...
            function setService(operator) { 
                window.location.href='add_campaign.php?sel_opt='+operator;
            }
            function setSC(service) { 
                var operator=document.forms["tstest"]["opeartor"].value;
                window.location.href='add_campaign.php?sel_opt='+operator+'&sel_service='+service;
            }
        </script>
    </body>
</html>