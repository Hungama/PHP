...
exit;
	 ...
	 $file = $_FILES['upfile'];
		...
		if(move_uploaded_file($_FILES['upfile']['tmp_name'], $path)) {
					...
					//$message = msgStrEncode($message);
	$Uploadquery="insert into master_db.bulk_tdbsms_history(batch_id, file_name, message,added_by, added_on, upload_for,status,total_file_count,ip) values('$batchId', '$dbmakFileName', '".mysql_real_escape_string($message)."','$added_by', '$thisTime', '$_POST[upfor]',0,'$fileCount','$remoteAdd')";
