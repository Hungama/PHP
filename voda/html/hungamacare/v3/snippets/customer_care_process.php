    ...
    <div id="result-table">
        ...
        <TABLE width="85%" align="center" bgcolor="#0369b3" border="0" cellpadding="1" cellspacing="1" class="table table-condensed table-bordered" >
            ...
            for ($i = 0; $i < $no_of_servicename; $i++) {
                ...
                $select_query2_main = "( select msisdn, date_time, chrg_amount,circle,plan_id,SC from master_db.tbl_billing_success nolock where msisdn='$msisdn' and event_type !='Recharged' and ";
                $select_query2_main .=" plan_id IN (" . implode(",", $planData) . ") and ";
                $select_query2_main .= " service_id=" . $serviceId . " )";

                $select_query3_bak = "(select msisdn, date_time, chrg_amount,circle,plan_id,SC from master_db.tbl_billing_success_04_06_2013 nolock where msisdn='$msisdn' and event_type !='Recharged' and";
                $select_query3_bak .=" plan_id IN (" . implode(",", $planData) . ") and ";
                $select_query3_bak .= " service_id=" . $serviceId . " )";

                $select_query2_bak = "(select msisdn, date_time, chrg_amount,circle,plan_id,SC from master_db.tbl_billing_success_backup nolock where msisdn='$msisdn' and event_type !='Recharged' and";
...
//                    if ($SC == '5464646' || $SC == '5464628' || $service_info == '1402') {
                    if ($service_info == '1402' || $service_info == '1202' || $service_info == '1002') {
                        //$select_query_tkt = mysql_query("select SUB_DATE,SUB_TYPE as TicketStatus1, RENEW_DATE, circle, MODE_OF_SUB, DNIS, STATUS, DATEDIFF(RENEW_DATE,NOW()) as diff,chrg_amount from Hungama_RaginiMMS.tbl_celebchat_subscription nolock where ANI='" . $msisdn . "'", $dbConn);
                        $select_query_tkt_opeartor = "select SUB_DATE,SUB_TYPE as TicketStatus1, RENEW_DATE, circle, MODE_OF_SUB, DNIS, STATUS, DATEDIFF(RENEW_DATE,NOW()) as diff,chrg_amount from Hungama_RaginiMMS.tbl_celebchat_subscription nolock where ANI='" . $msisdn . "'";
                        if ($service_info == '1402') {
                            $select_query_tkt_opeartor .=" and operator='UNIM'";
                        } elseif ($service_info == '1202') {
                            $select_query_tkt_opeartor .=" and operator='REL'";
                        } elseif ($service_info == '1002') {
                            $select_query_tkt_opeartor .=" and operator='TATC'";
                        }

                        $select_query_tkt = mysql_query($select_query_tkt_opeartor, $dbConn);
                        list($SUB_DATE_TC, $TicketStatus1, $RENEW_DATE_TC, $circle1_TC, $MODE_OF_SUB_TC, $DNIS_TC, $subStatus_TC, $daysLeft_TC, $chrg_amount_TC) = mysql_fetch_array($select_query_tkt);
                    }
                            ...
                            <?php if ($subStatus == 0) {
                                ...
                                if (!empty($RENEW_DATE)) {
                                    ...
                                    echo '-';
                                        ...
                                        <TD><?php
                                if (!empty($date_time)) {
                                    echo date('j-M \'y g:i a', strtotime($date_time));
                                } else {
                                    echo '-';
                                }
                                            ?></TD>
                                ...
                                if (!empty($MODE_OF_SUB)) {
                                    ...
                                    echo '-';
                                        ...
                                        <!---Added for RAgniMMS Ticket Status Report start here-->
                                        <?php
                                        if ($TicketStatus1 != '') {
                                            ?>
                                            ...
                                            <TR height="30">
                                                ...
                                                <TD bgcolor="#FFFFFF" align="center">
                                                    ...
                                                    <?php if ($subStatus_TC == '0') {
                                                        ?>
                                                        <span class="label label-warning">
                                                            <?php
                                                            echo STATUS_0;
                                                        } else if ($subStatus_TC == '1') {
                                                            ?>
                                                            <span class="label label-success">
                                                                <?php
                                                                echo STATUS_1;
                                                            } else if ($subStatus_TC == '11') {
                                                                ?>
                                                                <span class="label label-info">
                                                                    <?php
                                                                    echo STATUS_11;
                                                                } else if ($subStatus_TC == '5') {
                                                                    ?>
                                                                    <span class="label label-info">
                                                                        <?php
                                                                        echo STATUS_5;
                                                    ...
                                                    if (!empty($Service_DESC[$sname_ks[$service_info]]['Name'])) {
                                                        ...
                                                        echo $sname_ks[$service_info];
                                                                ...
                                                                <TD><?php
                                                    if (!empty($SUB_DATE_TC)) {
                                                        echo date('j-M \'y g:i a', strtotime($SUB_DATE_TC));
                                                    } else {
                                                        echo '-';
                                                    }
                                                                    ?></TD>
                                                    ...
                                                    if (in_array($plan_id, $autochurnrenwplanid)) {
                                                        ...
                                                        if (!empty($RENEW_DATE_TC)) {
                                                            echo date('j-M \'y g:i a', strtotime($RENEW_DATE_TC));
                                                        } else {
                                                            echo '-';
                                                        }
                                                    }
                                                    ...
                                                    if (!empty($date_time)) {
                                                        ...
                                                        echo '-';
                                                                    ...
                                                                    ?></TD>
                                                                <TD><?php echo 'Rs. ' . $chrg_amount_TC; ?>&nbsp;</TD>
                                                                <TD><?php echo $circle_info[strtoupper($circle)]; ?>&nbsp;</TD>

                                                                <TD><?php
                                                    if (!empty($MODE_OF_SUB_TC)) {
                                                        echo $MODE_OF_SUB_TC;
                                                    } else {
                                                        echo '-';
                                                    }
                                                                    ?></TD>	
                                                                <TD><?php
//                                                    if ($SC == '5464646' || $SC == '54644628' || $service_info == 1402) {
                                                    if ($service_info == 1402 || $service_info == 1202 || $service_info == '1002') {
                                                        echo $TicketStatus1;
                                                    } else {
                                                        echo '-';
