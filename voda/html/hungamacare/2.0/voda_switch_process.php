...
if ($mode == 'circle') {
    if ($qryMsg == 1) {
        echo "<div width=\"85%\" align=\"left\" class=\"txt\">
				<div class=\"alert alert-danger\">Data inserted successfully.</div></div>";
    } else {
        echo "<div width=\"85%\" align=\"left\" class=\"txt\">
				<div class=\"alert alert-danger\">Data not inserted successfully.</div></div>";
    }
    exit();
}
...
<div class=\"alert alert-danger\">Please upload file of less than 50,000 numbers otherwise it will not process.</div></div>";
    ...
    $uploaddir = "../uninorswitch/" . $serviceid . "/";
    if (!is_dir($uploaddir)) {
        mkdir($uploaddir);
   ...
   $path = $uploaddir . $makFileName;
    ...
    if (move_uploaded_file($_FILES['upfile']['tmp_name'], $path)) {
        $file_to_read = "../uninorswitch/" . $serviceid . "/" . $makFileName;
        $file_data = file($path);
        $file_size = count($file_data);
        if ($file_size <= 50000) {
            $selMsgId = "select max(swid) from Inhouse_IVR.tbl_unim_switch_config";
            $qryMsg = mysql_query($selMsgId, $dbConn);
            list($swid) = mysql_fetch_array($qryMsg);
			
		$updateQuery = "UPDATE Inhouse_IVR.tbl_unim_switch_config SET filename='".$makFileName."' WHERE swid=".$swid;
