...
//$get_query="select batch_id,file_name,added_on,upload_for,total_file_count,service_id,status from ";
	//$get_query .=" master_db.bulk_message_history nolock where added_by='$_SESSION[loginId]' and status in(1,2) order by batch_id desc";
	
	
	$get_query = "select m.batch_id,m.file_name,m.added_by,m.added_on,m.status,m.total_file_count,m.service_id,md.message,m.upload_for,m.scheduleDate from master_db.bulk_message_history m INNER JOIN master_db.tbl_message_data md ON m.msg_id=md.msg_id order by m.added_on desc limit 20";
...
<TABLE class="table table-condensed table-bordered">
  ...
  <TR height="30">
	...
	<th align="left"><?php echo 'Uploaded By';?></th>
	<th align="left"><?php echo TH_ADDEDON;?></th>
	<th align="left"><?php echo 'Schedule For';?></th>
 ...
 <?php
while(list($batchId,$file_name,$addedby, $addedon,$status,$total_count, $service_id,$message,$upload_for,$scheduleFor) = mysql_fetch_array($query)) {
$sname_ks = array_flip($serviceArray);
?>
	  <TR height="30">
	  <TD><?php echo $batchId; 
	  $fileurl='http://10.43.248.137/hungamacare/smsbulkuploads/'.$service_id.'/'.$file_name.'.txt';
	  ?></TD>
		<TD><a href="<?php echo $fileurl;?>" target="_blank"><?php echo trim($file_name);?></a></TD>
		<TD><?php echo $addedby;?></TD>
		<TD><?php if(!empty($addedon)){echo date('j-M \'y g:i a',strtotime($addedon));} ?></TD>
	<TD><?php if(!empty($scheduleFor)){echo date('j-M \'y g:i a',strtotime($scheduleFor));}  ?></TD>
		<TD><?php 
