...
function get_queued($status, $xml,$smsc_id) {
    ...
    while (($y = XPathValue("smsc", $x)) != "") {
        ...
        if (substr(XPathValue("status", $y), 0, strlen($status)) == $status) {
			if(XPathValue("id", $y)==$smsc_id){	
				$n = XPathValue("queued", $y);
 			    $z = explode(" ", XPathValue("status", $y));
    ...
    while (($y = XPathValue("smsc", $x)) != "") {
        ...
        switch ($a[0]) {
            ...
            case "re-connecting":
                ...
                break;
            case "dead":
                echo "<span class=red>dead - Link Stoped Click Start to start link</span>";
                break;
            case "unkown":
